 ...
 * （会让未配置该变量的部署在启动期失败），不在 1.7 C 块范围内，需另行裁定。
 *
 * **这是已知的占位密钥，部署必须覆盖**（`docker-compose.yml` 亦有同字面量）。密钥红线约束的是「不得进入
 * 日志 / 响应 / 错误文案」，不是「不得出现在声明里」——保留字面量正是为了不改部署行为；上线前「改必填 +
 * 提供无密钥 env 模板」的收口与 review §8.1 第 5 条的连接串回退同批处理。
 */
