 ...
 *
 * 背景：机器断连/重连路径（经 `machine-instance-cleanup` 的 `convergeMachineInstances`，
 * 由宿主 core-bootstrap 在 `unregisterRemoteNode` / `registerRemoteNode` 重连分支触发）
 * 删除 core 实例与 supplement 后，若不同步清理编排域 AgentController
 * 活跃表与节点引用计数，断连期间产生的"幽灵实例"会永久计入环境并发额度
 * （maxConcurrency=1 时环境永久无法再 spawn），且引用计数残留导致节点空闲回收
 * 不触发。
 *
 * 本模块独立于 `machine-instance-cleanup` 保留：编排域收敛自带一处依赖注入口
 * （`setOrchestrationMachineCleanupDeps`，SP-C2 的 Y.Doc 实时回收接线）与一套专项用例，
 * 机器维度收敛只调用它，不承担这段接线；反过来它也不碰 core runtime 与实例登记表。
 */
 ...
 *
 * 调用方：`machine-instance-cleanup.convergeMachineInstances`（机器维度的实例收敛入口，
 * 删除 core 实例与登记表条目后调用）；本函数只清理编排域内存状态，不触碰 core runtime
 * （core 实例的删除由调用方完成）。
 *
