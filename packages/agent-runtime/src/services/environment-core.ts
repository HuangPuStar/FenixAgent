...
import type { EnvironmentResponse } from "../../../../src/types/api";
...
let environmentLifecyclePort: {
  closeAcpConnections(environmentIds: string[]): void;
  stopInstances(environmentIds: string[]): Promise<unknown>;
} | null = null;

/** 绑定 Environment 删除时关闭 ACP 连接的传输回调；缺失装配时明确失败。 */
export function bindEnvironmentAcpLifecyclePort(port: NonNullable<typeof environmentLifecyclePort>): void {
  environmentLifecyclePort = port;
}

/** 测试后解绑 Environment 生命周期回调，避免模块级装配状态泄漏。 */
export function resetEnvironmentAcpLifecyclePort(): void {
  environmentLifecyclePort = null;
}

/** 获取已装配的 Environment 生命周期回调；遗漏宿主装配时显式失败。 */
export function getEnvironmentAcpLifecyclePort(): NonNullable<typeof environmentLifecyclePort> {
  if (!environmentLifecyclePort) throw new Error("Environment ACP lifecycle port is not bound");
  return environmentLifecyclePort;
}
...
export async function deleteEnvironment(envId: string): Promise<boolean> {
  const lifecyclePort = getEnvironmentAcpLifecyclePort();
  lifecyclePort.closeAcpConnections([envId]);
  await lifecyclePort.stopInstances([envId]);
