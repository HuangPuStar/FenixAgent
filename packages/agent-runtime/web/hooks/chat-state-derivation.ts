// web/hooks/chat-state-derivation.ts
// 纯派生层：从两份 Y.Doc 投影出 ChatStateSnapshot 的两半（Chat Doc token / Session Doc meta）。
//
// 与 `use-chat-state.ts` 的分工（§4.7 三层拆分里的「纯模型」层）：本模块零 React 依赖、
// 无副作用，可从 doc 全量重建，因此能直接构造 Y.Doc 做单元测试；`use-chat-state.ts` 只剩
// 订阅与 DocHub 绑定，并再导出本模块的 `computeTokenSnapshot` / `computeMetaSnapshot`，
// 出口形状不变（`src/index.ts` 的 `export *` 与既有测试都按原路径引用）。
//
...
export interface ChatMetaSnapshot {
  sessionId: string;
  title: string | null;
  status: string;
...
export function computeChatSnapshot(token: ChatTokenSnapshot, meta: ChatMetaSnapshot): ChatStateSnapshot {
  return {
