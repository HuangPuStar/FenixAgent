...
| `web/chat/composer/CommandMenu.tsx`、`web/chat/css/chat-design-command-menu.css` | 命令/技能行的名称是蓝色 `/{name}` 文本（`#3d5f95`，能力面板内 `#58739d`），hover 与 `.is-active` 取 `#294d87` / `#f4f7fc`，选中勾选 `#5f83bd`；只有 MCP 行有 16px 图标列（技能行名称比 MCP 行名称左缩 22px）；行的提示与勾选是并列的网格子项 | 行首 `/` 改为 `SquareSlash` 图标并独立成网格首列，技能行与 MCP 行的名称列因此对齐；名称改菜单正文色 `#263247`，hover 底色改中性 `#f5f7fa`，勾选改与 MCP「已连接」同源的绿 `#25856e`，`.is-active` 左侧强调条改 `var(--color-brand)`；提示与勾选收进 `.chat-command-menu-tail`（源实现里两者同时出现的行会多出一个网格子项被挤到隐式第二行，行高翻倍）；插入草稿的文本仍是 `/${name} `，协议未变（2026-09-18） |
