 ...
 *
 * 来源：`packages/agent-runtime/web/components/chat/PeriTaskList.tsx` 逐字复制。
 *
 * 状态覆盖：
 ...
 *
 * 纯化改动点：`PeriTaskViewProjection` 改从包内 `../types` 导入（不引 `@fenix/chat-channel`）；
...
/** Peri Task 活动面板：头部计数/同步态 + 可折叠任务列表。复制自 `packages/agent-runtime/web/components/chat/PeriTaskList.tsx`。 */
  ...
  return (
    <div className="mx-auto max-w-3xl w-full px-4 sm:px-8 pt-1">
      <div className="rounded-lg border border-border bg-surface-2/50 overflow-hidden">
        ...
        >
          <span className="font-display font-medium text-text-secondary">{t("chat.components.periTask.title")}</span>
          {!loaded ? (
            <span className="flex items-center gap-1 text-text-muted" role="status">
              <Loader2 className="h-3 w-3 animate-spin" />
              {t("chat.components.periTask.syncing")}
            </span>
          ) : activeCount > 0 ? (
            <span className="flex items-center gap-1 text-status-running">
              <Loader2 className="h-3 w-3 animate-spin" />
              {t("chat.components.periTask.activeCount", { count: activeCount })}
            </span>
          ) : null}
        ...
        {reconnecting && (
          <div className="px-3 py-1 text-[10px] bg-warning-bg/60 text-warning-text" role="status">
            {t("chat.components.periTask.reconnecting")}
          </div>
        )}
          ...
          <div id={BODY_ID} className="max-h-64 overflow-y-auto overscroll-contain border-t border-border/50">
            {!loaded ? (
              ...
              <div className="px-3 py-2 text-[11px] text-text-muted" role="status">
                {t("chat.components.periTask.loading")}
              </div>
            ) : tasks.length === 0 ? (
              // 空态：已同步但当前会话暂无 Task 活动
              <div className="px-3 py-2 text-[11px] text-text-muted">{t("chat.components.periTask.empty")}</div>
            ) : (
