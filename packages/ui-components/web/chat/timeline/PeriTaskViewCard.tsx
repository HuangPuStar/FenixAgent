 ...
 *
 * 来源：`packages/agent-runtime/web/components/chat/PeriTaskViewCard.tsx` 逐字复制。
 *
 * 展示语义：
 ...
 *
 * 纯化改动点：`PeriTaskStatus` / `PeriTaskViewProjection` 改从包内 `../types` 导入
 * （不引 `@fenix/chat-channel`）；`cn` / Badge 改从包内相对路径导入；i18n key 加
 * `chat.components.` 前缀。
 */
...
import type { PeriTaskStatus, PeriTaskViewProjection } from "../types";
...
/** Peri Task 紧凑卡片：状态图标 + 标题/徽标 + 有界摘要 + 可选详情入口。复制自 `packages/agent-runtime/web/components/chat/PeriTaskViewCard.tsx`。 */
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {task.taskSubtype && (
            <Badge variant="outline" className="px-1.5 py-0 text-[10px]">
              {t(`chat.components.periTask.subtype.${task.taskSubtype}`)}
            </Badge>
          )}
          <Badge
          ...
          >
            {t(`chat.components.periTask.status.${task.status}`)}
          </Badge>
        </div>
        ...
        {task.detailAvailability !== "unavailable" && (task.summary || task.detailAvailability === "expired") ? (
          <div className="mt-1 flex items-center gap-2">
            {task.detailAvailability === "preview" ? (
              <p className="text-[10px] text-text-dim">{t("chat.components.periTask.previewOnly")}</p>
            ) : null}
            {onOpenDetail ? (
              <button
                type="button"
              ...
              >
                {t("chat.components.periTask.viewDetail")}
              </button>
            ) : null}
          </div>
        ) : null}
