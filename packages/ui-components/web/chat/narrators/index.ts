 ...
 * - `resolveToolCardKind` 现读取投影写入的 `tool.kind` / `tool.semantic`，不再调用宿主分类器。
 */
...
export function narrate(
  ...
  // 其他状态用过去时模板（"读取 X"）
  const titleKey =
    normalizedStatus === "running" ? "chat.toolNarrator.common.subtitleRunning" : "chat.toolNarrator.common.subtitle";
  const title = t(titleKey, { verb, object });

  // 第 5 阶段：拿状态词（全局统一）
  const statusLabel = t(`chat.toolNarrator.common.status.${normalizedStatus}`);
