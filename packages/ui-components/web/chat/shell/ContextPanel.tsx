...
import type { ThreadEntry, TokenUsage, ToolCallEntry } from "../types";

// =============================================================================
// ContextPanel — 方案 A 紧凑流式布局
//
// 来源：逐字复制 `packages/chat-channel/web/components/ContextPanel.tsx`。
// 纯化改动点：`@fenix/chat-channel` 的 `PromptUsage` → 包内 `TokenUsage`（字段一致）；
// 宿主 `@/src/lib/model-config-utils` 的 `simplifyModelDisplayName`、`@/src/lib/token-stats`
// 的 `formatTokenCount` 改为包内导入；`cn` 与 i18n 命名空间收敛到包内。
// =============================================================================

/** ContextPanel 属性。复制自源 `ContextPanel.tsx`。 */
interface ContextPanelProps {
  ...
  acpUsage?: TokenUsage | null;
}

/**
 * 会话上下文面板：模型信息、token 用量、工具调用统计与待确认队列。
 *
 * 复制自 `packages/chat-channel/web/components/ContextPanel.tsx`。
 * 纯化改动点：类型与工具函数改为包内导入，i18n 收敛到 `UI_COMPONENTS_NS`（键前缀 `chat.components.`）；
 * 组件内 `computeStats` / `simplifyToolName` 与包内 `../lib/token-stats`、`../lib/tool-call-utils`
 * 的同名函数口径不同（本面板统计工具调用次数并按小写工具名分桶），故保留本地实现，不复用 lib 版本。
 */
  ...
  return (
    <div className="relative flex shrink-0">
      ...
      <button
        ...
        onClick={onToggle}
        title={
          collapsed ? t("chat.components.contextPanel.showContext") : t("chat.components.contextPanel.hideContext")
        }
        aria-label={
          collapsed ? t("chat.components.contextPanel.showContext") : t("chat.components.contextPanel.hideContext")
        }
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[11px] text-text-muted mt-px font-mono">
                {modelName ? simplifyModelDisplayName(modelName) : t("chat.components.contextPanel.unknownModel")}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-1.5 mt-2">
            ...
            <span className="text-[10px] font-semibold text-accent-green uppercase tracking-[0.05em]">
              {t("chat.components.contextPanel.running")}
            </span>
        ...
        <div className="px-4 py-3 border-b border-border">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[11px] font-semibold text-text-secondary">
              {t("chat.components.contextPanel.tokenUsage")}
            </span>
          ...
          <div className="flex gap-3 mt-1.5">
            <span className="text-[10px] text-text-muted flex items-center gap-1">
              ...
              />
              {t("chat.components.contextPanel.input")}{" "}
            ...
            <span className="text-[10px] text-text-muted flex items-center gap-1">
              ...
              />
              {t("chat.components.contextPanel.output")}{" "}
              <span className="font-mono font-semibold text-text-secondary">
        ...
        <div className="px-4 py-3 flex-1">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[11px] font-semibold text-text-secondary">
              {t("chat.components.contextPanel.toolCalls")}
            </span>
            <span className="text-[10px] font-mono text-text-muted">{stats.totalToolCalls}</span>
          </div>
          {stats.totalToolCalls === 0 ? (
            <div className="text-[11px] text-text-muted py-1">{t("chat.components.contextPanel.noToolCalls")}</div>
          ) : (
            ...
            {stats.pendingTools.map((tool) => (
              <div key={tool.id} className="flex items-center gap-1.5 [&+&]:mt-1.5">
                ...
                <span className="text-[9px] font-bold text-accent-yellow uppercase ml-auto shrink-0">
                  {t("chat.components.contextPanel.pendingConfirmation")}
                </span>
              </div>
            ))}
          </div>
...
/**
 * 面板专用统计：工具调用次数 / 用户消息数 / 工具名分桶 / 待确认队列 + token 估算。
 * 与 `../lib/token-stats.computeStats` 口径不同（后者只返回 token 估算），故保留本地实现。
 */
...
function simplifyDisplayName(name?: string, t?: (key: string) => string): string {
  if (!name) return t?.("chat.components.contextPanel.defaultSession") ?? "默认";
  if (name.startsWith("env_")) return name.length > 16 ? `${name.slice(0, 16)}…` : name;
  if (name.length > 20) return `${name.slice(0, 18)}…`;
  return name;
}

/** 面板专用：把工具标题压成小写首词（分桶键），与 `../lib/tool-call-utils.simplifyToolName` 的 PascalCase 口径不同。 */
