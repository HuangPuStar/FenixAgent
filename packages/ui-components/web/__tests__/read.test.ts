...
import { readNarrator } from "../chat/narrators/read";
 ...
 *
 * 来源：`packages/agent-runtime/web/__tests__/read.test.ts` 逐字迁移，
 * 仅改写导入路径（narrator / 类型改指包内 `../chat/**`），
 * 并把 mockT 匹配的 i18n key 加上纯化后的 `chat.toolNarrator.` 前缀。
 */
