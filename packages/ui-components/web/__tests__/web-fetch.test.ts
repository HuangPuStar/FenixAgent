...
import { webFetchNarrator } from "../chat/narrators/web-fetch";
 ...
 *
 * 来源：`packages/agent-runtime/web/__tests__/web-fetch.test.ts` 逐字迁移，
 * 仅改写导入路径（narrator / 类型改指包内 `../chat/**`）；本用例为纯逻辑断言，
 * 不涉及宿主依赖，故无桩替身需要翻转为 prop。
 */
