...
import { skillNarrator } from "../chat/narrators/skill";
 ...
 *
 * 来源：`packages/agent-runtime/web/__tests__/skill.test.ts` 逐字迁移，
 * 仅改写导入路径（narrator / 类型改指包内 `../chat/**`）。
 * 用例与中文意图注释保持不变，无需 DOM / i18n 引导（被测模块只 import 包内纯逻辑）。
 */
