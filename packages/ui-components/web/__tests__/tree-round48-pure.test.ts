...
/**
 * narrators helpers 高缺口纯逻辑回归（round 48）。
 *
 * 来源：`packages/agent-runtime/web/__tests__/tree-round48-pure.test.ts` 逐字迁移，
 * 仅把导入路径换成包内同名实现（该实现为源文件语义等价复制）：
 * `../components/chat/narrators/helpers` → `../chat/narrators/helpers`，
 * `../lib/types` → `../chat/types`。所有 test 名称与上方中文意图注释保持不变。
 *
 * 测试范围：目录/文件输出识别、目录条目计数、display 元数据优先级、卡片类型判定。
