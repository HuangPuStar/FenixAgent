...
import { supportsFilePreview } from "../chat/lib/tool-call-utils";
import { extractDisplayMeta, resolveToolCardKind } from "../chat/narrators/helpers";
import type { ToolCallData } from "../chat/types";

/**
 * 工具卡片类型解析单测（display 采集链 + resolveToolCardKind 4 级解析链）。
 *
 * 来源：`packages/agent-runtime/web/__tests__/resolve-tool-card-kind.test.ts` 逐字迁移，
 * 仅把导入路径换成包内实现：`@/components/chat/narrators/helpers` → `../chat/narrators/helpers`、
 * `@/components/chat/tool-call-utils` → `../chat/lib/tool-call-utils`、
 * `@/src/lib/types` → `../chat/types`。断言与每个 test 上方的中文意图注释保持不变。
 *
