...
import { globNarrator } from "../chat/narrators/glob";
 ...
 *
 * 来源：`packages/agent-runtime/web/__tests__/glob.test.ts` 逐字迁移，
 * 仅改写导入路径（narrator / 类型改指包内 `../chat/**`），并把 mock t 的
 * key 对齐包内命名空间（`glob.files` → `chat.toolNarrator.glob.files`）。
 */
