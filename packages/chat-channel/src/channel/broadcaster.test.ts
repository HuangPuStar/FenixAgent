...
// YjsBroadcaster 测试（C6 迁移自 src/__tests__/yjs-frontend-broadcaster.test.ts，语义原样保留）：
// 按 rcsSessionId 广播隔离、64 KB 慢消费者背压、snapshot envelope、
...
import { createWs, type MockWs } from "./connection-test-helpers";
...
describe("YjsBroadcaster", () => {
  ...
  // 缓冲区超过 64KB 的慢连接必须跳过发送，等待其重连后获取 snapshot（慢消费者背压）。
  ...
  // unregister 后旧 Doc 的更新不能继续广播 yjs:update（防僵尸监听器）。
