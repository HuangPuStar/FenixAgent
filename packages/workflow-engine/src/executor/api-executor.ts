...
export class ApiExecutor extends RemoteExecutorBase {
  ...
  /** 执行单次 HTTP 请求 */
  private async doRequest(node: ApiNodeDef, ctx: NodeExecutionContext, signal: AbortSignal): Promise<NodeOutput> {
    ...
    try {
      ...
      if (error instanceof DOMException && error.name === "AbortError") {
        ...
        await this.emitNodeFailed(node.id, node.type, ctx, isExternalCancel ? "cancelled" : "timeout");
        throw new WorkflowError(
