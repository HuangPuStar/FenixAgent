...
import type { NodeDef, WorkflowDef, ShellNodeDef, AuditNodeDef } from '../../types/dag';
...
test('SKIPPED 状态沿依赖链深度传播', async () => {
  ...
  const skippedIds = new Set(
    events.filter((e) => e.type === 'node.skipped').map((e) => e.node_id),
  );
...
test('并行执行中取消全部节点', async () => {
  ...
  const completedIds = new Set(
    events.filter((e) => e.type === 'node.completed').map((e) => e.node_id),
  );
