...
import { buildReverseAdjacency, identifyParallelGroups, topologicalSort } from "../../scheduler/topological-sort";
...
// 线性 DAG：A → B → C
test("线性 DAG 按依赖顺序排序", () => {
  const nodes: NodeDef[] = [shellNode("C", ["B"]), shellNode("A"), shellNode("B", ["A"])];
...
test("扇出 DAG A 在 B/C 之前，B/C 在 D 之前", () => {
  const nodes: NodeDef[] = [shellNode("D", ["B", "C"]), shellNode("B", ["A"]), shellNode("A"), shellNode("C", ["A"])];
  const order = topologicalSort(nodes);
  expect(order.indexOf("A")).toBeLessThan(order.indexOf("B"));
...
test("环检测抛出 WorkflowError", () => {
  const nodes: NodeDef[] = [shellNode("A", ["B"]), shellNode("B", ["C"]), shellNode("C", ["A"])];
...
// 自环
test("自环检测", () => {
  const nodes: NodeDef[] = [shellNode("A", ["A"])];
  expect(() => topologicalSort(nodes)).toThrow(WorkflowError);
});

// 未知依赖
test("依赖不存在的节点抛出错误", () => {
  const nodes: NodeDef[] = [shellNode("A", ["Z"])];
  expect(() => topologicalSort(nodes)).toThrow(WorkflowError);
...
test("identifyParallelGroups 环检测", () => {
  const nodes: NodeDef[] = [shellNode("A", ["B"]), shellNode("B", ["A"])];
...
// 独立节点
test("独立节点反向邻接表为空", () => {
  const nodes: NodeDef[] = [shellNode("A"), shellNode("B")];
  const adj = buildReverseAdjacency(nodes);
