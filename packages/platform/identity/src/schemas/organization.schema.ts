...
/**
 * `/web/organizations` 与 `/web/api-keys` 的请求与响应 schema。
 *
 * 迁移自 `packages/resources/identity-admin/src/server/schemas/organization.schema.ts`
 * （CE 阶段 2 任务 1.2）。形状与描述保持原样；`WebOkSchema` 的来源由宿主
 * `@server/schemas/common.schema` 换为 `@fenix/platform-sdk`：`/web/*` 信封是跨模块协议契约，
 * 只能在平台契约包里定义一处，identity 不能导入宿主实现（ce-ee-engineering-standards §2.3）。
 */
