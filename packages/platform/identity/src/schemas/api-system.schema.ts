...
/**
 * `/api/system/*` 身份管理面的请求与响应 schema。
 *
 * 迁移自 `packages/resources/identity-admin/src/server/schemas/api-system.schema.ts`
 * （CE 阶段 2 任务 1.2）。形状与字段描述保持原样；唯一变化是 `ApiSystemErrorResponseSchema`
 * 不再在本地定义，改为从 `@fenix/platform-sdk` 重导出——该错误响应面由 identity 与 observer
 * 共同贡献，形状必须只有一处定义（见 `protocol/system-api.ts`），复制定义会漂移。
 */

export { ApiSystemErrorResponseSchema } from "@fenix/platform-sdk";
