...
import type { SystemApiRouteDependencies } from "../dependencies";

/**
 * `/api/system/*` 路由（CE 阶段 2 任务 1.2）。
 *
 * 从 `packages/resources/identity-admin` 迁入并改为工厂：守卫由宿主注入，理由见 `../dependencies`。
 *
 * **这是已发布合同**：路径、请求/响应形状与错误码必须逐字保持。系统 API 用独立的
 * `RCS_SYSTEM_API_KEYS`（CLAUDE.md「认证、组织与密钥」），不得与普通请求认证混用。
 */
...
/** `/api/system/*` 路由工厂；守卫由宿主注入（`RCS_SYSTEM_API_KEYS` 解析留在宿主 env 真相来源侧）。 */
export function createApiSystemRoutes(deps: SystemApiRouteDependencies) {
  const app = new Elysia({ name: "api-system", prefix: "/api/system" }).use(deps.systemApiGuardPlugin).model({
