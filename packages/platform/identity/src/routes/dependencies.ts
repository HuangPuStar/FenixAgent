 ...
 * `/web/*` 身份路由的宿主注入依赖。
 *
 * 只声明真正需要的守卫：`/web/*` 靠 `authGuardPlugin` 取得 `authContext` 与 `uuid`，与系统
 * API key 守卫无关；让两个工厂共用一份依赖包会迫使宿主为不需要的守卫编造值。
 */
export interface WebIdentityRouteDependencies {
  readonly authGuardPlugin: AnyElysia;
}

/** `/api/system/*` 身份路由的宿主注入依赖：系统 API key 守卫（`RCS_SYSTEM_API_KEYS`）。 */
export interface SystemApiRouteDependencies {
