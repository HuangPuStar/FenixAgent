...
export function createChatDoc(userId: string, agentId: string, redis: RedisConn | null): ChatDoc {
  const docName = `chat:${userId}:${agentId}`;
...
export function loadChatDoc(userId: string, agentId: string, redis: RedisConn | null): ChatDoc {
  const docName = `chat:${userId}:${agentId}`;
  const ydoc = new Y.Doc({ guid: docName });
...
export function createSessionDoc(acpSessionId: string, redis: RedisConn | null): SessionDoc {
  const docName = `session:${acpSessionId}`;
  const ydoc = new Y.Doc({ guid: docName });

  const meta = ydoc.getMap("meta");
  meta.set("status", "idle");
  meta.set("acpSessionId", acpSessionId);
...
export function loadSessionDoc(acpSessionId: string, redis: RedisConn | null): SessionDoc {
  const docName = `session:${acpSessionId}`;
  const ydoc = new Y.Doc({ guid: docName });
  ...
  if (!ydoc.getMap("meta").has("status")) {
    ...
    ydoc.getMap("meta").set("acpSessionId", acpSessionId);
  }
