...
export async function installSkills(
  ...
  const tempRoot = await mkdtemp(join(dirname(skillsDir), ".peri-skills-"));
  ...
  if (skills.length === 0) {
    try {
      ...
      console.log(`[peri-skill-installer] 无 skills 需要安装, workspace=${workspace}`);
      return [];
    } finally {
  ...
  console.log(
    `[peri-skill-installer] 开始安装 ${skills.length} 个 skills: workspace=${workspace}, skillsDir=${skillsDir}`,
  );
  ...
  try {
    ...
    for (const skill of skills) {
      ...
      console.log(`[peri-skill-installer] 下载 skill "${skill.name}"`);
      ...
      if (!response.ok) {
        console.error(
          `[peri-skill-installer] 下载 skill "${skill.name}" 失败: status=${response.status} ${response.statusText}`,
        );
        throw new Error(`Failed to download skill '${skill.name}': ${response.status} ${response.statusText}`);
      }

      const archiveBuffer = Buffer.from(await response.arrayBuffer());
      console.log(`[peri-skill-installer] 下载 skill "${skill.name}" 成功: 大小=${archiveBuffer.length} bytes`);
      await writeFile(archivePath, archiveBuffer);
      await extractArchive(archivePath, stagedTargetDir);
      console.log(`[peri-skill-installer] 解压 skill "${skill.name}" 完成: targetDir=${installedTargetDir}`);
    ...
    await replaceInstalledSkills(skillsDir, stagedSkillsDir);
    console.log(`[peri-skill-installer] 全部 skills 安装完成: 共 ${installed.length} 个`);
    return installed;
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
  }
