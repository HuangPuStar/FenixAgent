...
function createFixture(
  ...
  scheduleInterval?: (callback: () => void) => ReturnType<typeof globalThis.setInterval>,
) {
  ...
  const handle = createRelayHandle(
    { instanceId: "lifecycle-test", port: 8123, token: "token-test" },
    {
      ...
      keepAliveIntervalMs,
      setInterval: scheduleInterval ? (callback) => scheduleInterval(callback) : undefined,
      clearInterval: () => {},
    },
  );
...
describe("Peri relay handle 生命周期补充", () => {
