...
function launchSpec(overrides: Partial<AgentLaunchSpec> = {}): AgentLaunchSpec {
  return {
    ...
    env: { FEATURE: "enabled" },
    agent: { name: "test-agent", prompt: "test prompt" },
    model: {
      provider: "test",
      protocol: "openai",
...
type RuntimeFakes = {
  dependencies: PeriRuntimeDependencies;
  allocated: number[];
  released: number[];
  starts: string[];
  stops: string[];
...
function createFakes(overrides: Partial<PeriRuntimeDependencies> = {}): RuntimeFakes {
  ...
  return {
    ...
    dependencies: {
      ...
      prepareWorkspaceEnvironment: async (workspace) => {
        prepared.push(workspace);
        return {
          runtimeDir: join(workspace, ".claude"),
          skillsDir: join(workspace, ".claude", "skills"),
          configPath: join(workspace, ".claude", "settings.local.json"),
        };
      },
      writePeriSettings: async (workspace) => {
        settings.push(workspace);
        return join(workspace, ".peri", "settings.json");
      },
      portAllocator,
      processManager,
      ...overrides,
...
async function prepare(runtime: ReturnType<typeof createPeriRuntime>, instanceId = "instance"): Promise<void> {
  await runtime.prepareEnvironment({ instanceId, launchSpec: launchSpec({ environmentId: `env-${instanceId}` }) });
}

async function start(runtime: ReturnType<typeof createPeriRuntime>, instanceId = "instance"): Promise<void> {
  await prepare(runtime, instanceId);
  await runtime.startInstance({ instanceId });
}

describe("peri-runtime 注入式生命周期", () => {
  // 未 prepare 的实例不允许启动，避免无 workspace 的进程资源泄漏。
  test("拒绝在 prepare 前启动", async () => {
    const runtime = createPeriRuntime(createFakes().dependencies);
  ...
  test("prepare 在缺少 environmentId 时使用用户 workspace", async () => {
    ...
    expect(runtime.getInstanceState("no-env")?.workspace).toMatch(/test-org[/\\]test-user$/);
  });
  ...
  test("connectRelay 使用注入的 fake relay", async () => {
    ...
    const runtime = createPeriRuntime(createFakes({ connectRelay: async () => relay }).dependencies);
    await start(runtime);
  ...
  test("connectRelay 复用打开的 relay", async () => {
    let connections = 0;
    const relay = openRelay();
    const runtime = createPeriRuntime(
      createFakes({
        connectRelay: async () => {
          connections += 1;
          return relay;
