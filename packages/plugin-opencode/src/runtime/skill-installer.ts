...
export async function installSkills(
  ...
  console.log(`[skill-installer] 开始安装 ${skills.length} 个 skills: workspace=${workspace}, skillsDir=${skillsDir}`);
  ...
  try {
    ...
    for (const skill of skills) {
      ...
      if (!response.ok) {
        console.error(
          `[skill-installer] 下载 skill "${skill.name}" 失败: status=${response.status} ${response.statusText}`,
        );
      ...
      console.log(`[skill-installer] 解压 skill "${skill.name}" 完成: targetDir=${installedTargetDir}`);
