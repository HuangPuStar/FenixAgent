// packages/web-runtime/web/chat/chat-doc-to-structured.ts
// 新 schema（文档 5.2 Chat Doc：entries/blocks）→ 展示层 StructuredMessage[] 的投影与增量派生。
//
// 与 `structured-to-thread.ts` 的分工（§4.7 按职责切）：本文件只碰 Y.Doc——entry/toolCall 结构、
// per-entry 派生缓存与失效观察者；StructuredMessage → ThreadEntry 的展示映射留在那边。
// 本模块的公开函数由 `structured-to-thread.ts` 原样再导出，`@fenix/web-runtime/chat/structured-to-thread`
// 的对外形状不变。
...
import type { AssistantChunk } from "@fenix/ui-components/chat/types";
