...
import { resolveWorkspacePath as computeWorkspacePath, environmentRepo } from "@fenix/agent-runtime/server";
...
/** 判断相对 workspace 路径是否包含黑名单条目。 */
export function shouldHidePath(path: string): boolean {
  return path
    .split("/")
    .filter(Boolean)
