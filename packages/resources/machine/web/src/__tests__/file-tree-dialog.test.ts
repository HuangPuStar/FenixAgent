...
describe("file tree dialogs", () => {
  ...
  test("clears sticky folder at the top of the tree", () => {
    const componentPath = join(webSourceRoot, "components", "agent-panel", "file-tree-view.tsx");
  ...
  test("validates basename without changing legal surrounding spaces", () => {
    ...
    const componentPath = join(webSourceRoot, "components", "agent-panel", "FileTreeTab.tsx");
    const source = fs.readFileSync(componentPath, "utf-8");
  ...
  test("rejects basenames longer than 255 UTF-8 bytes", () => {
    ...
    const dialogPath = join(webSourceRoot, "components", "agent-panel", "file-tree-input-dialog.tsx");
  ...
  test("renders a sticky folder indicator for arborist trees", () => {
    const stylesheetPath = join(webSourceRoot, "pages", "agent-panel", "artifacts-workspace.css");
  ...
  test("renders file tabs above the preview content", () => {
    const componentPath = join(webSourceRoot, "components", "agent-panel", "artifacts-files-workspace.tsx");
    const source = fs.readFileSync(componentPath, "utf-8");
  ...
  test("does not use window.prompt in file tree components", () => {
    const componentDir = join(webSourceRoot, "components", "agent-panel");
