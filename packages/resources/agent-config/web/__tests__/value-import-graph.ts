...
// 值导入图遍历器（守卫的**工具**侧，策略与断言在同目录的 agent-config-browser-surface.test.ts）。
// 自 `packages/resources/sandbox/web/__tests__/value-import-graph.ts` 逐字复制：沙盒是 1.3 的黄金样本，
// 守卫口径必须同源，各包只改策略与白名单，不改遍历语义。
//
