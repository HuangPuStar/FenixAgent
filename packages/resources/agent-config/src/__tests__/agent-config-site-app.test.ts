...
describe("agent-config-site-app service", () => {
  ...
  test("listAgentSiteAppIds 返回绑定 siteAppId 数组", async () => {
    ...
    const { listAgentSiteAppIds } = await import("../server/services/config/agent-config-site-app");
  ...
  test("syncAgentSiteApps 空数组只删不插", async () => {
    ...
    const { syncAgentSiteApps } = await import("../server/services/config/agent-config-site-app");
