...
describe("config agent resource access", () => {
  ...
  test("listAgentConfigs 返回内部和外部共享 Agent", async () => {
    ...
    const { listAgentConfigs } = await import("../server/services/config/agent-config");
  ...
  test("getAgentConfigByResourceKey 按授权返回外部 Agent", async () => {
    ...
    const { getAgentConfigByResourceKey } = await import("../server/services/config/agent-config");
  ...
  test("assertAgentConfigInternalWritable 拒绝外部 Agent", async () => {
    ...
    const { assertAgentConfigInternalWritable } = await import("../server/services/config/agent-config");
  ...
  test("createAgentConfig 和 updateAgentConfig 透传 publicReadable", async () => {
    ...
    const { createAgentConfig, updateAgentConfig } = await import("../server/services/config/agent-config");
