...
import { toAgentConfigWriteData } from "./config/agent-config";

/**
 * Meta Agent 编排需要的主体投影。
 *
 * 只声明用到的三个字段（§6.4 对宿主 `AuthContext` 的第一种改法）：这些编排入口既被宿主启动流程调用
 * （`sync-builtin`），也被请求路径调用（`/web/meta-agent/ensure`），两处的上下文都直接满足本形状，
 * 不需要把它升格成平台契约或让调用方伪造 `ActorContext`。`role` 只写进 API Key 的 metadata，
 * 不参与任何授权判断——本文件走的是 Skill/AgentConfig 的系统路径（无请求主体）。
 */
...
export type MetaAgentModelResolver = (ctx: MetaAgentContext) => Promise<string | null>;
...
async function resolveDefaultMetaModelRef(ctx: MetaAgentContext): Promise<string | null> {
 ...
 */
export async function syncBuiltinSkills(ctx: MetaAgentContext): Promise<void> {
...
async function listBuiltinSkillIds(_ctx: MetaAgentContext): Promise<string[]> {
...
export async function syncBuiltinSkillsToSystemAdmin(
  ...
  deps: {
    syncBuiltinSkills?: (ctx: MetaAgentContext) => Promise<void>;
    listBuiltinSkillIds?: (ctx: MetaAgentContext) => Promise<string[]>;
 ...
 */
async function ensureMetaConfig(ctx: MetaAgentContext): Promise<string> {
...
async function ensureMetaApiKey(ctx: MetaAgentContext, headers: Headers, deps: MetaAgentDependencies): Promise<string> {
