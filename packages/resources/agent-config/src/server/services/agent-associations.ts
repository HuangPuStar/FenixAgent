...
import { listAgentMcpIds, syncAgentMcps } from "../repositories/agent-config-mcp";
import { listAgentSkillIds, syncAgentSkills } from "../repositories/agent-config-skill";
 ...
 *
 * 绑定表并不都在本包：`agent_config_skill` / `agent_config_mcp` 随 Agent 配置聚合归本包（任务 1.7 B7），
 * 知识库绑定归 knowledge、记忆开关归 memory。但"谁拥有哪张绑定表"这件事只应由本模块知道：路由与
 * Facade 通过它读写绑定，不直接 import 资源包，避免每加一种关联就在协议层散落一处跨包导入。
 *
