 ...
 * 组装对外 Agent 列表项。
 */
async function toAgentListItem(
  ctx: AuthContext,
  agent: Awaited<ReturnType<typeof routeConfigDeps.listAgentConfigs>>[number],
) {
  return {
    id: agent.id,
    name: agent.name,
...
async function buildAgentDetail(ctx: AuthContext, agentId: string) {
  const agent = await routeConfigDeps.getReadableAgentConfigById(ctx, agentId);
...
async function listWritableAgents(ctx: AuthContext) {
  const rows = await routeConfigDeps.listAgentConfigs(ctx);
...
app.get(
  ...
  async ({ store, query, error }: any) => {
    ...
    try {
      const agents = await routeConfigDeps.listAgentConfigs(authCtx);
...
app.post(
  ...
  async ({ store, body, error }: any) => {
    ...
    try {
      ...
      const agentId = await routeConfigDeps.createAgentConfig(authCtx, payload.name, toAgentData(payload), {
...
app.put(
  ...
  async ({ store, params, body, error }: any) => {
    ...
    try {
      const existing = await routeConfigDeps.getAgentConfigById(params.id, authCtx.organizationId);
      if (!existing) {
        return error(404, { error: { code: "NOT_FOUND", message: `Agent '${params.id}' not found` } });
      }

      await routeConfigDeps.updateAgentConfig(authCtx, existing.name, toAgentData(payload), {
        publicReadable: payload.publicReadable,
      });
...
app.delete(
  ...
  async ({ store, params, error }: any) => {
    ...
    try {
      ...
      const deleted = await routeConfigDeps.deleteAgentConfig(authCtx, existing.name);
      if (!deleted) {
        return error(404, { error: { code: "NOT_FOUND", message: `Agent '${params.id}' not found` } });
      }
