...
describe("config skill resource access", () => {
  ...
  test("listSkills 返回内部和外部同名 skill", async () => {
    ...
    const { listSkills } = await import("../server/services/config/skill");
  ...
  test("getSkill 无内部同名时返回外部授权 skill", async () => {
    ...
    const { getSkill } = await import("../server/services/config/skill");
  ...
  test("upsertSkill publicReadable 创建公开授权", async () => {
    ...
    const { upsertSkill } = await import("../server/services/config/skill");
  ...
  test("deleteSkill 拒绝删除外部 skill", async () => {
    ...
    const { deleteSkill } = await import("../server/services/config/skill");
