 ...
 *
 * 历史（2026-09-20 实测）：本包 web 自身一直零别名（硬条件，见「本包源码不残留宿主别名」），但
 * `@fenix/identity/web` 当时仍有 `@/src`、`@/components`；本包改不动也不该替它掩盖，于是把债务
 * **钉到具体目录**（`UPSTREAM_ALIAS_DEBT_DIR`）。§1.6 T4 把身份包的别名归零后，这份白名单失去
 * 唯一成员，原地删除并升级为严格断言：任何包往本包图里渗别名都直接失败。
 */
...
describe("skill web 入口浏览器可达面", () => {
  ...
  test("遍历有效性自检：包内模块与跨包 exports 目标都在到达集合中", () => {
    ...
    // web-runtime 的 request / 命名空间 / org-session 契约）。少了这一段，「@fenix/* 被当成
