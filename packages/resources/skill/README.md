...
- **宿主页面与别名的 owner 切换已落地（§1.6 T11e）**：`apps/web/src/routes/agent/_panel/skills.tsx` 已直连 `@fenix/resource-skill/web`；`apps/web/src/__tests__/agent-form-dialog-pure-logic.test.ts` 仍按 `@/src/lib/skill-resource-access` 别名取本包的纯逻辑助手（宿主用例，别名表随 T11e 收尾批次处理）。此条原列的 `apps/web/src/components/PermissionTab.tsx` 已不存在。
