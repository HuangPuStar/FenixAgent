...
- **表定义迁出后无需改句柄类型（B5 已核实）**：`SkillDatabase` 刻意不写 `typeof schema`，只做 `select` / `insert` / `update` / `delete`，不使用 `db.query.*` 关系查询，迁表后形状未改动。
- **宿主页面与别名的 owner 切换已落地（§1.6 T11e）**：`apps/web/src/routes/agent/_panel/skills.tsx` 已直连 `@fenix/resource-skill/web`；`apps/web/src/__tests__/agent-form-dialog-pure-logic.test.ts` 已随 §1.6 T11e-4a 改从 `@fenix/resource-skill/web` 取本包的纯逻辑助手（用例本身仍在宿主，归 §1.6 的归位登记项），`@/src/lib/skill-resource-access` 别名随之无消费方并随 T11e-4b 删除（2026-09-21 实测 0 命中）。此条原列的 `apps/web/src/components/PermissionTab.tsx` 已不存在。
