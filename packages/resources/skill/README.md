...
- **别名清零**：`web/**` 内 `@/…` 宿主别名与越出到 `apps/web/src/**` 的相对路径各 0 处；跨包引用改为 `@fenix/ui-components/*`、`@fenix/web-runtime/*` 与 `@fenix/identity/web`。
...
- **`@fenix/identity/web`**（`AgentSkillsPage.tsx:1` 的 `useOrg`）1 处，命中架构门禁 `special-dependency`（§2.3 禁止 resource → platform-impl）。本波次 skill / mcp / knowledge 同形（宿主只维护一个组织上下文实现），台账条目已登记（`special-dependency`，owner 1.6，removeWhen 指向 §1.6 WebShell），不各自发明第二份上下文。
