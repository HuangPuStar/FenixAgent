...
import { getMcpServerModule, resetMcpServerModule } from "../server/runtime";
...
beforeEach(() => {
  resetAllStubs();
  resetMcpServerModule();
});

// 工厂必须产出真实模块实例（而不是装配结果的命名空间包装）并装入进程级槽位：路由与系统路径都在
// 调用时经 `getMcpServerModule()` 读同一份。
test("createMcpModule 产出真实例并装入进程级槽位", () => {
  const module = createMcpModule(factoryContext(new Map([["access-control", accessControlSuite]])));

  expect(module.resource).toBe(mcpServerResource);
  expect(module.identity).toBeDefined();
  expect(getMcpServerModule()).toBe(module);
});

// 授权端口缺失必须当场失败：静默退让会让所有 MCP 端点以「资源不存在」响应，把装配故障伪装成业务结果。
test("access-control 未提供授权端口时拒绝装配", () => {
  expect(() => createMcpModule(factoryContext(new Map([["access-control", { accessControl: {} }]])))).toThrow(
