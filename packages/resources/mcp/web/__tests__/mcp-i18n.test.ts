// web/__tests__/mcp-i18n.test.ts
// 守护 mcp 字典的完整性：en/zh 键集一致、插值占位符一致、源码里写死的 t("key") 都能查到、
...
const EN = JSON.parse(readFileSync(join(WEB_ROOT, "i18n/locales/en/mcp.json"), "utf8")) as Record<string, unknown>;
const ZH = JSON.parse(readFileSync(join(WEB_ROOT, "i18n/locales/zh/mcp.json"), "utf8")) as Record<string, unknown>;
/**
 * 字典落点：宿主 `apps/web/src/i18n/index.ts` 经 `@fenix/resource-mcp/web/i18n` 消费这两个文件，
 * 包内 `web/i18n/index.ts` 也指向同一批 JSON，路径漂移会静默丢文案。
 */
const JSON_PATHS = ["i18n/locales/en/mcp.json", "i18n/locales/zh/mcp.json"] as const;
...
describe("mcp 字典完整性", () => {
  ...
  test("en / zh 键集完全一致", () => {
    ...
    expect(enFlat.size).toBeGreaterThanOrEqual(130);
  });
  ...
  test("源码中的字面量 t() 键都在字典内", () => {
    ...
    expect(literalKeys.size).toBeGreaterThanOrEqual(70);
  });

  // 动态键族无法被字面量扫描覆盖，逐个键族点名断言：漏一个就是界面上多出一串 key。
  // 注：`mcp-resource-access` 的 `getMcpResourceBadgeKey` 返回的 `resource.*` 三个键属于共享的
  // `components` 命名空间（本包页面不渲染角标，宿主与 machine 的用例才消费该 helper），
  // 不要求出现在本包字典里。
  test("动态键族齐备（校验提示、服务器类型、OAuth 字段）", () => {
    for (const key of [
      // agent-mcp-utils 的 McpImportError 与 validateMcpEditor 返回值
      "validation.nameRequired",
      "validation.nameNoDoubleHyphen",
      "validation.namePattern",
      "validation.nameTooLong",
      "validation.commandRequired",
  ...
  // 键的最终所在地 = 包的 owner：mcp 的键本就注册在 `mcp` 命名空间，不得出现寄居在其他命名空间的
  // 前缀（如 observer 迁出后残留的 `mcp.` 前缀键），否则宿主对不上命名空间。
  test("字典中不存在带命名空间前缀的寄居键", () => {
    const nested = [...enFlat.keys()].filter((key) => key.startsWith("mcp."));
    expect(nested).toEqual([]);
  });

  // JSON 文件路径是宿主消费这批文案的合同（`apps/web/src/i18n/index.ts` 经包出口 `./web/i18n`
  // 取用），迁移只允许增删键、不允许改路径；`web/i18n/index.ts` 必须指向同一批文件。
  ...
  // 命名空间常量由本包声明（键归属本包）；宿主 `NS.MCP` 与这里的字面量必须同值。
  test("命名空间常量与字典文件名一致", () => {
    const namespaceSource = readFileSync(join(WEB_ROOT, "i18n/namespace.ts"), "utf8");
    expect(namespaceSource).toContain('export const MCP_NS = "mcp"');
    const entrySource = readFileSync(join(WEB_ROOT, "i18n/index.ts"), "utf8");
    expect(entrySource).toContain("mcpResources");
  });
});
