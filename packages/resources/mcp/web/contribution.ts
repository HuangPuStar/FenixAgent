// web/contribution.ts
// mcp 向 WebShell 贡献的浏览器面能力（§1.6 T11b5）。
//
...
//   config 组内 mcp 排第 5，故取 50；同组内其它包的项落在各自的步长上，留出的空档让后续
//   插入不必重排。迁移前后侧栏顺序逐项一致。
// - **文案 owner 是本包的 `mcp` 命名空间**（`nav.mcp` 一条）。宿主 `agentPanel` 字典里的
//   `mcp` 键（迁移前以 `agentPanel:mcp` 形式被引用）在 Shell 改由 registry 渲染的那一片删除：
//   两个字典都持有同一批文案会让宿主删键时静默打断侧栏（§7.26）。
// - **图标随项贡献**：`Plug` 与迁移前逐字相同，侧栏外观零变化。
//
...
import { MCP_NS } from "./i18n/namespace";

export const webContribution: WebAppContribution = {
  navigation: [{ id: "mcp", groupId: "config", order: 50, ns: MCP_NS, labelKey: "nav.mcp", icon: Plug }],
};
