...
// 值导入图遍历器（守卫的**工具**侧，策略与断言在同目录的 task-browser-surface.test.ts）。
//
// 与 `packages/resources/sandbox/web/__tests__/value-import-graph.ts` 逐字同源：各包守卫不得依赖
// 仓库根 `scripts/` 或另一个包的内部文件（包单独检出/独立构建时那里不存在），因此按包各持一份；
// 口径漂移会被两侧测试同时暴露（见文件尾部的边界说明）。
//
