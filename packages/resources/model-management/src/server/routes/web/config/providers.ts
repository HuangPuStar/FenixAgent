...
app.get(
  ...
  safeWebHandler(async (ctx, actor) => {
    ...
    return typeof raw === "string" && raw.length > 0 ? handleProviderGet(actor, raw) : handleProviderList(actor);
  }),
  {
    sessionAuth: true,
...
app.put(
  "/config/providers",
  safeWebHandler(async (ctx, actor) =>
    handleProviderSet(actor, requireName(ctx.query), (ctx.body ?? {}) as Record<string, unknown>),
  ),
  {
    sessionAuth: true,
    query: nameQuerySchema,
...
app.post(
  ...
  safeWebHandler(async (ctx, actor) => {
    ...
    return handleFetchModels(actor, requireName(ctx.query), {
...
app.post(
  ...
  safeWebHandler(async (ctx, actor) => {
    ...
    return handleTestModel(actor, requireName(ctx.query), modelId);
  }),
  {
    sessionAuth: true,
    query: nameQuerySchema,
...
app.delete(
  "/config/providers/actions/models/:modelId",
  safeWebHandler(async (ctx, actor) => handleRemoveModel(actor, requireName(ctx.query), ctx.params.modelId as string)),
  {
    sessionAuth: true,
    query: nameQuerySchema,
    response: {
