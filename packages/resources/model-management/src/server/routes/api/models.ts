...
import { modelWriteDataFromApi } from "../../services/model-write-data";
import type { ApiModelManagementRouteDependencies } from "../dependencies";

/**
 * `/api/models` 协议层（对外已发布合同）。目录按计划 §2.3 收敛到 `src/server/routes/**`。
 *
...
/**
 * 构造 `/api/models` 路由（已发布对外合同，响应字段逐字不变）。
 *
 * 会话守卫由宿主注入：`/api` 与 `/web` 共用同一个认证解析（session cookie / API Key），包内自建的
 * 实例拿不到宿主的 `store.actor`，会把所有请求静默变成缺少 actor 的 401。
 */
export function createApiModelsRoutes(deps: ApiModelManagementRouteDependencies) {
  const { authGuardPlugin } = deps;
  ...
  app.delete(
    ...
    async ({ store, params, error }: any) => {
      ...
      try {
        ...
        const { facade } = getModelManagementModule();
        const result = await facade.removeModel(
          actor,
          { by: "resourceId", value: providerId },
          { by: "id", value: id },
        );
