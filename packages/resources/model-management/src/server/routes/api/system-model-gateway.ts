...
import type { SystemApiModelManagementRouteDependencies } from "../dependencies";
...
/**
 * 构造 `/api/system/model-gateway` 路由。
 *
 * 系统 key 守卫由宿主注入（Elysia 的 `macro` / `state` 是实例作用域的，包内自建会让同一进程出现两套
 * 互不可见的认证状态），见 `../dependencies`。模块配置在请求期经 `getModelManagementConfig()` 读取：
 * 装配期宿主可能尚未完成基础设施初始化。
 */
export function createApiSystemModelGatewayRoutes(deps: SystemApiModelManagementRouteDependencies) {
    ...
    .post(
      ...
      async ({ body, status }) => {
        try {
          ...
          return status(500, {
            error: { code: "MODEL_GATEWAY_ERROR", message: "Unable to remove model gateway keys" },
          });
        }
      },
      {
