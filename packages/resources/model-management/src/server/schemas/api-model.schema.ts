/**
 * `/api/models`、`/api/providers`（对外稳定接口）的请求 / 响应契约。
 *
 * 从宿主 `apps/server/src/schemas/api-model.schema.ts` 迁入：该文件在宿主侧**零消费者**（实测
 * `command grep -rn "api-model.schema" apps packages` 只命中本包的 `src/routes/api/models.ts`），
 * 属于「随资源迁移的协议」而不是宿主协议。宿主那份的删除登记在 `sharedPatches`。
 *
 * `resourceAccess` 仍是**唯一**保留旧字段形状的位置（决策 D2 的对外兼容面）：
 * `toResourceAccessView` 从 `scope + access.actions` 派生它，`/web` 侧则不再出现该字段。
 */
...
export const ApiProviderListItemSchema = z
  .object({
    ...
    resourceAccess: ResourceAccessViewSchema.optional().describe("资源访问控制信息。"),
  })
