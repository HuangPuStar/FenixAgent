...
- **宿主残留副本**：`apps/web/src/lib/use-workflow-events.ts` 已删除，唯一实现为包内 `web/lib/use-workflow-events.ts`。编辑器经 `web/index.ts` 发布，宿主 route adapter 不穿透 `web/pages/**`。旧的零消费 `WorkflowPage` 与专属路径工具已删除，四个视图由宿主文件路由分别渲染；与死页无关的 location 写操作 P0 守卫保留在 `web/__tests__/web-location-write-guard.test.ts`。
