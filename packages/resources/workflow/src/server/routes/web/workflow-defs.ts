...
export function createWebWorkflowDefsRoutes(deps: WorkflowRouteDependencies) {
  const app = new Elysia({ name: "web-workflow-defs" }).use(deps.authGuardPlugin).model({
      ...
      try {
        switch (action) {
          ...
          case "getVersion": {
            ...
            if (!yaml)
              return error(404, { success: false, error: { code: "NOT_FOUND", message: "Version not found" } });
