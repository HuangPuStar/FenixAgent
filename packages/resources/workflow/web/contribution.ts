// web/contribution.ts
// workflow 向 WebShell 贡献的浏览器面能力（§1.6 T11b）。
//
...
//   `core` 组内 workflow 排第 3，故取 30；同组内其它包的项落在各自的步长上，留出的空档让后续
//   插入不必重排。迁移前后侧栏顺序逐项一致。
// - **文案 owner 是本包 `workflows` 命名空间的 `nav.workflow`**，取值与迁移前宿主 `agentPanel` 字典的
//   `workflow` 键逐字相同。该键在 Shell 改由 registry 渲染的那一片（T11d）删除：两个字典都持有同一批
//   文案会让宿主删键时静默打断侧栏（§7.26）。
// - **图标随项贡献**：`Workflow` 与迁移前逐字相同，侧栏外观零变化。
//
