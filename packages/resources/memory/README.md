...
`bun test packages/resources/memory`：**107 pass / 0 fail / 276 expect() calls / 10 文件**（`env -u ANTHROPIC_MODEL` 消除会话环境变量污染后实测；§1.7 B10 表定义迁出后本包测试集合未变，`expect` 计数 +1 来自浏览器面负例新增的递归深度断言）。较 W2 切片收口的 97 pass / 8 文件增加 `web/__tests__/hindsight-failure.test.ts`、`web/__tests__/hindsight-failure-notice.test.tsx` 两个文件，以及 §1.3(6) 的失败分类与渲染契约用例。
