// 发布视图侧栏面板（ProdViewsPanel）列表滚动容器的高度约束守卫。
//
// 覆盖的用户报告：「ArtifactsPanel 切到 Views 模式后，视图一多就滚不动、也看不到滚动条」
// （同一成因已在 Chat 侧栏会话列表与定时任务面板上复现并修复）。
// 根因：视图列表的 ScrollArea 根是列向 flex 的子项（`flex-1`），此前缺 `min-h-0`，
...
// 修复前——可用高 876px、40 张卡片时 ScrollArea 根高 3648px，视口 clientHeight == scrollHeight == 3648，
// 滚轮与 `scrollTop` 赋值均无效、Radix 滑块不挂载；修复后——根 842.5px，视口 843:3648，
// 滚轮 0→300、程序化 0→2805、hover 后滑块挂载且可拖。
// 本文件守住的是让它成立的唯一前置条件 —— 该 flex 子项不得依赖 `min-height: auto`。
//
// 装配沿用同目录 `prod-view-list-states.test.tsx`（真实组件 + happy-dom + fetch 桩）：跨包依赖不 mock，
// 只换掉会把副作用或上下文依赖带进用例的三处 —— `react-i18next` / `sonner` / `@tanstack/react-router`
// （面板内的 `<Link>` 与编辑器需要路由上下文）。替身取**跨包并集**而非本用例用到的子集，理由见那些文件的说明。
...
const { ProdViewsPanel } = await import("../pages/agent-panel/ProdViewsPanel");
...
function stubFetch(): () => void {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL) => {
    ...
    const body = url.startsWith("/web/config/agents")
      ...
      : { success: true, data: createViews(itemCount) };
    return {
      ok: true,
      status: 200,
      statusText: "200",
...
async function renderPanel(): Promise<void> {
  await act(async () => {
    root.render(createElement(ProdViewsPanel, { agentId: "agent-1" }));
  });
  for (let i = 0; i < 5; i += 1) {
    await act(async () => {
...
describe("ProdViewsPanel 列表滚动容器", () => {
  ...
  test("视图条数少时同一条约束仍在（与列表长度无关）", async () => {
    itemCount = 2;
    await renderPanel();
