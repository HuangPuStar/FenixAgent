...
// 值导入图遍历器（守卫的**工具**侧，策略与断言在同目录的 prod-view-browser-surface.test.ts）。
//
// 与 sandbox 的同名工具是**逐字同源**的副本，不是共享模块：守卫属于各包自身，仓库根的
// `scripts/` 在包单独检出时不存在（见下「口径漂移」段），而把该工具放进某个资源包会让其余
// 包的浏览器守卫依赖那个包。两处口径漂移会被两侧测试同时暴露。
//
