 ...
 *
 * 组织隔离规则仍在本层：绑定的可见性与可写性由「绑定目标 Environment 属于调用者组织」决定
 * （列表过滤 + 写操作 403/404）。它是对外协议语义的一部分（越权时列表隐藏、写操作显式拒绝），
 * 与 `channel-binding` 的领域 CRUD 分开，便于用例逐条断言对外行为。
 */
export function createWebChannelsRoutes(deps: WebChannelRouteDependencies) {
  ...
  app.get(
    ...
    async ({ store, request: _request }: any) => {
      ...
      const teamEnvs = await environmentLookup.listByOrganizationId(authCtx.organizationId);
      ...
      for (const b of filtered) {
        const env = await environmentLookup.getById(b.agentId);
  ...
  app.delete(
    ...
    async ({ store, params, error, request: _request }: any) => {
      ...
      const env = await environmentLookup.getById(target.agentId);
      if (!env || env.organizationId !== authCtx.organizationId) {
  ...
  app.patch(
    ...
    async ({ store, params, body, error, request: _request }: any) => {
      ...
      const b = body as { platform?: string; chatId?: string | null; agentId?: string; enabled?: boolean };
      // 改指到别的环境必须与 POST 用同一口径校验归属。
      //
      // 上面校验的是**原绑定**的 agentId；请求体里的 `agentId` 是这次写入的**新**目标，它同样是不可信输入。
      // 只在原来那一侧校验，任何已认证用户都能把自己的绑定改写到其它组织的 Environment，写库已经发生，
      // 响应里还会把该组织的环境名回显给调用者（越权写入 + 组织信息泄露）。
      const nextAgentId = b.agentId;
      if (nextAgentId !== undefined && nextAgentId !== target.agentId) {
        const nextEnv = await environmentLookup.getById(nextAgentId);
        if (!nextEnv || nextEnv.organizationId !== authCtx.organizationId) {
          return error(404, { success: false, error: { code: "NOT_FOUND", message: "Agent 不存在" } });
        }
      }
      ...
      const updatedEnv = await environmentLookup.getById(updated.agentId);
