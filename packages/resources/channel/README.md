...
- **依赖映射**（`@/` 别名计数为 0）：`@fenix/web-runtime/api/request` 的 `request` / `unwrap`、`@fenix/ui-components/{ui,config,layout,components}/*` 的基础组件、`@fenix/agent-runtime/web/api/environments` 的 `envApi`；包内相对引用只到 `../../../api/channels`、`../../../lib/channel-list-state` 与 `../components/ChannelBindingForm`（表单字段体，见下条）。`lucide-react`（错误态图标）与 `sonner`（toast）已在浏览器守卫的白名单里。
