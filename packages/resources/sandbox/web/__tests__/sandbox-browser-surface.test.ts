...
describe("sandbox web 入口浏览器可达面", () => {
  ...
  test("遍历有效性自检：包内模块与跨包 exports 目标都在到达集合中", () => {
    ...
    expect(reachedWebFiles.size).toBeGreaterThanOrEqual(20);
  ...
  // admin-key 迁出的回归点：包内不得复活第二份实现，三个导出名必须从新 owner 转出（导出面不缩水）。
  test("admin-key 无包内副本，三个导出名由 @fenix/web-runtime 转出", () => {
    expect(existsSync(join(WEB_ROOT, "src/lib/admin-key.ts"))).toBe(false);
    const source = stripComments(readFileSync(WEB_ENTRY, "utf8"));
    expect(source).toContain('from "@fenix/web-runtime/lib/admin-key"');
