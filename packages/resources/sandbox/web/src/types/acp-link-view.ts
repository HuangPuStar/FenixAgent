...
//
// 为什么不直接引用真实类型：类型定义在 `packages/resources/observer/web/api/observer.ts`，而
// observer 的 `./web` 出口尚未登记，经对方 `src/**` 深链被架构门禁禁止；且 observer → sandbox
// 已是既有依赖方向，反向 import 会形成包级环。
//
...
// 修改任一形状时必须同步改两处（收敛条件：observer 补 `./web` 出口后删除本文件并改指公开类型）。
