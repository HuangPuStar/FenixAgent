...
import { mapSandboxClusterAdminError } from "../../error-mapping";
import {
...
import { type SandboxServerAdminService, sandboxServerAdminService } from "../../services/sandbox-server-admin-service";
...
export function createApiSandboxServerRoutes(deps: SystemApiSandboxRouteDependencies) {
  const app = new Elysia({ name: "api-sandbox-server", prefix: "/api/system/sandbox-server" })
    ...
    .get(
      ...
      async ({ params, query }) => {
        try {
          return (await service.listSandboxes(params.serverId, query)) as z.infer<
            typeof RemoteSandboxListResponseSchema
          >;
        } catch (error) {
