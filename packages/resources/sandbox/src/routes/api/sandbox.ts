...
} from "../../server/schemas/api-sandbox.schema";
import * as sandboxApi from "../../server/services/sandbox-admin-service";
import { SandboxProviderNotConfiguredError, SandboxRuntimeNotReadyError } from "../../server/services/sandbox-errors";
import type { SystemApiSandboxRouteDependencies } from "../dependencies";
...
export function createApiSandboxRoutes(deps: SystemApiSandboxRouteDependencies) {
  const app = new Elysia({ name: "api-sandbox", prefix: "/api/system" }).use(deps.systemApiGuardPlugin).model({
