...
- **浏览器出口**：`web/index.ts` 是 `exports["./web"]` 的目标，包与宿主消费本包浏览器能力的根入口（`./web/i18n` 是只导出语言资源的独立子路径，供宿主 i18n 启动期注册：从根入口导入会把整张沙盒页面图拉进首屏 bundle）。实测 22 个值导出 + 类型导出，覆盖包外 8 个消费文件（口径 `grep -rln '"@fenix/resource-sandbox/web"' --include='*.ts' --include='*.tsx' packages apps` 去掉本包与 `__tests__` 后为 8 个，含 `observer/web/__tests__/admin-observer-utils.test.ts` 则 9 个；资源包侧 7 个见下，另有宿主 `apps/web/src/routes/admin/sandbox.tsx`）：
  - `observer`（5 个文件，另有 1 个测试文件）取 `FlatRow` / `IntegrityRow` / `YjsSessionGroup` 类型与 `mergeFlatRows` / `machineReverseIndex` / `integrityRows` / `name` / `groupYjsSessions` / `sessionTabCounts` / `chatRelayPayload` / `formatClockTime` / `formatDuration`（`AdminObserverPage` 与 4 个展示组件；`AdminLogsPage` / `AdminPeoplePage` 此前只取 `MasterKeyGate`，门下沉后不再消费本包）；
  - `model-management/web/pages/admin/AdminModelGatewayPage.tsx` 取 `SearchableUsageFilter`；
  - `agent-config/web/pages/agent-panel/agent-editor/use-agent-editor.ts` 取 `sandboxPoolApi`。
  宿主侧消费 `apps/web/src/routes/admin/sandbox.tsx`（懒加载 `AdminSandboxPage`）与 `apps/web/src/i18n/index.ts`（经 `./web/i18n` 子路径，不经过 `./web` 根入口，故不计入上述 8 个）。
- **Master Key 门不再由本包出口（2026-09-22 前端去重）**：`MasterKeyGate` 原先住在本包（`web/src/pages/admin/components/MasterKeyGate.tsx`）却被 observer 与 model-management 跨包引用——一道管理密钥门迫使别的资源包依赖整个沙盒资源包。按「归属由消费者集合决定」拆成两件共享基建后原文删除：纯展示门是 `@fenix/ui-components/config/AdminKeyGate`（受控 props、不依赖 web-runtime），门的状态机是 `@fenix/web-runtime/hooks/use-admin-key-gate`（读/写 key 与 401 回门）。本包 `AdminSandboxPage` 同时改指这两处，**不保留 re-export shim**；`web/__tests__/sandbox-browser-surface.test.ts` 的到达清单与 `src/__tests__/sandbox-source-migration.test.ts` 的迁移映射同批收敛。`@fenix/web-runtime/lib/admin-key` 的三个导出仍由本包转出（既有断言「admin-key 无包内副本」不变），因为它们是沙盒 API client 的鉴权口径而不是门组件的一部分。
