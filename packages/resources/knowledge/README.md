...
- **左栏目录收敛到共享构件集（2026-09-23）**：`agent-knowledge-directory.tsx` 的目录栏改用 `@fenix/ui-components/components/agent-catalog-index` 的 `AgentCatalogIndex`（目录栏容器 + 头部）/ `AgentCatalogIndexNav` / `AgentCatalogIndexItem` / `AgentCatalogIndexIcon` / `AgentCatalogIndexCopy` / `AgentCatalogIndexArrow`。它此前是四处同构拷贝里形态最特殊的一份——行尾删除按钮渲染在条目 `<button>` **之外**（按钮里套按钮是非法 HTML，浏览器会把内层按钮甩出外层，点击区与焦点顺序都会错），现由条目的 `trailing` 外壳槽位承担，删除按钮仍是外壳的兄弟节点；`aria-current="page"` / `aria-disabled` / `onSelect` / `onDelete` 与删除按钮的 `aria-label` 逐字未变。**目录级三态仍画在目录栏内部**（`role="status"` + `aria-busy` + `aria-label={t("directory.loading")}` 的加载块、`KnowledgeLoadFailure`、`EmptyState`）：共享组件的 `children` 是自由内容，本页不必学技能库 / MCP / 模型库改成页面级提前返回。
- **目录栏取值全部归还共享组件（2026-09-23，同日稍后）**：五页统一裁定落地后，本页目录栏只剩两条自有样式——`.knowledge-directory-item.is-unavailable { opacity: .58 }`（本页状态语义）与副标题行的 `display:flex + gap:7px`（状态圆点与资源数并排）。此前的 `18px 10px` 内边距、`0 8px 12px` 头部内边距、24×22 徽标、4px 行距、64px 行高、9px 内边距、34×34 图标盒与 13px/11px 字号**全部删除**，改为共享构件集的 rem 默认值（13/16 根字号下：目录栏内边距 18.6875/9.75、行高 56.875、图标盒 27.625、条目标题 11.375px、副标题 9.75px）。同批的**结构变化**：`columns="icon-copy"`（两列）改为默认四列模板，尾注列留空、新增 `AgentCatalogIndexArrow`——选中行的行尾从此与其余四页一致；选中底色与文字色不再由 `.is-active` 类驱动，改由共享 CSS 按 `aria-current="page"` 驱动。`≤760px 隐藏目录栏` 是布局行为，未动。
