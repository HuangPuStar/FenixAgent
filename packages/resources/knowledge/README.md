...
本包属 `resources` 类别。`dependsOn: []` 来自实测：`src/**` 对外只有 `@fenix/platform-sdk`（根入口的响应信封 schema `ApiErrorResponseSchema` / `WebErrSchema` / `WebOkSchema`，`/server` 子路径的 `getDatabase` / `getModuleConfig` / `overrideModuleConfig`，`/testing` 子路径的 `stubDb` / `getDbStub` / `readJson` / `resetAllStubs` / `initializeTestApplicationInfrastructure`，后者只出现在 `src/server/testing.ts` 与用例）。它属 `platform` 基建，不产生 `resource` 装配边。

`db/schema.ts`（§1.7 B9 新增）是唯一的例外面：它按外键目标导入 `@fenix/agent-config/db`（`agent_knowledge_binding.agent_config_id`，1 次）与 `@fenix/identity/db`（`user`，1 次）的**列对象**，两者已写入 `package.json` 的 `dependencies`。它们**不**进 `dependsOn`：装配校验（`scripts/generate-module-registry.ts` 的 `assertDependsOnComplete`）只扫 `src/**`，表定义声明的是「列对象来自谁的迁移链」，不是服务端运行期耦合——`@fenix/identity` 还是基础模块（profile 固定槽位），本就不参与资源装配拓扑。（同一口径见 `@fenix/agent-config` 的 manifest：它的 `dependsOn` 也未列 `machine` / `model-management`，尽管其 `db/schema.ts` 取了两者的列对象。）

- `package.json` 的 `exports`（每个目标文件均已核对存在）：`.` → `src/index.ts`、`./db` → `db/schema.ts`（B9 新增，`drizzle.config.ts` 按此声明）、`./module` → `fenix.module.ts`、`./server` → `src/server.ts`、`./server/schema` → `src/server/schemas/knowledge.schema.ts`、`./server/summaries` → `src/server/knowledge-summaries.ts`、`./server/testing` → `src/server/testing.ts`、`./web` → `web/index.ts`、`./web/i18n` → `web/i18n/index.ts`。
- **`./server/summaries` 是 B9 新增的只读投影出口**：`findKnowledgeBaseSummariesByIds({ organizationId, knowledgeBaseIds })` 返回 `Map<id, { name, slug }>`，供 `@fenix/resource-agent-config` 的 `agent-related-resources.ts` 把绑定 ID 渲染成标签——此前它直读宿主 `@server/db/schema` 的 `knowledge_base`，B9 表迁出后改走这条出口（本包 `dependsOn` 无 agent-config，边的方向由对方声明）。与同族的 mcp / skill 投影（`./server/config`）有两处刻意差异：多一个 `organizationId` 条件（`knowledge_base` 没有 `visibility` 列，归属就是 `organization_id`；配错组织退化为用 ID 当标签，而不是泄露他组织名称），多返回 `slug`。出口名与它们不同名是因为本包已有 `src/server/config.ts`（模块配置，未导出），键名是公开契约、事后改名要动消费方。
...
- **DB**：`src/server/db.ts` 的 `getKnowledgeDatabase()` 返回 `getDatabase<KnowledgeDatabase>()`（`@fenix/platform-sdk/server` 的基础设施，未初始化即抛错）；句柄类型仍刻意不写 `typeof schema`（与 agent-runtime / agent-config 同口径），表对象自 §1.7 B9 起从本包 `@fenix/resource-knowledge/db` 取得，因此 `@server/*` 的残留已归零。
...
- **表定义已迁入本包（§1.7 B9，2026-09-22，此项已关闭）**：`knowledgeBase` / `knowledgeResource` /
  ...
  `bun run check:schema-ddl-drift` 零差异），经出口 `@fenix/resource-knowledge/db` 汇入
  `drizzle.config.ts` 声明的同一条迁移链。宿主侧的引用面本就为零，迁出后宿主文件连 `import` 都不留。
...
- **左栏目录收敛到共享构件集（2026-09-23）**：`agent-knowledge-directory.tsx` 的目录栏改用 `@fenix/ui-components/components/agent-catalog-index` 的 `AgentCatalogIndex`（目录栏容器 + 头部）/ `AgentCatalogIndexNav` / `AgentCatalogIndexItem` / `AgentCatalogIndexIcon` / `AgentCatalogIndexCopy`（本页没有尾注列与箭头，故 `AgentCatalogIndexMeta` / `AgentCatalogIndexArrow` 未用；`columns="icon-copy"` 即原来的两列 `34px minmax(0, 1fr)`）。它此前是四处同构拷贝里形态最特殊的一份——行尾删除按钮渲染在条目 `<button>` **之外**（按钮里套按钮是非法 HTML，浏览器会把内层按钮甩出外层，点击区与焦点顺序都会错），现由条目的 `trailing` 外壳槽位承担，删除按钮仍是外壳的兄弟节点；`aria-current="page"` / `aria-disabled` / `onSelect` / `onDelete` 与删除按钮的 `aria-label` 逐字未变。`agent-knowledge.css` 随之删除 8 条已下沉的骨架规则（头部 `flex` + 两端对齐 + 13px、徽标 `display:grid` + 居中 + 底色、`nav` 的 `display:grid`、条目两列模板、按钮网格与 `34px minmax(0, 1fr)` 模板、图标槽 `display:grid` + 居中、文案槽 `min-width: 0`、`strong` 的两行截断三条），页面只留字号 / 内边距 / 行高 / 圆角 / 配色 / 行间距这些各页刻度。**头部保留 2 条高特异性覆盖**（共享 CSS 未分层、压过 `@layer utilities`，工具类改不动它写过的属性）：`.knowledge-directory .agent-catalog-index-header { padding: 0 8px 12px }` 与 `.knowledge-directory .agent-catalog-index-header > div > span { min-width: 24px; height: 22px; border-radius: 6px; color: #64748b }`——前两处之差来自共享默认的 22×20 徽标 / 5px 圆角 / 14px 下内边距，`#64748b` 是本页原色（共享默认 `#65728a`，肉眼不可分，要彻底归一只需删这一行）。**目录级三态仍画在目录栏内部**（`role="status"` + `aria-busy` + `aria-label={t("directory.loading")}` 的加载块、`KnowledgeLoadFailure`、`EmptyState`）：共享组件的 `children` 是自由内容，本页不必学技能库 / MCP / 模型库改成页面级提前返回。**两处口径差异**：①没有删除按钮的行同样显式传 `trailing={null}`——共享组件只在 `trailing === undefined` 时不渲染外壳，而外壳正是本页悬停 / 选中底色的落点，缺了它会静默丢掉整行外观；②副标题 `small` 从共享组件继承截断三条（`overflow` / `text-overflow` / `white-space`），超长文案由溢出改为裁切（视觉行为变化，资源数文案极短，实际不触发）。
...
- **宿主 i18n 注册表在本轮切到包出口**：切换过程中宿主一度按 `packages/resources/{channel,observer,prod-view,task,workflow}/web/i18n/{en,zh}/*.json` 的旧布局导入（字典已迁到 `locales/`），任何加载宿主 i18n 注册表的用例都会在加载期抛 `Cannot find module`（本包 `web/src/__tests__/context-panel-ssr.test.tsx` 与 `model-management` 的 web 用例均被挡；该文件已于 CE 阶段 2 §1.6 T5d 删除——它断言的 `ContextPanel` 宿主从不渲染）；宿主已改为经 `@fenix/<pkg>/web/i18n` 登记（本包见 `apps/web/src/i18n/index.ts:15`），本包用例随之恢复全绿（412 pass / 0 fail）。

## 前端状态补齐与死副本清理（2026-09-20，§1.3(6) 缺口修复）

口径来源：`docs/design/ce-ee-refactoring/ce-ee-engineering-standards.md` §10.5.3（资源 Web 流程覆盖 loading、empty、error、retry、无权限、成功反馈和可访问性）。本轮只动 `web/**`，未改服务端与 schema。
