...
- `@fenix/identity` 是 web 侧新增依赖（`AgentKnowledgeBasesPage.tsx:1` 的 `useOrg` / `useSession`，§6.5 裁定组织上下文必须取宿主同一份）。`resource → platform-impl` 在 §2.3 矩阵里是 `special-dependency`，已登记：`scripts/architecture/exceptions.json` 的 `@fenix/resource-knowledge → @fenix/identity`（`owner: 1.6`，rationale 与实测的 1 处 / 1 文件一致）。
...
- **web 面宿主 alias 待撤（§1.6）**：`apps/web/vite.config.ts` 仍为知识库 web 文件保留 9 条 alias（实测行号 62 / 71 / 75 / 79 / 83 / 84 / 96 / 100 / 104）。其中 `apps/web/src/pages/agent-panel/components/KnowledgeGraphPanel.tsx` 已被包内 `web/pages/agent-panel/KnowledgeGraphPanel.tsx` 取代，成为死代码（实测全仓无 import、无 alias 指向它，仅自身文件内出现；其 alias 已不在配置里），随宿主切换一并删除。消费方（`agent-config`、`model-management`）改用 `@fenix/resource-knowledge/web` 后，本包 web 出口是唯一公开面。
