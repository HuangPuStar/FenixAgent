...
import { mergeKnowledgeBaseMetadata } from "../../services/knowledge-metadata";
...
/** Gotenberg 服务地址（Docker: docker run -d -p 3000:3000 gotenberg/gotenberg:8）取自模块配置。 */
...
/**
 * `/web/knowledgeBases*` 控制台路由工厂（知识库 CRUD、资源上传/导入/删除、切片、检索、图谱、embedding 模型管理）。
 ...
 *
 * 端点一律声明 `sessionAuth: true` 并在处理函数开头取 `store.authContext`：组织隔离由每条查询的
 * `organizationId` 谓词落地（`store.authContext` 是唯一来源，不接受请求体/查询串里的组织标识）。
 */
