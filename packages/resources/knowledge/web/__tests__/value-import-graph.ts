...
// 值导入图遍历器（守卫的**工具**侧，策略与断言在同目录的 knowledge-browser-surface.test.ts）。
//
// 来源：逐字取自 `packages/resources/sandbox/web/__tests__/value-import-graph.ts`（黄金样本的
// T2b 守卫工具，计划 §1 与 review §6.3 要求各包按同一口径守护浏览器面）。差异只有这一行出处说明——
