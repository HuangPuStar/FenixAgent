...
let mockKnowledgeBindings: Array<{
...
mock.module("../repositories", () => ({
    ...
    sessionWorkerRepo: {
        ...
        upsert: mock(async (_sessionId: string, patch: any) => ({
...
mock.module("../utils/executable", () => ({
    resolveExecutable: mock(
        () => process.cwd() + "/node_modules/.bin/acp-link",
    ),
}));
...
describe("InstanceService", () => {
    ...
    test("spawnInstance falls back to system PATH when local acp-link is missing", async () => {
        const tempDir = mkdtempSync(
            join(tmpdir(), "instance-service-no-acp-link-"),
        );
        const cwdBeforeSwitch = process.cwd();
        try {
            mkdirSync(join(tempDir, "node_modules", ".bin"), {
                recursive: true,
            });
    ...
    test("spawnInstance uses project-local acp-link binary", async () => {
        const tempDir = mkdtempSync(
            join(tmpdir(), "instance-service-local-acp-link-"),
        );
        const cwdBeforeSwitch = process.cwd();
        try {
            ...
            const lastCall = mockSpawn.mock.calls.at(-1) as
                | [string, ...unknown[]]
                | undefined;
...
describe("InstanceService multi-instance", () => {
    ...
    test("multiple instances can be created for the same environment", async () => {
        const inst1 = await spawnInstanceFromEnvironment(
            "test-user",
            "env_test_1",
        );
        const inst2 = await spawnInstanceFromEnvironment(
            "test-user",
            "env_test_1",
        );
    ...
    test("instance numbers are strictly increasing", async () => {
        ...
        const inst3 = await spawnInstanceFromEnvironment(
            "test-user",
            "env_test_num",
        );
    ...
    test("listInstancesByEnvironment returns only active instances", async () => {
        ...
        expect(
            active.every((i) => i.status !== "stopped" && i.status !== "error"),
        ).toBe(true);
    });
    ...
    test("default agent with knowledge bindings injects kb MCP config", async () => {
        mockKnowledgeBindings = [
            { knowledgeBaseId: "kb_1", priority: 0, enabled: true },
        ];
