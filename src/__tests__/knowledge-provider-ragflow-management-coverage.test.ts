...
describe("RagFlow 供应商管理真实分支", () => {
  ...
  test("listConfiguredProviders 过滤无效厂商并保留有效名称", async () => {
    installFetch(
      mockFetch(async () => response({ code: 0, data: [{ name: "OpenAI" }, {}, { name: "" }, { name: "RagFlow" }] })),
    );
  ...
  test("listConfiguredProviders 在上游失败时降级为空数组", async () => {
    installFetch(mockFetch(async () => response({ code: 500, message: "unavailable" })));
  ...
  test("listProviderInstances 在请求失败时降级为空数组", async () => {
    installFetch(mockFetch(async () => response({ code: 403, message: "forbidden" })));
  ...
  test("listInstanceModels 在请求失败时降级为空数组", async () => {
    installFetch(mockFetch(async () => response({ code: 500, message: "broken" })));
  ...
  test("addProviderInstance 发送修剪后的 baseUrl 与覆盖凭据", async () => {
    const fetchSpy = mockFetch(async () => response({ code: 0 }));
    installFetch(fetchSpy);
  ...
  test("listConfiguredEmbeddingModels 回退旧端点并映射 legacy 模型", async () => {
    const fetchSpy = mockFetch(async (url) => {
      if (String(url).endsWith("/api/v1/models")) return response({ code: 500, message: "unsupported" });
      return response({
        code: 0,
  ...
  test("listConfiguredEmbeddingModels 双端点失败时降级为空数组", async () => {
    installFetch(mockFetch(async () => response({ code: 500, message: "down" })));
  ...
  test("getKnowledgeGraph 在缺少 graph 字段时返回 null", async () => {
    installFetch(mockFetch(async () => response({ code: 0, data: { mind_map: { root: "x" } } })));
  ...
  test("pollKnowledgeGraphProgress 为缺失数据提供零进度", async () => {
    installFetch(mockFetch(async () => response({ code: 0, data: {} })));
