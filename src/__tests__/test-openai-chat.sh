...
echo "  URL:  $BASE_URL/v1/agents/:agentId/chat/completions"
echo "============================================"
echo ""
...
RESP=$(curl -s -w "\n%{http_code}" \
  -X POST "$BASE_URL/v1/agents/$AGENT_ID/chat/completions" \
...
ERR_404=$(curl -s -w "\n%{http_code}" \
  -X POST "$BASE_URL/v1/agents/nonexistent-agent-999/chat/completions" \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"messages":[{"role":"user","content":"hi"}]}' 2>&1)
