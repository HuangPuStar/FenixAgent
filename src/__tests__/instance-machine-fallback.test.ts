...
describe("instance machine/engine fallback", () => {
  ...
  // local 执行时传递 engineType，使用 RCS_DEFAULT_ENGINE_TYPE 或默认 Peri。
  test("local 执行时传递 engineType，优先使用 RCS_DEFAULT_ENGINE_TYPE", () => {
    const resolvedNodeId: string = "local-default";
    const engineType = resolvedNodeId === "local-default" ? (config.defaultEngineType ?? "peri") : undefined;
    expect(engineType).toBe("peri");
  });
