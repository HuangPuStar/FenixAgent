...
export interface SharedRelay {
  ...
  /**
   * 回放窗口标记：会话同步请求转发后置位、对应响应消费后复位。窗口内来自 agent 的
   * user_message_chunk 属于 session/load 历史回放（数据重建，不是新 turn），聚合层
   * 必须抑制其 loading 设置——否则切换会话后 loading 残留、刷新恢复时覆盖真实进行
   * 中的 loading。用户消息（服务端 writePromptText 写入）不走此标记，不受抑制。
   */
