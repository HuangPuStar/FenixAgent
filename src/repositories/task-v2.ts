...
// ── ScheduledTaskV2 仓储 ──
// 权限模型：用户 + 组织双重隔离，所有查询按 userId AND organizationId 过滤

export interface IScheduledTaskV2Repo {
  listByUserAndOrg(userId: string, organizationId: string): Promise<ScheduledTaskV2Row[]>;
  listByUserAndOrgPaged(
    userId: string,
  ...
  getByUserAndOrgAndId(userId: string, organizationId: string, taskId: string): Promise<ScheduledTaskV2Row | null>;
  ...
  deleteByUserAndOrgAndId(userId: string, organizationId: string, taskId: string): Promise<boolean>;
...
class PgScheduledTaskV2Repo implements IScheduledTaskV2Repo {
  async listByUserAndOrg(userId: string, organizationId: string) {
    return db
      ...
      .where(and(eq(scheduledTaskV2.userId, userId), eq(scheduledTaskV2.organizationId, organizationId)))
      .orderBy(desc(scheduledTaskV2.createdAt));
  }

  async listByUserAndOrgPaged(
    userId: string,
    organizationId: string,
    page: number,
    pageSize: number,
  ...
  async getByUserAndOrgAndId(userId: string, organizationId: string, taskId: string) {
    const rows = await db
      ...
      .where(
        and(
          eq(scheduledTaskV2.id, taskId),
          eq(scheduledTaskV2.userId, userId),
          eq(scheduledTaskV2.organizationId, organizationId),
        ),
      )
      .limit(1);
    return rows[0] ?? null;
