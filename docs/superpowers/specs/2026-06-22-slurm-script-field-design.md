...
workflow-examples/pe-rna-seq-single-sample.yaml        # 8 节点迁移到新写法
```
...
11 步流程对应 8 个 yaml 节点（有的 Galaxy 步骤合并为一个节点，如 FastQC 单样本 R1+R2 跑一次）。8 个节点全部机械重构。以 `trim_galore` 为例：
