...
**现状**：Environment 的 `agentName` 字段通过字符串匹配 AgentConfig 的 `name`。AgentConfig 改名会导致 Environment 找不到配置。

**目标**：Environment 通过 AgentConfig 的 `id`（UUID）强绑定，而不是 `name` 字符串。

**影响**：
- `db/schema.ts`：environment 表新增 `agentConfigId` 列（UUID，references agentConfig.id），`agentName` 列保留过渡期后移除
- `services/instance.ts`：`spawnInstanceFromEnvironment()` 从 `env.agentConfigId` 获取 AgentConfig，不再靠 name 匹配
- `routes/web/environments.ts`：创建/更新 Environment 时接受 `agentConfigId` 而非 `agentName`
- `services/config-pg.ts`：AgentConfig 改名不再影响 Environment
- 前端：Environment 表单中选择 AgentConfig 的下拉改为 ID 选择器
- 领域图：实线强关联替代虚线松耦合
...
**现状**：Session 是 RCS 的一等公民，有独立的 DB 表（agent_session）、仓储（SessionRepo）、路由（/web/sessions）、Service 层（session.ts）。RCS 存储 Session 元数据（标题、状态、创建时间等），管理 Session 生命周期。
...
**文件系统**：当前按 Session 组织的文件系统（`/web/sessions/:id/user/*`）需改为按 Environment 维度组织
...
- autoStart：服务器启动 → `index.ts` 遍历 autoStart 环境 → `spawnInstanceFromEnvironment()`
- IMChannel：消息到达 → HermesClient 直接调用 `sendToInstanceLocalWs()` 或 `sendToAgentWs()`，不触发 spawn

**目标**：Environment 作为资源管理层，统一管理 Instance 的 spawn 决策。所有触发者不直接 spawn，而是向 Environment 发出"需要运行"的请求，Environment 根据策略决定：
...
**影响**：
- `services/instance.ts`：spawn 决策逻辑上移到 Environment 层，或 instance 服务增加 `ensureRunning(environmentId)` 接口
- `services/hermes-client.ts`：消息路由时不再自己找 Instance，改为请求 Environment 确保 Instance 运行
- `routes/web/environments.ts`：`POST /environments/:id/enter` 成为统一的"启动并连接"入口
...
- `services/instance.ts`：spawn 逻辑简化，只需把 AgentConfig 的完整配置（包括 MCP 和 Skill 引用）写入 workspace，不需要 Environment 特殊处理 KB
...
**现状**：ScheduledTask 是一个复杂的领域概念，绑定 Environment，包含任务描述文本（task 字段），执行时需要找 Instance 或 spawn 临时进程。内部有 `AgentTaskRunner` 负责构造 prompt 并发送给 Agent。
...
**影响**：
- `db/schema.ts`：`scheduled_task` 表简化，移除 `environmentId`（外键）、`task`（任务描述）、`timeoutMinutes` 等字段，改为 `url`（目标 URL）、`method`（HTTP 方法）、`headers`（请求头）、`body`（请求体）
- `services/task.ts`：大幅简化，执行逻辑变为 HTTP 请求调用
- `services/agent-task-runner.ts`：移除（不再需要）
- `services/scheduler.ts`：简化为触发 HTTP 请求
- `routes/web/tasks.ts`：CRUD 接口简化
- 领域图：Task 从领域模型中大幅简化，不再关联 Environment 或 AgentConfig
...
**当前状态**：代码中已有 `routes/web/workflow-proxy.ts` 反向代理到 acpx-g 引擎，核心 Workflow 领域模型待设计实现。
...
**影响**：
- `db/schema.ts`：可能需要新建 `im_channel` 表，或重构 `channel_binding` 表
- `routes/web/channels.ts`：升级为 IMChannel 的完整 CRUD + 连接管理
- `services/channel-binding.ts`：逻辑融入 IMChannel 服务
- `services/hermes-client.ts`：成为 IMChannel 的底层传输实现，用户不直接接触
...
**现状**：Provider、Model、AgentConfig、Skill、McpServer 等配置由 RCS 管理在 PostgreSQL 中，Instance spawn 时部分配置会写入 workspace 的 `.opencode/opencode.json`，但注入不完整（只有 default_agent 和 KB MCP 端点）。
...
**影响**：
- `services/instance.ts`：`spawnInstanceFromEnvironment()` 的注入逻辑需要扩展，覆盖所有配置维度
- `services/config-pg.ts`：可能需要新增批量读取接口（一次性获取 AgentConfig + 关联的 Skill + MCP + KB）
- workspace 的 `.opencode/opencode.json` 变为 RCS 自动生成的只读文件
...
**现状**：Model 作为独立领域概念存在（有独立的 DB 表 `model`，独立的节点在领域图中）。Skill、McpServer、Provider 虽然是独立资源，但在领域图中被画在 AgentConfig 的 subgraph 内，视觉上像是 AgentConfig 的"组成部分"。
...
**影响**：
- `db/schema.ts`：`model` 表保留（数据层面不变），但上层领域概念中 Model 不再独立
...
- `config-pg.ts`：Model 的 CRUD 逻辑保持不变（Provider 的子资源管理）
...
**现状**：User 是所有资源的所有者，每条记录带 `userId`，查询 `WHERE user_id = ?`，用户之间完全隔离。
...
- `db/schema.ts`：所有资源表 `userId` 列改为 `teamId`（或新增 `teamId`，过渡期兼容）
- `services/config-pg.ts`：查询条件从 `WHERE user_id = ?` 改为 `WHERE team_id = ?`
- 认证层：session 存 `activeTeamId`，支持切换团队
- 路由层：handler 从 `store.user.id` 改为 `store.teamId`
- 前端：团队管理 UI、团队切换、资源列表按团队过滤
- 参考详细设计：`docs/arch/15-team-permissions-design.md`
