...
| 实例管理 | [05-instance.md](./05-instance.md) | 通过 CoreRuntimeFacade 管理 acp-link 实例的 spawn、stop、生命周期（由 Environment 资源管理层调度） |
...
| 文件 | [12-files.md](./12-files.md) | Environment 工作区文件系统 |
| 插件层 | [13-plugins.md](./13-plugins.md) | Elysia 插件：CORS、日志、错误处理、静态文件 |
| 用户与权限 | [14-auth-and-permissions.md](./14-auth-and-permissions.md) | 认证链路、多租户组织隔离、权限能力 |
| 团队权限方案 | [15-team-permissions-design.md](./15-team-permissions-design.md) | 从用户隔离→团队共享+管理员全局可见的设计（部分已通过 organization 插件实施） |
| 领域模型关联图 | [16-domain-model.md](./16-domain-model.md) | Agent/Environment/Session/Channel/Knowledge 等概念的关联关系 |
| 工作流引擎 | [17-workflow.md](./17-workflow.md) | DAG 工作流定义、执行引擎、版本管理、Board/Job/Trigger |
