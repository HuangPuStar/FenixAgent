...
- **exports 用显式子路径，禁止通配符**：全仓 `package.json` 零 `"./x/*"` 通配出口，跨包一律显式列出
  ...
  > **执行补记（2026-09-19，阶段 1 落地两个新包时）**：本条规则在本次交付的
  > `packages/ui-components/package.json` 里**并未落实**——它带的是 7 条通配（`./ui/*`、`./config/*`、
  > `./chat/*`、`./components/*`、`./layout/*`、`./lib/*`、`./i18n/locales/*`）。落地时已按本条展开为
  > **78 条显式子路径**（保留 `"."` 与 `"./styles.css"`，全部带真实扩展名），与文档原描述不同。
  > 展开清单取全仓 `@fenix/ui-components/*` 引用的并集，**并且必须覆盖包内自引用**：`demo/**` 与
  > `web/**` 用包名而非相对路径引用自己，而 Node 的 self-reference 解析要求 `exports` 精确完整，
  > 漏一条包自身就解析失败。`packages/web-runtime/package.json` 的 11 条出口本就是显式的，无需改造。
