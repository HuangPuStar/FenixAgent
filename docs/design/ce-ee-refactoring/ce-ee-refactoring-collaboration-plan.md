...
**范围：** 本计划不一次性迁移所有历史功能，但不会错误地将 AgentConfig 与其强依赖资源拆开。Environment、Instance、Runtime、relay、ACP session、Chat 与 YJS 在 AGT-01 中先作为高耦合整体迁入单一 `@fenix/agent-runtime` package；Skill、MCP、模型/Provider、知识库、记忆、Machine/Sandbox、站点应用的 **AgentConfig 所需能力** 属于首个闭环；工作流、调度等其余能力在首个闭环验收后按本文的任务模板逐个迁移。
...
| `ORC` | Orchestration & Collaboration | 基于 `@fenix/agent-runtime` 既有 Chat/YJS 边界的 Workflow、Scheduler、Webhook、Channel 等协作与编排能力 |
...
| 5. 执行与连接 | EXE-01 | AgentConfig 闭环外的 Machine、workspace/file、Sandbox 与引擎适配能力迁移；复用 `@fenix/agent-runtime` relay/runtime port |
| 6. 自动化与协作 | ORC-01 | 基于已迁移 Chat/YJS 边界拆分并迁移 Workflow、Scheduler、Webhook、Channel |
...
- [ ] 确认 AgentConfig、单一 `@fenix/agent-runtime` package 和强依赖资源的公开 Service/DTO、module ID、`/app/agent-configs` 路由与 Web contribution ID；Agent Runtime package ID 不再作为待选项。
- [ ] 确认模型/Provider、Skill、MCP、知识库、记忆、节点/Sandbox、Site App 的首期迁移范围与依赖顺序，并确认 Environment 只作为 `@fenix/agent-runtime` 包内运行上下文；新包不得调用旧 `src/services/**`。
...
### AGT-01：抽取单一无权限 Agent Runtime package

**负责人：** CE 任务池  
**前置：** ARC-03、FND-01、AGT-00
**主要文件：** `packages/agent-runtime/`、现有 Environment/Instance/Runtime、relay/ACP session、Chat/YJS 实现，以及 `packages/orchestration/` 中的实例职责。

- [ ] 将 Environment、Instance、Runtime、relay、ACP session、Chat 与 YJS 作为高耦合整体迁入单一 `@fenix/agent-runtime` package；不为分包改写在线控制流、状态机、协议或释放顺序。
- [ ] 通过 facade/port 注入 AgentConfig、LaunchSpec、认证和 transport 宿主能力；package 不读取 cookie、API Key、成员关系、`AuthContext` 或资源权限。
- [ ] 保留实例创建/复用/停止、Coordinator singleflight 与 generation fencing、relay/session 隔离、Chat/YJS 恢复和背压等 AGT-00 特征行为。
- [ ] 在 ARC-03 冻结后定义 package 内部目录、公开 exports、通用 launch spec 与启动端口；这些内部职责不得再次按 runtime 或 instance 拆成独立 workspace package。
...
**验收：** workspace 中只有一个 `@fenix/agent-runtime` package；AGT-00 特征测试保持通过；可仅凭通用 launch spec 启动/停止实例；package 中搜索不到成员、角色、组织语义解释或资源权限查询。
...
**验收：** `@fenix/agent-runtime` 的实例启动边界只接收完整 LaunchSpec；package 不直接查询任意资源表或读取资源权限。
...
- [ ] 为模型/Provider、Skill、MCP、知识库、节点/Sandbox、Site App 提供当前 AgentConfig 用户流程所需的列表、创建/编辑或选择页面和 `/app` client；不创建 Environment 管理页面或公开 client。
...
- [ ] 基于 ARC-01 的盘点，将非 AgentConfig 使用的 Machine、workspace/file、Sandbox 与引擎 adapter 拆为边界清晰的后续子 task；每个子 task 只迁移一个可验证能力簇。
- [ ] Machine `/acp/ws` 等宿主 route/transport 通过公开 port 接入 AGT-01 已迁移的 runtime/control/relay；不得在资源或 adapter 层复制 relay、session 或 Instance 生命周期。
...
**所属阶段：** 6. 自动化与协作
**前置：** EXE-01 相关依赖完成
**主要文件：** `@fenix/agent-runtime` 的 Chat/YJS 公开边界、Workflow、Scheduler、Webhook、Channel 对应模块与专项测试。

- [ ] 为 Workflow、Scheduler、Webhook、Channel 分别建立迁移 task，明确其资源、执行器/触发器、与既有 Chat/YJS 边界的协作方式及数据迁移边界；不重新提取或复制 Chat/YJS 内核。
...
EE 初版的差异只限于用户体系、认证和授权。它复用 CE 的 AgentConfig、模型/Provider、Skill、MCP、知识库、单一 `@fenix/agent-runtime`、资源 Web contribution 和业务页面；不创建 EE AgentConfig 发布模块、不复制 CE CRUD、不增加 EE 专属资源表，也不对外暴露包内 Environment。
...
- [ ] 创建 EE assembly profile，选择 EE access-control 与 CE 的 Agent Runtime、全部资源和 Web contribution；初版不选择 EE 资源扩展模块。
...
首期能力簇以外的每个资源都创建一组 `RSC-<resource>-01` task，不允许用“批量迁移所有资源”的大 ticket。Skill、MCP、模型/Provider、知识库、记忆、节点/Sandbox、Site App 已在首期以 AgentConfig 所需能力迁移；它们剩余的非首期功能也应按本模板补齐。Environment 不是资源，不进入此任务模板。
...
建议后续顺序：先补齐首期资源未覆盖的边缘功能，再处理 Machine/Sandbox/ACP transport 的非 AgentConfig 能力，之后是 Workflow/Scheduler/Webhook 与既有 Chat/YJS 边界的集成。AgentConfig 仅在强依赖资源的首期能力均完成后进入 EE 的端到端接入验证。
...
| 3 | AGT-01 单一 Agent Runtime package | 🔒 等待 ARC-03 | AGT-00、FND-01、ARC-03 | `@fenix/agent-runtime`：Environment、Instance、Runtime、relay/session、Chat/YJS；不改资源 schema |
| 3 | WEB-01 CE Web Shell 与装配骨架 | ⬜ 可领取 | FND-03、FND-05 | `apps/web` Shell、路由装配；不创建资源页面 |
| 3 | REF-01 至 REF-04 资源运行能力 | 🔒 等待 DAT-01 | DAT-01、DB 锁空闲 | 模型/Provider、Skill、MCP、知识库/记忆的首期 service/route/web/运行解析；独占 DB 锁 |
| 3 | ENV-01 节点、Site App 与 Environment 过渡 facade | 🔒 等待 DAT-01、AGT-01 | DAT-01、AGT-01、DB 锁空闲 | 节点/Sandbox/Site App service/route/web、Environment 兼容入口收口；不创建 Environment 资源包；独占 DB 锁 |
...
| 5 | EXE-01 非 AgentConfig 执行与连接能力 | 🔒 等待 M1 | M1、ARC-01 | Machine/workspace/file/Sandbox/引擎 adapter 的后续 task；复用 `@fenix/agent-runtime` port |
| 6 | ORC-01 协作与编排能力 | 🔒 等待阶段 5 相关依赖 | EXE-01 相关依赖完成 | 基于既有 Chat/YJS 边界拆分 Workflow、Scheduler、Webhook、Channel task |
