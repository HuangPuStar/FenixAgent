...
| T12 | 收尾：台账复核、文档修订、证据留痕 | **已交付**（六批功能改动 + 评审记录） | ①`aa9a3cc22` ②`7afd2188c` ③`c961fca02` ④`5a3238dbe` ⑤`f98c8bf7d` ⑥`e8714f8f1`；评审记录 `98c47f642` + `971a56027`，详见 §7.31 |
...
「迁移后产物稳定」留下可复核的证据。按「每批独立可 `precheck`、独立提交」拆成六批功能改动；
评审记录本身是第七次提交（`98c47f642`），其后的口径订正为第八次（`971a56027`，见下方「三处口径订正」）。
...
本节汇总 T3 起的迁移波次中**用户可感知**的差异（前半为「包内实现与线上 chat-channel 实现」之间，T9
起为宿主与包之间的装配变化），供发布说明与回归验收使用。分两类：切换后修复（此前行为是坏的）、
有意的呈现取舍（此前行为正常，包内实现选择不同）。
