...
| 表对象总数 | 45（包内）+ 4（宿主） | 宿主 4 张 = D3 裁定的 `resource_permission` / `share_link` / `share_event_snapshot` + `data_migrate_record`（见 §8.5 第 1 条） |
