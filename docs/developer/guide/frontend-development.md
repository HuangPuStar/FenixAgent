...
> **版本**：v3.0.29 | **最后更新**：2026-09-23 | **维护者**：前端团队
>
> **最近变更**：
> - v3.0.29 (2026-09-23)：**第 20 轮（收尾轮）——门禁终检 + 前十九轮主题复扫 + 文档对账**。全部门禁复跑：`precheck` 17 步全绿（106.6s）、`build:web` 通过、`check:web-style` **存量 0 / 新增 0**、`architecture:check`（2414 文件 / 10 规则 / 1 条已登记例外）、`check:dependencies`（2616 模块 / 12 条已登记例外 / 0 条新增违规）。九项主题按各自口径复扫，**均无回退**：① 前端生产文件超 500 行 **0 个**（701 个文件，最大 496 = `mcp/…/agent-mcp-dialog.tsx`，与 §4.8 口径同命令）；② `check:web-style` 存量仍为 0；③ 裸 `fetch` / `XMLHttpRequest` / `WebSocket` / `EventSource` 在 `apps/web/src` + `packages/**/web/**` 内命中 **11 个文件**，逐个对得上 §5.3 登记（9 处登记 + 统一层底座 `request.ts` + 仅注释提及的 `prompt-input-context.tsx`），R18 下沉的 `html-plugin.ts` 未回流；④ `dangerouslySetInnerHTML` 仍是 **3 处**、全部经 `knowledge/web/lib/sanitize-html.ts` 的显式白名单，iframe 的 UGC 面（`IframePreview` 两处、knowledge `srcDoc`、`html-plugin`）取值与 R2/R18 收口时一致、无 `allow-same-origin` 回流，`SiteFrame` / `AgentSitesCard` 的 `allow-same-origin` 属 §6.2 表内既定分级；⑤ 组织身份的 `localStorage` 读点仍只有契约 `@fenix/web-runtime/lib/active-org`、写点仍只有身份包 `OrgContext`（键经常量，无字面量）；⑥ 手写取数复扫出**一处漏网**并修掉（见下）；⑦ 直接 `await` 域模块不解包：逐条核对命中均落在 §5.9 登记的 11 个「数据或抛错」模块签名上，或调用点自带 `success` 判断，无新增；⑧ 用户可见中文字面量按 §9.4 的可复现命令在本轮改动**前**实测 **504 行 / 103 文件**，与 R19 收口后的 504 逐字一致（未回流；比 R19 记的「54 个文件」多出的部分，是命令口径里「代码行尾注释 / JSX 注释块里的成对引号」造成的假阳性）；本轮唯一的代码改动又加进 1 处**诊断日志串**（`console.error("[RetrievalTestPanel] …")`，属 §9.4 已登记的「开发诊断串」类、不进界面），故交付态同口径为 **505 行 / 103 文件**；⑨ 原生 `confirm()` / `alert()` / `prompt()` 与 `window.location` 写操作仍是**零命中**（`ui-components/web/testing.ts` 那处属测试工具）。**本轮唯一代码改动（一处漏网）**：`knowledge/…/RetrievalTestPanel.tsx` 的 rerank 候选列表原为手写 `useState` + `useEffect` 且用 `resp.data ?? []` 直取**未** `unwrap`——失败会被读成「确实没有模型」并与空下拉静默合流（§5.2 / §3.4 末条），现改 `useRequest` + `unwrap()`，失败只进诊断日志、下拉仍为空（与 §3.6 登记的「候选失败 = 空下拉」同一口径）；用例加在已有的 `knowledge/web/__tests__/knowledge-panel-load-failure-states.test.tsx`（替身新增 `rerankResponses` 口子），同时钉住「不阻断检索入口」与「不静默」两侧。**同批复扫出的另 2 处同类形态经核实仍保留手写**并按位置登记进 §3.6（理由与移除条件在该处）。文档对账：§11.1 第 3 条「`precheck` 覆盖不到的前端测试」表述已过期（第 17 步就是 `apps/web/src/__tests__/`）→ 改准；§11.2 的 17 步步骤名清单与 `scripts/ci.ts` 逐条核对**一致**（含 `web-style`、`tsc (packages)`、`dependency-boundaries`），R17 的 15 → 17 订正完整；§1.3 / §11.2 的 `*-browser-surface.test.ts` 份数 13 → **14**（插件市场移植带入第 14 份，早于本轮）；§4.8 的 400–499 行区间 29 → **30**；§6.5 的第三处 sanitize 调用点文件名按 §4.7 拆分后的实际位置改准（`RetrievalTestPanel.tsx` → `retrieval-chunk-card.tsx`）；§10.1 的 CSS 四类计数按本节命令重跑与文档逐项一致，未改。
> - v3.0.28 (2026-09-23)：**前十八轮报出的 i18n 遗留命中收口 + 组件重复实现核查（第 19 轮，§9.4 / §4.8 / §11.3）**。A 项（i18n）逐条处置前批报出、当时判为超范围的六处：① `task/agent-tasks-utils.ts` 的 zod 校验与 cron 校验共 20 处中文改成字典键——`validateCronExpression` 由「返回文案」改成**返回键**（`error.cronRequired` / `cronFieldCount` / `cronInvalid`），`taskFormSchema` 的 `message` 只装键（新增 `message()` 包装把键收进 `TaskValidationKey` 联合类型，写错键在 `tsc` 阶段就报错），译发生在 `TaskForm` 的字段错误行与 `CronEditor` 的防抖错误行；`error.*` 里原本 6 个**无读取方**的键有 5 个就此获得消费者。② `task/components/CronEditor.tsx` 的 `describeCron` 四条动态模板（每 N 分钟 / 每天 / 每周 / 每月 / 指定月）+ 时段词 + 星期字形共 15 处改走 `cron.describe.*`（含 `daySeparator`：中文顿号与英文逗号是不同的连接符，不能写死）；**各分支的既有差异原样保留**（指定月分支的时段与时刻之间没有空格，由 `monthlyOnDate` 这条模板自己承担），文案逐字不变由 `cron-text-stub.ts`（读真实 zh 字典做插值的最小替身）驱动的三份表驱动用例钉住。③ `knowledge/…/RetrievalTestPanel.tsx` 的 10 个语言名改走 `retrieval.languages.*`（后端认的值仍是 English 名，只有展示名进字典）。④ `knowledge` 的导入 / 覆盖弹窗与两个域 hook：`importDialog.importBtn/importing/confirmImport/cancel`、`confirm.overwriteTitle/overwriteDescription/overwriteNameSeparator`、`toast.imported`、`toast.remoteDeleted`，并删掉 `t("validation.embeddingModelFormat") || "向量模型格式不对…"` 这行**永不命中**的死兜底（i18next 缺键时回显 key，不会返回空串）与一条与实现不符的旧注释（「先检查 RAGFlow key」——`openCreateDialog` 只复位字段）；同批顺扫 R9 拆出的新模块，未发现同类遗漏。⑤ `ui-components` 的预览失败态：`preview-source.ts` 不再抛写死中文，改抛**结构化**的 `PreviewSourceError`（带 `status`，message 是英文技术串、只供日志），`FileViewerPreview` 的失败块按当前语言取 `fileTree.preview.loadFailed`（插 `{{status}}`）/ `loadFailedUnknown`，原始错误只进 `console.error`——这同时收掉了「原样展示 raw message」（§9.3）与 README「已知限制」第 13 条（其移除条件正是「改为抛结构化错误」，本轮兑现）。⑥ 全仓复扫（口径：字符串字面量 + JSX 文本中的中文，排除注释 / 测试 / 字典；`apps/web/src` + `packages/**/web/**`，排除 `__tests__`、`routeTree.gen.ts`、`/src/server/`、`/src/routes/web/`）**修复前 554 处 / 62 个文件 → 修复后 500 处 / 54 个文件**，未修的按**具体文件位置**登记进 §9.4（最大一块是 model-management 两个生产路由页的 301 处领域数据；另有 chat 演示数据 96 处、chat 叙述层与默认值 30 处、预览插件 7 处、agent-config 兜底 12 处，以及两类判定为「不是用户可见文案」的 28 + 26 处）。**B 项（组件重复核查）**：§11.3 记的「2 处本地重复实现」经 git 追溯是文档重写带进来的旧数（§4.8 无对应条目）；按「生产 `.tsx` 文件名重名」与「同一包内组件声明重名」两个机械判据复扫，真实命中**只有 1 处**——workflow 的 `CollapsibleGroup` 在 `node-config-fields.tsx`（原生 `<details>`）与 `RunParamsDialog.tsx`（`useState` + `ParamGroupHeader`）各一份，已收敛为 `components/CollapsibleGroup.tsx`（标题行复用包内既有的受控件，展开态由容器持有；视觉差异：节点配置卡的工具分组由三角标改为 `▶`），新增 `collapsible-group-single-source.test.ts` 钉住「只有一份实现 + 两个调用点都走共享件」；另有 2 处**同名但契约不同**（`nodes.tsx` 的 `StatusDot` 是节点徽标里的图标槽、`ui-components/ui/status-dot` 是通用状态圆点；sandbox 与 knowledge 各自的 `InstanceRow`）核实为不是重复实现，写明理由后不动。`DataTable` / `BatchActionBar` 的零消费核实：命中只有实现文件、两处 barrel、`package.json` exports、**demo 页面**与 5 份包内用例，生产代码零命中——判为**保留**（对外 `config/` 契约 + demo 是活的画廊，与 `EmptyState` 的历次处置同口径），并把「外部确认不需要时才连 demo / 用例 / exports 一次删净」写进移除条件。测试：task 包新增 `cron-text-stub.ts` 替身（缺键直接抛错）并把三份 cron 表用例与 `cron-validation-pure` 改挂真实字典、`task-i18n.test.ts` 补「`message()` 键与 cron 校验键都在字典内」与 `cron.describe.period/weekday` 键组齐备两条；`ui-components` 的 `preview-source.test.ts` 补结构化错误断言、`barrel-exports.test.ts` 登记新导出。验证：`bun run precheck` 17 步全绿；`bun run build:web` 通过；task / knowledge / ui-components / workflow 四个受影响包与 `apps/web/src/__tests__/` 全绿。
> - v3.0.27 (2026-09-23)：**安全域的仓库级复扫（第 18 轮，§5.3 / §6.1–§6.5 / §11.3）**。R2 只修了 §6.5 登记的四条，本轮把 §5.8 与 §6 全章按实测重扫一遍（口径 `apps/web/**` + `packages/**/web/**`，不管辖 `ui-sandbox/`、`packages/**/src/server/**`、`chat-channel` 的传输层）：**新增违规为零**——`dangerouslySetInnerHTML` 仍 3 处且全走 `sanitize-html.ts` 显式白名单；`allow-same-origin` 未回流（`IframePreview` 两处 iframe 的 `sandbox` 仍不可被 props 覆盖、`src` 仍过协议白名单；knowledge 的 `srcDoc` 与工作区预览器的 `html-plugin` 两处 UGC 面仍是 `allow-scripts`）；原生 `confirm()` / `alert()` / `prompt()` 仍零命中；`localStorage` / `sessionStorage` 除已登记的 master key 外无任何凭据，也没有 `document.cookie` / `indexedDB`。**修掉两处、补一处过期登记**：① **预览器把后端读取握在组件包里**（本轮唯一的真违规）：`ui-components` 的 `FileViewerPreview` 用全局 `fetch` 读宿主文件代理路由、`html-plugin`/`preview-source` 各留一份 `fetch` 兜底（后者写作 `= fetch` 默认参数），而 `…/fs/<path>?preview=true` 这条后端路由字面量还写在包内的 `defaultBuildPreviewUrl` 里——§5.8 的「组件中裸调 `fetch`」与「组件中拼装后端 URL」两条同时成立，且**旧登记把它误记为「本地 blob 读取，不是后端调用」**（真正的本地 blob 读取是 `prompt-input-file.ts`，两者此前挤在同一行）。现按能力缺口下沉：宿主 `api/fs.ts` 新增 `buildPreviewSourceUrl` + `readPreviewSource`（裸 `fetch` + 返回原始 `Response` 由调用方消费，与 sandbox `executeCommand` 同形态），经 `artifacts-files-workspace.tsx` → `PreviewTab` → `FileViewerPreview` 的 `buildPreviewUrl` / `fetchPreview` 注入；`fetchPreview` 在 `PreviewTab` / `FileViewerPreview` 与 `htmlPreviewPlugin(fetchPreview)`、`loadByteAccuratePreviewSource(url, fetchPreview)` 四处都是**必填**（包内不留兜底，忘了注入直接编译不过），§5.3 拆行重写、README「已知限制」补第 14 条。守卫：包内 `preview-fetch-injection.test.ts`（注入生效 + 源码无 `= fetch` / `, fetch,` / 裸 `fetch(` 形态）与宿主 `preview-read-injection.test.ts`（两处注入齐备 + 全宿主只有 `api/fs.ts` 拼该 URL）。**顺带的复扫发现**：这个违规躲过了 R2 与历轮，是因为 `fetch` 被当值传递——形态 grep 扫不到，§11.3 的相应行已把口径写清楚。② **工作流运行参数整份落控制台**（`use-workflow-run-params.ts` 的 `console.log("[RunButton] meta.params:", JSON.stringify(meta.params), …)`，点一次运行按钮打一次，payload 无界且可能含密钥类默认值）**已删**并留注释；E 项其余命中都不含凭据（`prompt-input-file.ts` 只打长度/大小、`use-cytoscape-graph.ts` 打图谱初始化、Context Queue 那条由 Ctrl/Cmd+Shift+D 显式触发），`request.ts` 记完整 URL 但不脱敏的理由是**全仓没有调用方把凭据放进 query**（凭据只走 `Authorization` 头）。③ **过期登记**：§6.3 白名单表里的 `wf-editor:chat-open` 已随编辑器改造消失（另把表补成键名精确的全量表），`admin-key.ts` 注释里「§6.2」的交叉引用改回 §6.3；§11.3 的「`localStorage` 读写组织身份」行原写「当前 2 处违规」是 R14 收口前的旧状态，实测直读点 0、直写点 1（身份包 `OrgContext.tsx`，经键常量），本轮补上**写入侧**的静态守卫（读侧那条 R14 已有）。B 项另核实：`rcs-lang`、布局/折叠/语言等键与身份无关，不并入违规。验证：`bun run precheck` 17 步全绿；`bun run build:web` 通过；`apps/web/src/__tests__/` 与 `packages/ui-components`、`workflow`、`knowledge`、`agent-config` 等受影响包测试全绿。
> - v3.0.26 (2026-09-23)：**「Dialog 关闭清理状态、表单重置走 `key`、不手写 `useState` 校验」的仓库级扫描与批次收口（第 17 轮，§4.2 / §4.3）**。前十六轮覆盖了取数三态、错误文案、超 500 行拆分与重复实现；本轮扫 `apps/web/src` + `packages/**/web/**` 的 310 个含弹窗的 `.tsx`（口径与 §4.8 的 698 个生产文件同一集合，按 `<Dialog|AlertDialog|Sheet|FormDialog|ConfirmDialog>` 收窄），信号是 `onOpenChange`（249 处引用 / 64 处自定义处理器）、`reset(`（6 处）、`key=` 的有无，再逐条读代码核实**能不能真的看到残留**——这条核实改变了结论：**多数弹窗在「打开时重置」**（`openCreate` / `openEdit` 里清字段），因此「关闭后带上次输入重开」的可见缺陷比形态计数少，真正命中的是三类——① 重置写在 effect 里而不是 `key`（§4.3 明令），`useState` 初始化器才是那一份新草稿；② 关闭路径靠 `setTimeout` 绕开退出动画；③ 必填校验写在提交处理函数或请求体构造里，错误只落 toast 甚至**静默 return**。**已修 5 个生产文件**：① **模型供应商 / 模型两个编辑弹窗**（`model-management/…/provider-editor-dialog.tsx`、`agent-models-dialogs.tsx` 的 `ModelEditorDialog`）——删掉 `useEffect([target, editing])` / `useEffect([original])` 的回填，草稿改由 `useState` 初始化器从 props 算一次，调用方 `AgentModelsPage.tsx` 新增 `providerDialogKey` / `modelDialogKey` 且**只在打开时自增**（关闭时不动 key：弹窗内容要留在树上走完退出动画，换 key 会把退出动画一并换掉）；副产物是首帧不再闪上一轮草稿——effect 回填要等 paint 之后才跑，`key` 当场就是对的。② **添加向量模型供应商**（`knowledge/…/add-embedding-provider-dialog.tsx`）——删掉 `setTimeout(reset, 200)` 这条「200ms 对应退出动画时长」的时序 hack，改由 `EmbeddingModelManager` 的 `addDialogKey` 驱动重挂载。③ **新建通道绑定**（`channel/…/AgentChannelsPage.tsx`）与 ④ **新建/编辑应用**（`agent-config/…/AgentSitesPage.tsx`）——两处本已在用 `FormDialog` 却把它当「只做 `preventDefault`」的 `onSubmit` 用（§4.3 明确：不传 `formConfig` 的 `onSubmit` 不做校验），校验分别是 `toast.error(t("selectPlatformAndAgent"))` 与 `if (!name.trim()) throw new Error(...)`（后者被 `onError` 接成通用的「保存应用失败」，用户看不到真正的原因）。改为 `formConfig` + zod schema，字段体按 §4.7 拆成 `components/ChannelBindingForm.tsx` / `components/AgentSiteForm.tsx`，页面只留 `formResetKey`。**schema 刻意不携带文案**（错误文案在视图边界按字段取 i18n 键，与 `agentEditorSchema` 的「错误文案在视图边界转换为 i18n 文案」同口径）——带中文 `message` 会让文案绕过 `t()`（§9.1），译文也不再随语言切换；这两处是本仓仅有的「不硬编码校验文案」的实例，`taskFormSchema` 的硬编码中文属既有偏离，不在本轮范围。必填提示从右下角 toast 改为字段级 `role="alert"`：此前的形态看不出是哪一个输入没填。**保留未改**：`AgentApiKeysPage` / `knowledge-base-form-dialog` / `prod-view-editor` 的 `onOpenChange` 只关开关或只清目标项——它们的表单字段由调用点「打开时重置」覆盖，可见行为正确，登记为**潜在**风险（新增入口若忘记重置就会漏），不为一处潜在风险改动四处调用点。§4.8 新增一条（逐条登记）（首位是登录态的 `ChangePasswordDialog`——它用 `useState` 同时承担两条规则校验与服务端错误回显，换成 `FormDialog` 要新增字典键、把 `DialogDescription` 移进 children，且 `FormDialog` 恒渲染页脚取消按钮会多出一颗按钮；更要紧的是 Radix Dialog 在本仓 happy-dom 下挂不起来，改完没有回归网兜）。测试：新增 `channel-binding-form.test.tsx` 与 `agent-site-form.test.tsx`（各 7 条，钉住「默认值只来自表单实例」「必填缺失时提交回调一次都不触发且错误落在对应字段」「换 key 重挂载后字段取自新实例且上一次的校验错误不残留」「schema 拒绝纯空白」，已做反向对照：把 `platform` / `name` 的 `min(1)` 去掉后 3 条立即失败）。两条测试环境的已知限制写进了文件头：**不渲染 Radix `Dialog`**（portal 撞 `@radix-ui/react-focus-scope` 的一串缺失全局，本仓既有登记）与**不模拟键盘输入**（happy-dom 下 React 的 `ChangeEventPlugin` 认不出合成值变更——`onInput` 会触发、`register` 绑的 `onChange` 不会），因此断言一律走 `defaultValues` 与提交回传，并且刻意避开 `SelectValue` 文本与 `Textarea` 这类**会拿到同进程其他文件注册的进程级 `mock.module` 替身**的呈现。验证：`bun run precheck` 全绿；`bun run build:web` 通过。
> - v3.0.25 (2026-09-23)：**「失败被映射成 empty」的仓库级扫描与批次收口（第 16 轮，§3.4 末那条）**。前十五轮覆盖了 memory/hindsight、workflow 面板与 knowledge 图谱，本轮扫其余前端范围（`apps/web/src` + `packages/*/web` + `packages/*/*/web` 共 71 个生产文件、153 处 `useRequest` 调用），按 §3.4 的三态示例逐条核实后**修 8 个生产文件、登记 6 条剩余项（按具体位置）**。已修（失败时一律 `EmptyState` + `tone="danger"` + `role="alert"` + 重试，文案走 `t()`、en/zh 同批补齐）：① **模型网关的用量 / 预算两个 Tab**——都是「点查询才发请求」的形态，`data` 在失败时停在 `undefined`，此前分别渲染成「请先查询」提示与一张空表（`noMatches` 行），与「还没查过」「这个区间确实没有用量」同形；判定抽成纯函数 `web/lib/model-gateway-query.ts` 的 `resolveModelGatewayQueryBranch`（无 React/DOM，与 `resource-channel` 的 `channel-list-state.ts` 同款：规则可脱离 DOM 断言），失败分支**优先于** `hasData`——手动查询下把上一轮指标留在原位属于静默错误，这条与列表口径的分叉理由写在模块头（改轮询前必须回退成 `error && !hasData`）。② **智能体管理页主列表**（`agent-config/AgentManagementPage.tsx`）失败不再渲染「暂无智能体」，判据带 `filteredNodes.length === 0` 以保留刷新失败时的已渲染列表。③ **站点挂载弹层**（`MountSiteDialog.tsx`）失败不再渲染「没有可挂载的站点」。④ **组织页**（`identity/AgentOrganizationsPage.tsx`）的组织列表失败改为**页面级**失败块（列表是整页数据源，取不到时列表与详情一起空），§5.9 那条「组织页会按『无组织』渲染」的可见后果同批收口。⑤ **向量模型管理器**（`knowledge/EmbeddingModelManager.tsx`）失败不再渲染「暂无已配置的模型供应商」。⑥ **三处完全静默的取数**补上诊断日志与可见反馈（模型网关的主体下拉、组织页的机器列表与成员候选、沙箱的组织下拉候选）——此前 `onError` 一个都没有，出问题只能看到下拉是空的。**保留未改**：`use-shell-navigation.ts` 的「取不到隐藏项就按无隐藏项处理」与 `lib/app-brand.ts` 的品牌兜底属**有意降级**（裁剪是可选偏好、品牌是启动期非关键路径，两者文件头都有说明），不补失败态。§3.6 新增条目列出**未修的 6 个具体位置与优先级**（首位是上面那条「失败分支优先于 hasData」在改轮询时的回退要求）。测试：新增 `model-gateway-query-branch.test.ts`（6 条纯判定用例，钉住「失败不得派生 idle/ready」）与 `embedding-model-manager-states.test.tsx`（3 条渲染级用例，happy-dom + 真实 i18next 实例，钉住「失败渲染失败块而非空态」「重试真的重新发请求并恢复」「成功且为空才走空态」；已做反向对照：把失败分支短路掉后 2 条用例立即失败）。验证：`bun run precheck` 全绿；`bun run build:web` 通过；受影响包测试 `bun test packages/resources/{model-management,knowledge,agent-config,sandbox}/web packages/platform/identity/web` 全绿（`model-management-i18n.test.ts` 的键数基线按新增 3 键由 410 同步到 413）。
> - v3.0.24 (2026-09-23)：§5.9 三条「实现与契约不符」条目收口（内部头可被覆盖 / 注释与实现不一致的三处 / 5 个函数式模块），全部**以代码为准改代码或改注释**，不留「注释是承诺、代码不是」的状态。① **内部头不可被调用方覆盖（§5.2）**：`headers` 从 `init` 中解构排除（原先 `...init` 在合并后的 headers 之后展开，调用方同名头会盖掉 `content-type` / `x-file-op-id` / `authorization`），改为「内部头先写、调用方只补内部未设置的头」，判定用 `Headers.has`（大小写不敏感）。**这是安全相关的行为反转**：全仓生产代码本就没有调用方传 `headers`（唯一一处 `If-None-Match` 是宿主 `fs.ts` 的裸 `fetch` 条件请求，不经本层），唯一钉住旧行为的是本条用例自身——已改为断言内部值胜出，并补「调用方仍可补内部未设置的头」这一反向用例，避免把「优先」写死成「忽略」。② **超时覆盖 body 消费**：定时器原先在收到响应头即 `clearTimeout`，慢 `json()` / `text()` 逃出超时约束（页面永久 loading）；改为在 `finally` 里释放，超时因此覆盖到 body 读完，并可掐断 body 读。顺带把非 JSON 分支的内层 `catch` 收窄为只吞 `SyntaxError`（与 JSON 分支同款判据）——否则 body 读被 abort 时会误报成「服务器返回了意外的响应格式」。③ **`anySignal` 监听器不再泄漏**：合并到调用方 `signal` 上的监听器原先要到该 signal 被 abort 才消失（长生命周期 signal 会随每次请求累积），现由 `anySignal` 返回 `dispose()` 并在同一个 `finally` 摘除。④ **是否补自动重试：不补，改注释对齐实现**。§5.2 的 `opId` 只要求「重试须复用同一 opId」，未要求重试语义，而 `docs/arch/12-files.md` §7.2 的重试矩阵讲的是机器端 file-ws 消费者；在 `request()` 里对写操作自动重试会静默把写操作超时拉长到 2 倍（fs 写操作 120s → 240s）并改变既有失败时序，属未经裁定的行为变化。故按「注释与实现取其一」把 catch/`opId` 注释与 §5.2 改写为真实语义（**本层不自动重试，重试是调用方策略**），并把由此暴露的宿主侧缺口登记进 §5.9（`fs.ts` 每次调用新生成 opId，调用方重发不复用键 → 幂等承诺目前不成立）。⑤ **§5.5 的 5 个函数式模块收敛为 `*Api` 对象**：`modelGatewayApi`（12 个具名函数 → 去模块前缀的成员：`check` / `getConfiguration` / `sync` / `listBudgets` / `listUsers` / `listAgents` / `queryUsage` / `updateBudgets` / `resetBudgets` / `listKeys` / `removeKeys` / `queryMyUsage`）、`observerApi`、`systemLogsApi`（含裸 `fetch` 的 `download`）、`systemPeopleTreeApi`、`systemOrganizationsApi`，全部调用点（含包外 `use-model-gateway-dashboard` 经 `@fenix/resource-observer/web` 的引用）同步改指；**只改命名与导出形态**，返回值语义（域内 `unwrap()` 的「数据或抛错」两代写法）按 §5.9 保留未动，`buildSystemUserIdentifier` 这类纯值装配仍留对象外（与 sandbox `buildSandboxResourcePatch` 同形）。§5.9 删除已收口条目，其余未处理项（两代解包、`frontend-no-legacy-api-prefix` 只识别裸 `request()` 调用等）原样保留。验证：`bun run precheck` 17 步全绿；`bun run build:web` 通过；`bun test packages/web-runtime`（含 6 条新增/改写的契约用例，已做反向对照：摘掉 `dispose()` 或提前 `clearTimeout` 时对应用例立即失败）。
> - v3.0.23 (2026-09-23)：§8.6 实时通信现状偏离收口（4503 的 `uiCode` 缺口按原裁定**保持 `null`**，未动）。① **组织 id 读取收敛到唯一契约**：新增 `@fenix/web-runtime/lib/active-org`（`ACTIVE_ORG_STORAGE_KEY` + `readActiveOrgId()`），身份包 `OrgContext.tsx` 的 fetch 拦截器与两条 WS 通道（`agent-runtime/web/yjs/yjs-ws.ts` 的 `buildYjsUrl`、宿主 `api/file-events.ts`）**共用这一份读取**，键字面量不再各写一份——收敛前两处直读 `localStorage`，是 §3.6 登记的最后两处生产点。**不是第二份组织真相**：写入、乐观更新与失败回滚仍完整留在身份包 `switchOrg` / `refreshOrgs`；WS 无法带自定义头这一前提没变，组织仍走 query（移除条件写在模块头，`apps/web/src/__tests__/file-events-org-contract.test.ts` 静态钉住「全仓只有这一处读该键」）。② **`/web/file-events` 的 URL 与帧解析下沉到宿主域模块** `apps/web/src/api/file-events.ts`（`buildFileEventsUrl` / `openFileEventsConnection`），`use-file-tree-events.ts` 退化为只消费入口（重连退避、页面可见性与失效去抖仍是调用方策略）；同批补静态断言「组件与页面目录下不出现 `new WebSocket(` / `new EventSource(`」。③ **补前端发送背压**：`chat-channel/src/transport/ws.ts` 的 `send()` 增加 64 KB（**字节**，与服务端 `broadcaster` 同值）软阈值判定，超阈值拒绝本次发送并让 `send()` 返回 `false`——原先只判 `readyState`、静默排队，`use-chat-panel-runtime.ts` 的 `sendViaWs` 据此走既有失败反馈；只拦排队，**不改** `readyState`/close/`WS_CLOSE_CODE_POLICY` 的任何行为（§8.3 不变量）。④ **会话切换清理彻底走换代**：删除 `chat-writer.ts` 的 `clearSessionDocContent` 与它专用的 `clearPeriTaskViews`，并清掉其生产引用链（`chat-channel-bootstrap.ts` 的 `prepareClearSessionSnapshot` 注入 → `persistClearedSessionSnapshot` → `controller.ts`/`session-channel.ts` 的同名依赖端口；该端口本就零调用、注释自述"projection replacement 不再调用"）；`clearChatDocContent` 与本批无关，保留。同批同步 §3.3/§3.6/§8.1/§8.4/§8.5 第 7 条与 `CLAUDE.md` 的 YJS/Chat 第 7 条。验证：`bun run precheck` 17 步全绿；`bun run build:web` 通过。
> - v3.0.22 (2026-09-23)：§2.7 导航与路由装配的三条偏离收口，两条无裁定项写清取向。① **`DefaultAppShell.tsx` 的 `navigate({ to: `/agent/${pageId}` as never })`**（本文档点名过的反面样例）改为类型安全写法：路径拼法收进 `shell-navigation.ts` 的 `panelRoutePath()`（`/agent/` 前缀只写一份），导航前先经 `isPanelRoutePath()` 按 **router 的公开字段 `routesByPath`** 确认目标真是路由表里的面板页路径；接受它的类型 `PanelRoutePath` 取自注册的 router（`AbsoluteToPath<RegisteredRouter, "/">` ∩ `/agent/${string}`），与运行时判据同源（都是 `routeTree.gen`），因此 `as never` 不再需要——命不中的分支在正确构建下不可达，真出现只可能是「包声明了导航却没建路由壳」的装配漂移。② **补 `apps/web/src/__tests__/shell-navigation-routes.test.ts`**：把「导航 `id` → 路由目标」钉在两端真实产物上——导航侧取装配结果 `ASSEMBLED_NAV_GROUPS`（与侧栏渲染同源），路由侧取 `routeTree` 实例化后的 `routesByPath`（`createMemoryHistory` 使其在无 DOM 的 `bun test` 里可建），共用 `panelRoutePath()` 拼路径、不另写「id → 路径」映射表；另加一条反面断言（编造的面板页路径必须不被路由表认下）防止检查空转。实测把 skill 的 `id: "skills"` 改成 `"skill"`，该用例立即失败——此前这类错字装配通过、类型通过、既有测试全绿，只有用户点击才 404。③ **`CLAUDE.md` 两处陈旧表述同步**：删掉「Sidebar 导航项必须提供 `to`」（实现是 `ShellNavigation.tsx` 的 `<button onClick={onNavigate(item.id)}>`，路由目标由 `id` 拼装，没有 `to`），并从允许的导航手段里移除 `router.invalidate()`（全仓零使用、无真实范例）。④ §2.7 两条**无裁定项**补上核实结论、两种取向的影响面与建议：`/admin` 的第二张硬编码导航表建议**显式登记为平行装配**（收敛需改 §2.6 跨包契约，而 `/admin` 本就豁免 better-auth 会话）；三个「有路由无导航项」的页面核实为**包未进入装配 profile**（`resource-channel` / `resource-prod-view` 无 `web/contribution.ts` 且不在 `ce.json` 的 `web` 列表）而非迁移遗漏，建议保留并另立议题，**页面行为本轮未动**。验证：`bun run precheck` 17 步全绿；`bun run build:web` 通过；`apps/web/src/__tests__/` 全通过。
> - v3.0.21 (2026-09-23)：§4.8 超限文件收口**批次五（收尾）**——拆完 model-management 的网关管理页与 agent-config 的创建首页，前端生产文件（`apps/web/src/**` 与 `packages/**/web/**`，排除测试、生成文件与服务端路径）**不再有超 500 行的文件**（实测 0 个，最大 496，400–499 行带 29 个）。① `model-management/web/pages/admin/AdminModelGatewayPage.tsx`（1251 → 壳 153）按「壳与分派 / 跨 Tab 编排 / 各 Tab 控制器 / 渲染」切：`use-model-gateway-dashboard.ts`（193：检查、配置、同步、概览用量四个请求 + 用户 / Agent / 组织三路主体数据源，含「切到该 Tab 才拉」「输入 3 秒静默后才查」「新关键词先取消旧请求」三条既有策略，以及 `UNAUTHORIZED` 才清 Master Key 的归一）+ `use-model-gateway-budgets.ts`（128：预算 Tab 的筛选 / 分页 / 勾选 / 弹窗状态与三个预算请求）+ `use-model-gateway-usage.ts`（59：用量 Tab 的范围与四路筛选）+ `model-gateway-overview-panel.tsx`（213：概览面板，props 与拆分前逐字一致）+ `model-gateway-models-panel.tsx`（185）+ `model-gateway-budgets-panel.tsx`（369）+ `model-gateway-budget-dialogs.tsx`（119：设置 / 重置两个弹窗）+ `model-gateway-usage-panel.tsx`（262，含只在本 Tab 消费的 `UsageBreakdown`）+ `web/lib/model-gateway-format.ts`（26：金额与紧凑计数两个纯函数，概览与用量共用）；同时把同屏逐字重复的原生控件类串与指标卡 `Metric` 收进 `model-gateway-shared.tsx`（三 Tab 的筛选条、两个面板的指标区各拼一份）。**状态归属是本轮的切分判据**：模型搜索、预算筛选与勾选、用量范围仍由页面持有（四个 Tab 是同一长生命周期组件内的条件渲染，切走再切回不该丢），两个 Tab 控制器以 hook 形态由页面调用、整包传给面板正是为了让状态活过 Tab 切换——搬进面板会变成行为变更。全仓唯一一处 `useRequest` 的 `cancel()` 用法（`SearchableUsageFilter` 的 `onSearchChange`，§3.6 范例）原样保留，改由编排层导出 `onUserSearchChange` / `onAgentSearchChange`（先取消再记关键词）。② `agent-config/web/pages/agent-panel/pages/AgentHomePage.tsx`（675 → 257）拆成：`agent-home-styles.ts`（344：内联 `<style>` 的样式表，逐字搬运，页面仍以 `<style>{AGENT_HOME_STYLES}</style>` 就地渲染）+ `agent-home-template-pills.tsx`（72：`AgentTemplate` 形状 + 色系/图标登记 + 卡片列表）+ `agent-home-creation.ts`（78：取可用模型 → 创建 / 复用 agent 配置 → 解析实例跳转目标，归一为判别式结果，页面保留 toast 文案与路由选择）。③ **取数与请求顺序零改动**：十个 `useRequest` 的 `manual` / `onSuccess` / `onError` 语义、两条防抖 effect、预算重查的 `tab === "budgets" && hasQueriedBudgets && budgetQueryKey` 三段判据逐字保留（第三段以 `active` 入参继续生效）。④ 同批更新两个浏览器面白名单（model-management 补 9 条、下界 25 → 41；agent-config 补 3 条、下界 27 → 30）与 `agent-home-generation.test.tsx`（创建流程移出页面后，该源码断言改为同时读页面与 `agent-home-creation.ts`，契约不变）。纯结构重构：DOM 结构、类名、i18n key、请求与事件顺序未动；R6 的错误文案、R7 删除 `errorMessage()` 后的口径、R8 拆出的 `agent-models-dialogs.tsx` 均未回退。验证：`bun run precheck` 17 步全绿；`bun run build:web` 通过；`bun test packages/resources/model-management` 243 例、`bun test packages/resources/agent-config` 751 例、`apps/web/src/__tests__/` 261 例全通过；行数复测超限文件 0 个。
> - v3.0.20 (2026-09-23)：§4.8 超限文件收口**批次四（memory 包 hindsight 三件套）**——按职责拆开，拆完每个文件 ≤ 500 行（最大 393），全仓超限生产文件由 5 个降到 2 个（只剩 `AdminModelGatewayPage` 1251 与 `AgentHomePage` 675，留给后续批次）。① `hindsight/components/Graph2d.tsx`（713 → 壳 130）拆成：`graph-model.ts`（87：**两套图谱共用**的数据形状 + 因果族类型判定 + 接口返回 → 视图数据的转换，`Constellation` / `DataView` / `EntitiesView` 都从这里取类型）+ `graph2d-model.ts`（128：限量、cytoscape 元素表、初始化签名，纯函数）+ `graph2d-styles.ts`（157：主题化样式表 + fcose 布局参数 + 扩展注册）+ `use-cytoscape-graph.ts`（393：容器挂载、实例创建 / 重建 / 销毁、交互事件与悬停状态）；渲染壳只剩样式化容器与加载 / 空态 / 链接 tooltip。② `Constellation.tsx`（978 → 壳 329）拆成：`constellation-scene.ts`（202：星点世界坐标、Pretext 预排版、连线索引、热度色带与字体常量）+ `constellation-state.ts`（54：逐帧交互状态的字段口径与初值）+ `constellation-labels.ts`（73：卡片 / 行内两种标签画法）+ `constellation-paint.ts`（276：背景、连线、节点与标签去重栅格）+ `constellation-legend.ts`（125：HUD、连线类型图例、热度色带、尺寸图例、操作提示）+ `constellation-tooltip.ts`（110：节点 tooltip 的 DOM 重建与跟随定位）。③ `DataView.tsx`（955 → 壳 323）按 §3.5 三层拆：`data-view-model.ts`（106：视图概念类型、链接归类与统计、观察类型尺寸与近期热度两段扫描，纯函数）+ `use-data-view-data.ts`（211：图谱与整合状态两个 `useRequest`、图谱数据过滤、页面级派生回调）+ `data-view-control-panel.tsx`（79：右侧控制面板）+ `data-view-table.tsx`（173：表格 + 分页）+ `data-view-timeline.tsx`（299：时间线视图，粒度与游标仍自持）。④ **外围重复的收敛（本轮只有这一处，且不动图形逻辑）**：因果族链接类型名单（`causes` / `caused_by` / `enables` / `prevents`）此前在 `Graph2d` 的 tooltip 文案、`DataView` 的类型归类与链接配色里各写一份，收成 `graph-model.ts` 的 `CAUSAL_LINK_TYPES` + `isCausalLinkType()`；**§4.8 已裁定的两套图谱图形逻辑不做归一仍在遵守**——cytoscape 与自绘 canvas 各自的绘制、布局、命中测试零交叉，`graph-model.ts` 只承接数据契约与被三处共用的类型名单。⑤ 取数与状态归属不变：R4 的 `useRequest` 三态与 9 处手写取数收口原样保留（`data` / `loading` / `error` / `refresh` 仍由 `useRequest` 提供，失败仍由渲染层用 `toHindsightFailure` 派生成可见失败块）；页面级状态（当前视图、紧凑态、表格页码、选中节点、详情弹窗、面板开合）仍留在 `DataView`，未下沉到子组件——它们要跨视图切换存活。⑥ 同批更新 `memory-browser-surface.test.ts` 的可达面清单（补 5 条新共享件，下界 19 → 33）与 §4.8 的超限清单。纯结构重构：DOM 结构、类名、i18n key、绘制顺序、请求与事件顺序未动；`Graph2d` 的链接 tooltip 判断、`DataView` 表格里恒为假的那条标题分支、以及 `fullDataRef` / `_hoveredNode` / `TimelineView` 的 `_data` 三处既有空转原样保留（不在本轮范围）。验证：`bun run precheck` 17 步全绿；`bun run build:web` 通过；`bun test packages/resources/memory` 112 例（其中 web 面 39 例，含 `hindsight-data-fetching.test.tsx`）与 `apps/web/src/__tests__/` 261 例全通过；行数复测 web 口径超限文件 2 个、400–499 行带仍 29 个。
...
- **浏览器安全入口守卫未覆盖全部带 web 面的包**：14 份 `web/__tests__/*-browser-surface.test.ts` 全在 `packages/resources/*`；`platform/identity` 有完整 `./web` 面但无守卫，`ui-components` / `web-runtime` / `agent-runtime` 的 web 面同样无守卫（当前只作为别人导入图里的共享基础设施被间接断言）。
...
4. 宿主建路由壳 `apps/web/src/routes/agent/_panel/<id>.tsx`（`id` 必须与路由文件同名，否则点进去 404；这条不变量由 `apps/web/src/__tests__/shell-navigation-routes.test.ts` 按真实路由表逐项断言，打错 `id` 在装配期即失败）。
...
以下两条**尚无裁定**：行为维持现状，改动前先按各自的影响面裁定方向。

- **`/admin` 有第二张硬编码导航表**（`apps/web/src/routes/admin.tsx` 的 `NAV_ITEMS`，5 项，`to` 是字面量联合，因此它自己天然类型安全），完全绕过 contribution 装配。两种取向：
  - ① **收敛到 contribution**：`/admin` 页面不挂在 `_panel` 布局下，`id` 拼出的 `/agent/<id>` 对它们不成立，收敛得先给契约加「完整路径」字段（或第二套前缀规则），等于改 §2.6 的跨包契约并动到既有 15 项；且 `/admin` 走 `AdminKeyGate` 而非 better-auth 会话（§2.3），与侧栏本就不是同一张表。
  - ② **显式保留为平行装配**：只需在本文档写明它不参与 `hiddenTabs` 裁剪与分组表，新增 `/admin` 页面照现有形态加项。
  - **建议取 ②**：收敛的唯一收益是「只有一张表」，代价是把宿主路由前缀写进跨包契约。
- **三个页面有路由但无导航项**（`_panel/dashboard.tsx`、`channels.tsx`、`views.tsx`），只能靠输入 URL 到达。2026-09-23 核实结论如下（此前的「迁移遗漏」猜测不成立）：`channels` / `views` 的 owner（`resource-channel` / `resource-prod-view`）**没有 `web/contribution.ts`、也不在 `deploy/assembly/ce.json` 的 `web` 列表里**，根本没有可声明导航的位置；`dashboard` 的 owner（agent-config）在 profile 内但未为它声明项；三者在迁移前的宿主侧栏表里也从未出现，且全仓没有指向它们的跳转。两种取向：
  - ① **保留**（建议）：登记为「包未进入装配 profile」这条更根本的欠账——真要让它们进侧栏，先得给 channel / prod-view 建 contribution 并改 profile（见 §2.6「新增新包」五步），影响面在装配而不只是版式。
  - ② **清理**：认定是死路由就删壳与页面，属删除决策，需要产品确认。

其余导航与路由装配的偏离已在 v3.0.22 收口：`DefaultAppShell.tsx` 的 `as never` 反面样例已改为按真实路由表校验的类型安全写法，`id` 与路由目标之间的一致性由 `apps/web/src/__tests__/shell-navigation-routes.test.ts` 在装配期证明（详见文件头「最近变更」）。
...
**唯一的例外是「拿不到上下文的三处非 React 执行点」**，它们统一经契约 `@fenix/web-runtime/lib/active-org` 的 `readActiveOrgId()` 取值：身份包的 fetch 拦截器（注入 `X-Active-Org-Id`）、两条实时通道的 URL 拼装（`agent-runtime/web/yjs/yjs-ws.ts`、宿主 `api/file-events.ts`——WS 握手无法带自定义头）。该模块是**全仓唯一读该键的地方**，键常量 `ACTIVE_ORG_STORAGE_KEY` 也只在它的写入点（本文件 §3.3 下面的 `switchOrg`/`refreshOrgs`）与本模块之间共享，因此它不是第二份组织真相，而是身份域持久化结果的只读投影。代价与移除条件写在模块头：读到的是"最近一次本地写入"，即组织切换瞬间建立的实时连接可能带旧组织 id；HTTP 域模块与组件**不得**调用它。当前偏离见 §3.6。

**请求头注入由身份域持有**：`OrgContext.tsx` 通过 `installFetchInterceptor()` monkey-patch `window.fetch`，每次请求现场经契约读取激活组织 id 注入 `X-Active-Org-Id`；服务端解析优先级见 `CLAUDE.md`。域模块**不得**自己读组织 id 拼 URL 或头——那会绕过这套机制。
...
- ~~**组织身份被绕过读取的 2 处生产点**~~ **已收口（2026-09-23）**：`apps/web/src/components/agent-panel/use-file-tree-events.ts` 与 `packages/agent-runtime/web/yjs/yjs-ws.ts` 原先各自 `localStorage.getItem("active_org_id")`，现与身份包的 fetch 拦截器一起改经契约 `@fenix/web-runtime/lib/active-org` 的 `readActiveOrgId()`——键与读取动作各只剩一份，直读点实测为 0（`apps/web/src/__tests__/file-events-org-contract.test.ts` 静态钉住）。WS 无法带自定义头这一前提没有变，组织参数仍走 query，变的是它不再有第二份来源，见 §3.3。
- ~~**主题强制浅色 + 存在第二份主题实现**~~ 已消解（2026-09-23）：全站改为全局强制亮色，宿主与包内两份实现合一，见 §3.2。
- ~~**手写取数（`useCallback` + `useEffect` + `useState`）的现行违规**~~ **批次二已消解**（2026-09-23）：批次一（`packages/resources/memory/web/pages/hindsight/**`，实测 9 处）见 v3.0.13；批次二收掉规范原记的**最后两处**——`packages/resources/workflow/web/pages/workflow/components/` 的 3 处（`TriggerPanel` / `VersionIndicator` / `VersionPanel`，三态改由 `error` / `data` / `loading` 派生、重试复用 `refresh`、条件取数用 `ready: !!workflowId`，失败补 `role="alert"` 失败块），以及 `WorkflowList.tsx` 的手写 `setInterval(pollList, 15_000)`（改 `pollingInterval: 15_000` + `refreshDeps: [organizationId]` + `ready: !!organizationId`）；`packages/resources/knowledge/web/pages/agent-panel/KnowledgeGraphPanel.tsx` 的手写 `requestId` 令牌删除，改用 `useRequest` + `request()` 的 `signal`（域模块 `kbApi.getGraph` 同批补 `signal` 透传）。**同子树复查后仍残留的同类形态（5 处，2026-09-23 批次三拆分后重新清点）**：批次三把 workflow 编辑器与运行域按「状态归属 / 传输适配 / 纯模型」拆开，原记的 6 处里 **1 处已收口**——custom 工具列表移到 `hooks/use-workflow-custom-tools.ts`，它的 `customTools` 全仓只有那一个写入点，直接换成 `useRequest`（+ `onError` 保留原日志）不会产生第二个所有者；**其余 5 处经拆分后仍不成立，保持手写**：草稿加载在 `hooks/use-workflow-draft.ts`（`loadDraft` / `handlePreviewVersion` / `handleBackToDraft` 三个入口都写同一份 `wfData`，换 `useRequest` 会让请求内部再持一份，除非引入本仓零先例的 `mutate`）、运行回放在 `hooks/useWorkflowRun.ts`、轮询与待审批与节点输出在 `hooks/use-workflow-run-transport.ts`——这四处读写的是 `runSnapshot` / `runEvents` / `runApprovals` / `selectedNodeOutput`，它们由 `WorkflowEditor` 顶层持有、`RunStatusPanel` 也直接写（三处共用 `resetRunView` 整组复位），换 `useRequest` 同样造出第二个所有者；轮询另有两条 `useRequest` 表达不了/不该依赖的语义：退出运行视图与重跑时要**同步**停掉定时器（不能让上一个 run 的结果回写），以及 §3.4 明确要求的「不要依赖 `useRequest` 替你取消」。`WorkflowEditor.tsx` 的其余 effect 属 DOM 测量、事件登记与渲染同步，不属违规；拆分后它们分布在 `hooks/use-workflow-editor-events.ts`（SSE 订阅、提示、调试快捷键）与编辑器壳（工作流切换复位、`meta.params` 同步）。**第 20 轮补做 repo-wide 复扫**（判据就是 §3.4 禁的那一种组合：`useState` + `useCallback` + 由 `useEffect` 自动触发，且文件完全不引 `useRequest`；前几批是按子树推进的，从未全仓跑过这一遍），在 workflow 子树之外另有两处命中，**经核实仍保留手写、按位置登记在此**：① `packages/resources/sandbox/web/src/pages/admin/components/RemoteSandboxPanel.tsx:55` 的 `loadSandboxes`（`expanded` 变真时自动取数）——它的失败态同时承担「原始 `ApiError.message` 上屏」与「`unauthorized` 判定后回调 `onAuthFailure`」两件事，换 `useRequest` 要一并改错误块文案与失效链路，属**会动用户可见行为**的改动，需单独一批；② `packages/platform/identity/web/contexts/OrgContext.tsx:68` 的 `refreshOrgs`（Provider 挂载时取数）——`org` / `role` 有**两个所有者**（`refreshOrgs` 的落定，与 `switchOrg` 的乐观写 + 失败回滚 + §3.3 要求的 `localStorage` 快照），换 `useRequest` 会让请求内部再持一份状态，除非引入本仓零先例的 `cache.mutate`；它「失败只 `console.error`、上下文无失败态」这一条另登记在 §5.9。两处的移除条件分别是「错误文案口径单独裁定并落地」与「组织切换改成原子转换（见本节下一条）」。同批复扫中还发现并**当轮修掉**了一处：知识库检索面板的 rerank 候选列表改走 `useRequest` + `unwrap()`（见文档头 v3.0.29）。
- **组织切换不是原子转换**：`localStorage` 先于服务端确认写入（见 §3.3）。失败回滚已实现，但切换瞬间存在"本地快照已变、服务端未确认"的窗口。
- **「失败落到空态」的全仓扫描与批次收口（2026-09-23，第 16 轮）**：按 §3.4 末的「失败不得映射成 empty 或成功」逐条核实，**已修 8 个生产文件**——用量与预算两个 Tab 的手动查询（`model-management/web/pages/admin/model-gateway-{usage,budgets}-panel.tsx`，判定在 `web/lib/model-gateway-query.ts`）、智能体管理页的主列表（`agent-config/…/AgentManagementPage.tsx`）、站点挂载弹层（`agent-config/web/components/agent-panel/MountSiteDialog.tsx`）、组织页的组织列表（`identity/…/AgentOrganizationsPage.tsx`）、向量模型管理器（`knowledge/…/EmbeddingModelManager.tsx`），以及三处此前**完全静默**的取数（模型网关主体下拉、组织页机器列表与成员候选、沙箱组织下拉候选）——失败时一律渲染 `EmptyState` + `tone="danger"` + `role="alert"` + 重试（或至少补诊断日志与可见反馈），`data` 为空与请求失败不再同形。**仍未收口的具体位置（按影响面排序）**：
  1. `packages/resources/model-management/web/lib/model-gateway-query.ts` 的失败分支现在顶掉上一轮结果（`error` 优先于 `hasData`）——这是有意与列表口径（§3.6 上一条的 `error && itemCount === 0`）分叉：手动查询的旧数字会被读成这次查询的结果。**若这两个 Tab 将来改成自动轮询，必须改回 `error && !hasData`**，否则每次轮询失败都会掀掉已有结果。
  2. `packages/resources/agent-config/web/pages/agent-panel/pages/AgentHomePage.tsx`：模板 pills 取数（`:55`）失败只 toast，`templates` 落成空数组后首页引导区整块消失，无失败态与重试（要补需给 `agent-home-template-pills.tsx` 加分支）。
  3. `packages/resources/sandbox/web/src/pages/admin/use-sandbox-dashboard.ts`：组织下拉候选现在有 `onError` 日志，但**失败态仍为空下拉**（`OrganizationSelect.tsx` 只收 `organizations` 数组）；要落成可见失败态需该共享选择器接受错误端口，影响沙箱资源池表单与用量页筛选项两处。
  4. `packages/resources/model-management/web/pages/admin/use-model-gateway-dashboard.ts`：主体下拉失败已补日志与提示，但同样是「提示 + 空下拉」，没有可重试的持久态（同一个 `SearchableUsageFilter`，与上一条同一处缺口）。
  5. `packages/platform/identity/web/pages/agent-panel/pages/AgentOrganizationsPage.tsx`：机器列表（`:94` 的 `machineRegistry.list`）与成员候选（`:175` 的 `searchMemberCandidates`）现在有日志与提示，但机器区的持久失败态需要把状态透进 `OrganizationsWorkspace` 的 props，本轮未做。
  6. `packages/resources/task/web/pages/agent-panel/pages/AgentTasksPage.tsx`：任务列表（`:66`）与 Agent 下拉（`:89`）取数失败都是「toast + 空态/空下拉」——toast 已是 §5.8 允许的可见反馈，但按 §3.4 的口径仍属「失败与空合流」，补持久失败块需改各面板的 props。
  7. `packages/resources/knowledge/web/src/pages/agent-panel/components/RetrievalTestPanel.tsx`：rerank 候选列表已于第 20 轮改走 `useRequest` + `unwrap()`（原先既不 `unwrap` 又空 `catch`），失败现在有诊断日志，**但下拉仍为空**——与上面 3 / 4 同类；要让这个下拉有可见失败态，需给它（或共享的 `Select` 包装）开一个错误端口。
...
- **已清零：前端生产文件无一处超 500 行**（2026-09-23 §4.8 批次五收口后实测，超限 0 个；当前最大 496，为 `mcp/web/pages/agent-panel/pages/agent-mcp-dialog.tsx`；400–499 行区间另有 30 个。口径：`apps/web/src` + `packages/**/web/**`，排除测试、生成文件与服务端路径，行数是文件总行数；统计命令：`(find apps/web/src -name '*.ts' -o -name '*.tsx' | grep -v __tests__ | grep -v routeTree.gen; find packages -path '*/web/*' \( -name '*.ts' -o -name '*.tsx' \) | grep -v __tests__ | grep -v '/src/server/' | grep -v '/src/routes/web/') | xargs wc -l | awk '$1>500'`）。已消掉的超限文件按批次登记：批次一（v3.0.17）是 500–560 行带那 10 个——9 个按职责拆分、1 个（`workflow/components/NodeConfigPanel.tsx`，557 行）经核实全仓零引用且文件头已标 `@deprecated`，按 CLAUDE.md 原则 10 直接删除（不把死代码拆成多个死模块）；批次二（v3.0.18）是 knowledge 包残留的 2 个——`AgentKnowledgeBasesPage.tsx`（1243 → 壳 301 + 两个域 hook + 详情视图 + 三个弹窗族 + 错误码映射）、`ResourcePreviewContent.tsx`（570 → 纯模型 / 取数 hook / 渲染壳 / 占位件 / 表格预览，最大 177）；批次三（v3.0.19）是 workflow 包的 3 个——`NodeConfigCard.tsx`（1180 → 壳 233 + 字段原语 / 五个按类型的分区 / 纯模型 / 下游引用确认，最大 287）、`WorkflowEditor.tsx`（1077 → 壳 481 + 画布与其浮层 / 右下角动作组 / 浮层族 / 六个 hook，最大 248）、`hooks/useWorkflowRun.ts`（569 → 编排 388 + 传输层 139 + 生命周期命令 208 + 画布运行标记纯模型 119）；批次四（v3.0.20）是 memory 包 hindsight 的 3 个——`Constellation.tsx`（978 → 壳 329 + 场景模型 / 逐帧交互状态 / 标签 / 绘制主体 / 图例 / tooltip 六个模块，最大 276）、`DataView.tsx`（955 → 壳 323 + 纯模型 / 取数编排 hook / 控制面板 / 表格 / 时间线，最大 299）、`Graph2d.tsx`（713 → 壳 130 + 共用数据契约 / cytoscape 纯整形 / 样式表与布局 / 编排 hook，最大 393）。批次五（v3.0.21）是收尾的 2 个——`AdminModelGatewayPage.tsx`（1251 → 壳 153 + 跨 Tab 编排 hook / 两个 Tab 控制器 / 四个面板 / 弹窗族 / 金额与计数纯函数，最大 369）、`AgentHomePage.tsx`（675 → 壳 257 + 样式表 / 模板卡片 / 创建编排，最大 344）。`memory/hindsight` 的两套图谱属已裁定的刻意分叉，批次四只拆文件内部职责、并收敛了因果族链接类型这一处外围重复（图形逻辑零交叉，见本节末条）。`WorkflowEditor.tsx` 与 `useWorkflowRun.ts` 的取数 effect 已按「先抽状态归属、再拆传输适配」拆开，但**仍保留手写**（判断与理由见 §3.6 末条）。
- **§4.2 / §4.3 的「Dialog 关闭清理状态 / 表单重置走 `key` / 不手写 `useState` 校验」扫描（2026-09-23，第 17 轮）**：扫描口径与命中见文件头 v3.0.26。**本轮已修 5 个生产文件**（改完即合规，不留在本节）：`model-management/…/provider-editor-dialog.tsx`、`model-management/…/agent-models-dialogs.tsx`（`ModelEditorDialog`）、`knowledge/…/add-embedding-provider-dialog.tsx`、`channel/…/AgentChannelsPage.tsx`（+ 字段体 `channel/…/components/ChannelBindingForm.tsx`）、`agent-config/…/AgentSitesPage.tsx`（+ 字段体 `agent-config/…/components/AgentSiteForm.tsx`）。**仍未收口的 8 处（按影响面排序，逐条给位置与为什么这轮不动）**：
  - ① `platform/identity/web/components/ChangePasswordDialog.tsx:63`（`const [error, setError] = useState("")`）与 `:80-87`（`newPassword.length < 8` / `newPassword !== confirmPassword` 两条手写规则）——**§4.3 的教科书命中，也是影响面最大的一处**（登录态的密码弹窗）。不动的原因是「改完没有回归网兜」而非「不值得改」：它现在用**一个** `error` 字符串同时承担两条规则校验与服务端 `submitError.message` 回显，换成 `FormDialog` + `formConfig` 需要新增「当前密码必填」的字典键（本包有 11 键的基线断言，`identity-i18n.test.ts` 会跟着走）、把 `DialogDescription` 从 `DialogHeader` 移进 children（`FormDialog` 只渲染 `DialogTitle`），而且 `FormDialog` 恒渲染页脚取消按钮——这个弹窗现在**只有提交按钮**，换壳会多出一颗按钮，属于用户可见的交互变化。而 Radix `Dialog` 在本仓 happy-dom 下挂不起来（`@radix-ui/react-focus-scope` 缺 `NodeFilter` / `HTMLInputElement` 等全局，实测直接抛在 commit 阶段），改完只能靠静态审查。要做的话建议单独一轮：先把字段体拆成可脱离弹窗渲染的组件（与本文两处新字段体同形），补上回归网兜之后再换壳。
  - ② `resources/observer/web/pages/admin/AdminPeoplePage.tsx:162-169` —— `UserActionDialog` 的 `submit` 是 `if (!normalizedIdentifier || password.length < 8 || (isCreate && !name.trim())) return;`，**静默 no-op**：用户点提交，界面什么也不发生（提交按钮虽被 `disabled` 兜住，但 `disabled` 的判据与这里逐字重复了两遍，任一处漂移就会漏）。不换 `FormDialog` 的硬约束是它挂在 `DialogContent` 上的 `disableOverlayClose={loading}` / `disableEscapeClose={loading}`——**FormDialog 表达不了这两个开关**，换壳会丢掉「提交中不许关」这条约束（同类缺口见 ③ 与 §4.3 的 `formConfig` 契约）。补法建议：先给 `FormDialog` 定「提交中是否允许关闭」的 props 口径，再连同 ③ 一起收。
  - ③ `resources/prod-view/web/components/prod-view-editor.tsx:152` —— `useProdViewEditor.handleSubmit` 的 `if (!formName.trim()) return;` 与 ② 同形（静默 no-op）。同一处的 `:146-149` `closeDialog` 只清 `editingView`、`:266` 的 `onOpenChange` 也只调 `closeDialog`——三个表单字段靠 `openCreate` / `openEdit` 的「打开时重置」覆盖，可见行为正确，属**潜在**风险（新增入口若忘记重置就会漏）。
  - ④ `knowledge/…/ChunkDetailSheet.tsx:78-86` —— `useEffect([open, fetchChunks])` 里手写 `setPage(1)` / `setKeyword("")` / `setSearchInput("")` / `setTextMode("ellipse")`。这是 §4.3 说的「重置不要写在 effect 里」，但它**依赖数组里有 `fetchChunks`**（`kbId` / `resource.id` / `t` 变化就会换引用），因此语言切换或换知识库时会在 `open` 为真的情况下再跑一次、静默清掉用户当前的翻页与搜索词——这才是这处真正需要修的形态，比「换成 `key`」更优先。改法要先把「Sheet 打开」与「数据源变化」两件事分开（例如把 `open` 作为 `key` 交给外层），不宜就地改成 `key`。
  - ⑤ `sandbox/web/src/pages/admin/components/PoolDialog.tsx:50-58` —— `useEffect([open, pool])` 内手写五条 `set*`。与 ④ 同族但**依赖数组里是 `pool`（稳定对象）**，行为与「打开时重置」等价，无可见缺陷；只在统一口径时一并换成 `key`。
  - ⑥ `agent-config/…/agent-editor/agent-editor-body.tsx:118-122` —— `if (editor.data && initializedEditorKeyRef.current !== editorKey) form.reset(editor.data.initialValues)`，另用 `useEffect([props.open])`（`:123-129`）在关闭时清 `activeSection` / `templateOpen`。这是「重置写在 effect 里」的最大一处，但编辑器的 `useForm` **由页面自己持有**（不经过 `FormDialog`），`key` 的落点是整个编辑器弹窗而不是表单；要动得连同它已有的 `initializedEditorKeyRef` 去重逻辑一起重排，属单独一轮的量级。
  - ⑦ `platform/identity/web/pages/agent-panel/pages/AgentApiKeysPage.tsx:248-251` —— `onOpenChange` 里只清 `newKeyValue`（创建结果），三个表单字段靠 `openCreate` 重置。潜在风险与 ③ 同形。
  - ⑧ `knowledge/…/components/knowledge-base-form-dialog.tsx:64` —— `onOpenChange={props.onOpenChange}` 原样透传，字段是 props 受控（`name` / `description` 都在 `AgentKnowledgeBasesPage` 的 `catalog` 上）。这是「表单状态不在弹窗内部」的另一种形态，换成 `formConfig` 要把 `catalog` 的字段状态整片搬进表单实例——属知识库页面的单独一轮，不与本文两处同批。
- **2 个 config 组件生产零消费**：`DataTable` / `BatchActionBar` 目前只有包内测试与 demo 引用。`EmptyState` 早已不是零消费——2026-09-22 重写为内联状态块后，它已是本仓页面状态块的统一实现：实测 `grep -rln 'from "@fenix/ui-components/config/EmptyState"' packages apps` 命中 **37 个生产文件**（workflow 的 `WorkflowList.tsx` / `WorkflowRuns.tsx` / `WorkflowVersions.tsx`，observer 的 `AdminObserverPage.tsx`（此前的同名本地实现已删除），task 的 `TasksPanel.tsx` / `components/ExecutionLogTable.tsx`，agent-config 的 `pages/agent-sites-catalog.tsx`（空态与失败态，`542772c7`），以及 knowledge / memory / mcp / model-management / prod-view / channel / skill / identity 各包的页面与状态块；原记的 `task/components/TaskLogDialog.tsx` 已不直接消费它，日志弹窗经 `ExecutionLogTable` 间接复用）。收口前先确认包内 API 是否够用（`StatusBadge` 已在 2026-09 泛化后接入 task / workflow / prod-view 三处生产消费方，不再是零消费）。
  **2026-09-23（第 19 轮）核实结论与处置**：`rg -ln "DataTable|BatchActionBar" apps packages ui-sandbox` 的全部命中是——两个组件自身的实现文件、`web/config/index.ts` 与 `web/index.ts` 两处 barrel、`package.json` 的 exports、**demo 页面**（`demo/App.tsx` 经 `demo/sections/data-l1.tsx` / `data-l2.tsx` / `base-ui-p3/data-display.tsx` 渲染，`demo/non-chat-sections-smoke.test.tsx` 冒烟）与 5 份包内用例；`packages/resources/*` 与 `apps/web` 的生产代码**零命中**。**处置：保留，不按死代码删除**——① 它们是 §4.1 表里对外的 `config/` 契约（depth-link 已进 `exports`，`barrel-exports.test.ts` 逐条钉住），删掉等于改公开面；② demo 是包自带的可构建画廊（`bun --cwd packages/ui-components dev/build`），组件在那里是**活的**（不是无人引用的裸文件）；③ 判据与 `EmptyState` 的历次处置一致：零消费的 config 组件在这里的既定出路是「等第二个真实用例落地上移 / 收编」，而不是删除。**移除条件**：确认外部（EE 或下游宿主）不再需要这两个原语时，连同 demo 段落、5 份用例、`web/index.ts` 与 `exports` 的键一次删净；在那之前不要为了「清单上少两行」删它们。
- **本地重复实现（包内同名同形态）：2026-09-23（第 19 轮）全仓复扫与处置**。§11.3 此前记的「2 处」无从对应（该行随文档重写引入，`§4.8` 里没有对应的两条）。本轮按可机械化判据扫了两遍——① 生产 `.tsx` 文件名在全仓是否有重名（实测只剩 `index.tsx`）；② 组件声明（`function X` / `const X =`）在**同一包内**是否出现两次——真实命中**只有 1 处**：workflow 包的 `CollapsibleGroup`（`components/node-config-fields.tsx` 用原生 `<details>`、`components/RunParamsDialog.tsx` 用 `useState` + `ParamGroupHeader`；同名、props 逐字相同、调用形态也相同）。**已收敛**为 `components/CollapsibleGroup.tsx` 一份（标题行复用包内既有的受控件 `ParamGroupHeader`，展开态由容器持有），两个调用点改为 import；视觉差异是节点配置卡的工具分组由原生三角标改为 `▶`（字号 / 字重 / 颜色沿用旧 `summary`）。守卫：`workflow/web/__tests__/collapsible-group-single-source.test.ts`（三例：全包只有一份实现、两个调用点都从共享件引入、`node-config-fields` 不再导出它）。
  另有两处**同名但契约不同**的命中，核实为**不是重复实现，不动**：`workflow/pages/workflow/nodes.tsx` 的 `StatusDot`（画布节点徽标里的**图标槽**：`RUNNING` → 转圈、`COMPLETED` → 对勾，其余渲染半透明白点）与 `ui-components/web/ui/status-dot.tsx`（通用状态圆点，收语义色调）；`sandbox/…/components/InstanceRow.tsx`（沙箱实例行）与 `knowledge/src/…/components/embedding-model-rows.tsx` 的 `InstanceRow`（向量模型实例行）——两组各自的 props、数据来源与渲染都不同，只是名字撞了（纯逻辑模块里的通用名），改名不是本规范的目标。
...
- **刻意分叉（已裁定）：`memory/hindsight/components/` 的两套图谱**——`Graph2d.tsx`（Cytoscape.js）与 `Constellation.tsx`（自绘 canvas）的图形逻辑不做归一，只收敛它们外围的重复（可视化高度读取、暗色判定、加载块）。两者的渲染模型不同，抽公共层只会得到一层薄转发。2026-09-23 批次四复核：外围三项（可视化高度读取 / 暗色判定 / 加载块）在早前批次已收敛，本轮只补收「因果族链接类型名单」一处，并把两套图谱各自的纯逻辑与编排拆成独立模块；两图的绘制、布局、命中测试仍零交叉，共用件只有 `graph-model.ts` 的数据契约与类型名单。
...
| `opId` | 文件写操作幂等 ID，透传 `x-file-op-id`（`docs/arch/12-files.md` §7.2）。`request()` **不自动重试**（重试是调用方策略）；调用方重发同一动作须复用同一 opId，服务端据此去重。宿主 `fs.ts` 目前每次调用新生成 opId，见 §5.9 |
| `bearerToken` | 注入 `Authorization: Bearer`（如系统 Master Key，`docs/arch/21` §5）；属内部头，调用方 `headers` 覆盖不了，要换凭证就换本字段 |
| `headers` | 请求头透传（如 `If-None-Match` 条件请求）；只补内部未设置的头，**同名冲突时内部值胜出** |

> **内部头由基建决定，调用方覆盖不了**（v3.0.24 起为已实现行为）：`content-type` / `x-file-op-id` / `authorization` 三个头由 `request()` 自己写入，调用方 `headers` 只在内部未设置该头时生效（判定用 `Headers.has`，大小写不敏感——`Content-Type` 与 `content-type` 同等对待）。实现上 `headers` 已从 `init` 解构排除，不再经 `...init` 覆盖合并结果。三条行为都有用例钉住（`packages/web-runtime/web/__tests__/request.test.ts`：内部头不被覆盖 / 调用方仍可补未设置的头 / 大小写变体同样不覆盖）。
>
> 组织头 `X-Active-Org-Id` **不在这张清单里**：它不由 `request()` 注入，而由身份包 fetch 拦截器注入（§3.3），且拦截器只在调用方未提供该头时补值——域模块仍然不得自己拼组织头或组织参数。
...
- **超时覆盖 body 消费**（v3.0.24 起为已实现行为）：`timeout` 从发起请求算到响应体读完，不是只到收到响应头——慢 `json()` / `text()` 同样会被超时掐断（否则慢响应会绕过超时约束，页面停在永久 loading）。超时与外部取消都归 `NETWORK_ERROR`，文案分别为「请求超时」与「请求超时或已取消」（后者不可重试的语义由调用方判 `error.code` 决定）。
- **外部 `signal` 不留监听残留**（v3.0.24 起为已实现行为）：合并到调用方 `signal` 上的监听器在请求结束（成功 / 失败 / 取消）后即摘除，调用方可以长期复用同一个 signal 而不会累积监听器。
...
| 二进制 / 文本 / PDF 读 | knowledge `web/api/knowledge-bases.ts`（三处：`fetchResourceFileText` / `fetchResourceFileBinary` / `isResourcePdfPreviewAvailable`） | 裸 `fetch` + 域内 `buildResourceReadError()` 归一为 `ApiError`；第三处是**探测**语义（非 2xx 是正常分支，返回布尔而不抛错） | 能力缺口 |
...
| 文本流下载 | observer `web/api/system-logs.ts`（`systemLogsApi.download`） | 裸 `fetch` + `buildDownloadError()`；成功只回 `Blob`，建锚点与 `revokeObjectURL` 归调用方 | 能力缺口 |
...
| 文件预览器读源文件（文本 Blob / HTML「源码」页正文） | 宿主 `api/fs.ts`（`readPreviewSource` + `buildPreviewSourceUrl`） | 裸 `fetch` + **返回原始 `Response` 由调用方消费**（与 sandbox `executeCommand` 同一形态）；预览 URL 也归本模块拼（`.../fs/<path>?preview=true`），宿主经 `PreviewTab` / `FileViewerPreview` 的 `fetchPreview`、`buildPreviewUrl` 注入，包内不再自带宿主路由、也不留全局 `fetch` 兜底（2026-09-23 收口） | 能力缺口 |
| 本地 blob / 预览源读取 | `ui-components/web/chat/primitives/internal/prompt-input-file.ts` | 读的是本地 blob URL，**不是后端调用**，不属本节管辖（2026-09-23 复扫修正：此前同行还列着 `components/preview/html-plugin.ts`，那个读的是后端路由，已按上一行收口） | 不适用 |

**新增例外点的登记要求**：在提交里写明命中点、为何不能走 `request()`、失败归一函数、成功返回值形状，以及能力补齐后的回退动作。

**不在登记范围的两类 `fetch`**：① 统一层实现本身（`web-runtime/web/api/request.ts` 的那一次 `fetch` 是全表 `request()` 行的底座，不是调用点）；② 本地 blob URL 读取（上一行）。2026-09-23 全仓复扫的口径与结果见 §11.3 的「组件中裸调 `fetch()`」一行。
...
- **两处"失败被吞掉"的已知残留**（§5.8 判据命中，补法需改对外契约或属后台路径，2026-09-22 全量复核时未动）：① `platform/identity/web/contexts/OrgContext.tsx` 的 `refreshOrgs` 失败只 `console.error`——要补持久失败态必须扩 `OrgContextValue`，受影响的是宿主侧组织切换入口（**本轮更新**：原先连带记的「组织页会按『无组织』渲染、落到 `noOrgs` 空态」已由第 16 轮收口——`AgentOrganizationsPage` 自己的 `orgApi.list()` 失败现在渲染页面级失败块 + 重试，见 §3.6；本条的上下文缺失败态仍在）；② `agent-config/web/components/agent-panel/SiteFrame.tsx` 的挂载二维码在后台预生成，失败只留 `console.error`，分享弹层会停在永久 spinner（补法建议在弹层内落"最近一次生成失败"静态态，而不是 toast——弹层可能在失败之后才被打开）。
...
- **opId 的"重试复用"在宿主侧尚不成立**（§5.2）：`request()` 不自动重试（重试是调用方策略），而宿主 `api/fs.ts` 的写操作在**每次调用**里新生成 `opId`（`randomUUID()`）——调用方重发同一动作会拿到新键，服务端无法据此幂等去重。要兑现 `docs/arch/12-files.md` §7.2 的幂等承诺，需把 opId 的生成时机上移到"用户动作"层（或让域模块接受调用方传入 opId）；本轮只处理 `request()` 层的头/超时/监听器与模块命名，未动生成时机。
...
| 同源文件预览（pdf / office 转 pdf） | 可用 `srcDoc` 或同源 URL，`sandbox` 可选 | **三处无 `sandbox` 属性**（同源，风险低，2026-09-23 复测）：knowledge 的 pdf 与 office 转 pdf（`ResourcePreviewContent.tsx` 两处）、工作区预览器的原生 PDF（`ui-components/web/components/preview/native-pdf-plugin.ts`）。**但知识库的 HTML 预览不算这一类**——它是用户上传的 UGC，`srcDoc` 且 `sandbox="allow-scripts"`（2026-09-23 去掉 `allow-same-origin`：`srcdoc` 继承父页面源，与 `allow-scripts` 组合即可自行摘掉 sandbox）；工作区预览器的 HTML 预览（`html-plugin.ts`）同样是 UGC 且同源，固定 `sandbox="allow-scripts"` + `referrerpolicy="no-referrer"` |

新增 iframe 时必须显式写出 `sandbox` 属性并说明取值理由。

**外链的 `rel`**（2026-09-23 复扫，本轮未改）：全仓 6 处 `target="_blank"` **全部带 `rel`**——`message.tsx` 两处（Markdown 链接与媒体链接）为 `noopener noreferrer`，knowledge 预览与下载两处、站点目录、模型网关管理台各一处为 `noreferrer`。规范未单列该条，故按现状记录，不构成偏离。
...
| 用途 | 键 / 位置 |
|------|-----------|
| UI 维度与布局偏好 | `ChatArea`（`fenix:artifacts-width` / `fenix:artifacts-layout`）、`artifacts-files-workspace`（`fenix:file-tree-width`） |
| 面板开合状态 | `acp-sidebar-open`（chat panel，`chat-panel-ports.tsx`） |
| 侧栏折叠状态 | `agent-panel:sidebar-collapsed`（`AgentSidebar`；`AgentSidebarTree` 同属该折叠态） |
| 语言 | `rcs-lang`（i18n 检测器托管） |
| 主题 | 无（全局强制亮色，见 §3.2；`theme` 键已不再被读写） |
| 登录页偏好 | `apps/web/src/lib/auth-preference.ts`（`auth-preferred-method`） |
| 匿名 UUID | `apps/web/src/api/helpers.ts`（键 `rcs_uuid`，当前零消费） |

表里是**全量**（2026-09-23 复扫：除 `active_org_id` 与 master key 外，生产代码只碰上面这些键；原表里的 `wf-editor:chat-open` 已随 workflow 编辑器改造消失，属过期登记，本轮删除）。**组织 id 不在"便利"的范畴**：它决定请求归属哪租户，只允许经上下文读取（见 §3.3）——读点只有契约模块 `@fenix/web-runtime/lib/active-org`，写点只有身份的 `OrgContext.tsx`（`switchOrg` / `refreshOrgs` 的乐观写与回滚），两侧都有静态守卫钉住（`apps/web/src/__tests__/file-events-org-contract.test.ts`，2026-09-23 补上写入侧）。
...
**2026-09-23 的第 18 轮（安全域仓库级复扫，口径 `apps/web/**` + `packages/**/web/**`）**：按 §5.8、§6.1–§6.4 逐条复测，**新增违规为零**——`dangerouslySetInnerHTML` 仍是那 3 处（全部经 `sanitize-html.ts` 的显式白名单）、`iframe` 的 `allow-same-origin` 未回流（`IframePreview` / knowledge HTML 预览 / `html-plugin` 三处 UGC 面取值与 R2 收口时一致）、原生 `confirm()` / `alert()` / `prompt()` 仍是零命中、`localStorage` / `sessionStorage` 里除已登记的 master key 外没有任何凭据。本轮修掉的两处写在本节末尾（「预览器把后端读取握在组件包里」「工作流运行参数整份落控制台」）。

- ~~**3 处 `dangerouslySetInnerHTML` 全部经 DOMPurify，但都用默认配置**~~ **已收紧**：抽出 `knowledge/web/lib/sanitize-html.ts` 的两份显式白名单（`sanitizeHighlightHtml` / `sanitizeRichHtml`），三处调用点（`components/knowledge/ResourcePreviewContent.tsx` 的 docx 分支、`src/pages/agent-panel/components/ChunkDetailSheet.tsx` 的切片详情、`src/pages/agent-panel/components/retrieval-chunk-card.tsx` 的检索高亮——最后一个随 §4.7 拆分从 `RetrievalTestPanel.tsx` 移到单条结果卡片，文件名已于第 20 轮改准）改走 helper，`knowledge/web/__tests__/sanitize-html.test.ts` 钉住白名单内容与接线。**保留的取舍**：`class` 仍在两份白名单里（RAGFlow / mammoth 用它表达表格与段落样式），因此不阻断 Tailwind 类名注入造成的 UI 伪装；要挡需另行裁定。
...
- ~~**预览器把后端读取握在组件包里**~~ **已收口（2026-09-23）**：`ui-components` 的 `FileViewerPreview` 过去用全局 `fetch` 读宿主文件代理路由（`.../fs/<path>?preview=true`）、`html-plugin` 与 `preview-source` 各有一份 `fetch` 兜底（后者还以 `= fetch` 作默认参数），而 `…/fs/<path>?preview=true` 这条路由字面量写在包内的 `defaultBuildPreviewUrl` 里——即 §5.8 的「组件中裸调 `fetch`」与「组件中拼装后端 URL」两条同时成立。现改为宿主注入：域模块 `apps/web/src/api/fs.ts` 新增 `buildPreviewSourceUrl` + `readPreviewSource`，经 `artifacts-files-workspace.tsx` → `PreviewTab` → `FileViewerPreview` 的 `buildPreviewUrl` / `fetchPreview` 传入（`fetchPreview` 必填、包内无兜底，忘了注入直接编译不过）；`htmlPreviewPlugin(fetchPreview)` 与 `loadByteAccuratePreviewSource(url, fetchPreview)` 同批改为必填参数。包内默认构建器与 demo 的 `data:` URL 取数保留，仅服务 demo。守卫：包内 `preview-fetch-injection.test.ts`（注入生效 + 源码无兜底形态）与宿主 `preview-read-injection.test.ts`（宿主两处注入齐备 + 全宿主只有 `api/fs.ts` 拼该 URL）。§5.3 的登记同步拆行（原「本地 blob 读取」一行把它与真正的本地 blob 读取混在一起，描述失准）。
- ~~**工作流运行参数整份落控制台**~~ **已删（2026-09-23）**：`use-workflow-run-params.ts` 的 `onRunClick` 曾 `console.log("[RunButton] meta.params:", JSON.stringify(meta.params), …)`——点一次运行按钮就把用户自定义的运行参数（可能含密钥类默认值）整份打进控制台，且 payload 无界、与「有没有参数」的判定无关（判定只需 `hasParams`）。已删除并留注释说明排查入口不在用户数据上（CLAUDE.md 原则 4「敏感信息不得进入日志」）。**同批复扫结论**：其余 `console.log` 都不含凭据——`prompt-input-file.ts` 的 4 条只打长度/大小/类型，`use-cytoscape-graph.ts` 的打图谱初始化过程（含 2 条样例元素，属用户数据但非凭据，保留），`use-workflow-editor-events.ts` 的打 Context Queue 且**由 Ctrl/Cmd+Shift+D 显式触发**（刻意的调试入口）；`request.ts` 记的是完整 URL（含 query），全仓没有任何调用方把凭据放进 query（凭据只走 `Authorization` 头），故不脱敏。
...
| 通道 | 路径 | 建连点 | 鉴权 | 重连 |
|------|------|--------|------|------|
| Chat 状态同步（Yjs） | `ws(s)://<host>/acp/yjs/:agentId` | `packages/agent-runtime/web/yjs/yjs-ws.ts` 的 `buildYjsUrl` → `@fenix/chat-channel` 的 `createYjsWsClient` | 会话 cookie + query `active_org_id`（经 §3.3 的组织契约读取）；无 `Authorization` 头 | 指数退避，终态码不重连（§8.3） |
| 文件树事件 | `ws(s)://<host>/web/file-events` | 宿主域模块 `apps/web/src/api/file-events.ts` 的 `openFileEventsConnection`（URL/订阅帧/帧解析都在此）；组件 `use-file-tree-events.ts` 只消费入口 | 同上 | 组件内自持（退避、可见性与去抖是调用方策略） |
...
**不得在组件里拼装后端 URL（含 WebSocket URL）**：两条 WS 通道的 URL 分别在宿主域模块 `apps/web/src/api/file-events.ts` 与 `agent-runtime/web/yjs/yjs-ws.ts` 里拼装，组件只消费域模块入口。`apps/web/src/__tests__/file-events-org-contract.test.ts` 静态守着这条与「组织 id 只经契约读取」：组件/页面目录下不得出现 `new WebSocket(` / `new EventSource(`，全仓前端源码里读 `active_org_id` 的只允许是契约模块一处。
...
**背压分两端，两端的阈值都是 64 KB（字节）**：

- **服务端（接收侧/广播侧）**：某连接的发送积压超过 64 KB 时跳过该次发送，并在缓冲回落后**主动定向补发全量快照**（不需要客户端重连——`broadcaster.ts` 的注释明确写了不再依赖"下次重连"，属静默恢复）；持续滞后超过 30s 才 `close(1013, "slow consumer resync timeout")` 交回客户端重连路径。
- **前端（发送侧，§8.6 补齐）**：`chat-channel/src/transport/ws.ts` 的 `send()` 在 `bufferedAmount > 64 KB`（`SEND_BACKPRESSURE_THRESHOLD_BYTES`，单位仍是字节，与服务端同值）时**拒绝本次发送并返回 `false`**，不再把帧排进无界缓冲。**只拦排队**：不 `close`、不改连接状态、不碰 `WS_CLOSE_CODE_POLICY`（§8.3 的策略不变），失败交回调用方——`use-chat-panel-runtime.ts` 的 `sendViaWs` 把 `false` 走既有 `toast.error(wsSendFailed)`，心跳 `keep_alive` 被拒无副作用（§8.2 服务端不因客户端心跳超时关闭连接）。适配器不提供 `bufferedAmount` 时视为 0（未拥塞）。
...
7. **会话切换与内容清理走"换代"**：由 `DocManager.replaceProjection` 完成（`session-channel.ts` 调用）。`chat-writer.ts` 的 `clearSessionDocContent`（及其专用的 `clearPeriTaskViews`）**已删除**，不要恢复——`docs/arch/19-yjs-chat-streaming.md` §4.2 明确禁止回到旧的清空流程；也不要用 destroy + recreate 制造异步竞态。`create_session` 同样走换代，不沿用旧 Session Doc 的内容。
...
> 实时通道批次（v3.0.23，2026-09-23）已收口本节原有条目：组织 id 直读收敛到 `@fenix/web-runtime/lib/active-org` 契约（§3.3）、`/web/file-events` 的 URL 与帧解析下沉到宿主域模块 `apps/web/src/api/file-events.ts`、前端补 64 KB 发送背压（§8.4）、`clearSessionDocContent` 及其生产引用链删除（§8.5 第 7 条），`CLAUDE.md` 的 YJS/Chat 第 7 条已同批对齐。落点与取舍见文档头 v3.0.23 条目；本节只保留仍未修的条目。
...
- **硬编码用户可见文案**：2026-09-23 的前两批收口把 `agent-config/AgentSitesCard.tsx`（6 处）、`knowledge/ChunkDetailSheet.tsx`、`task/TasksPanel.tsx`、`task/TaskForm.tsx`（3 处）、`ui-components/FileViewerPreview.tsx`（内置预览文案 + 错误边界提示）与 `agent-config/SiteFrame.tsx`（2 条 toast）的文案改走各包字典。**第 19 轮（v3.0.28）再收五处前批报出但当时判为超范围的命中**：`task/agent-tasks-utils.ts` 的 zod 与 cron 校验文案（20 处中文 → `error.*` 键，`validateCronExpression` 改为返回键、schema 的 `message` 只装键，译在 `TaskForm` / `CronEditor` 的渲染边界取）、`task/components/CronEditor.tsx` 的 `describeCron` 四条动态模板与时段 / 星期字形（15 处 → `cron.describe.*`）、`knowledge/…/RetrievalTestPanel.tsx` 的 10 个语言名（→ `retrieval.languages.*`）、`knowledge` 的导入 / 覆盖弹窗与两个域 hook（`importDialog.*` / `confirm.overwrite*` / `toast.imported` / `toast.remoteDeleted`，并删掉 `t(key) || "中文"` 这行永不命中的死兜底）、`ui-components` 的预览失败态（`preview-source.ts` 改抛结构化的 `PreviewSourceError`，失败文案由 `FileViewerPreview` 按当前语言取 `fileTree.preview.loadFailed*`，原始错误只进 console）。
  本轮全仓复扫的口径与实测：**字符串字面量 + JSX 文本中的中文，排除注释、测试与字典文件**（`apps/web/src` + `packages/**/web/**`，排除 `__tests__`、`routeTree.gen.ts`、`/src/server/`、`/src/routes/web/`）——修复前 **554** 处 / 62 个文件，修复后 **500** 处 / 54 个文件。可复现的等价命令（口径略宽：**注释行**里带引号的中文串会被算进来，实测修复前 546 / 修复后 504；上面的 554 → 500 是用 TS AST 只取字符串字面量与 JSX 文本后的数。**第 20 轮终检再跑同一条命令**：R19 交付态仍是 504（未回流），本轮新增 1 条 rerank 诊断日志串后为 505）：

```bash
( find apps/web/src -name '*.ts' -o -name '*.tsx'; find packages -path '*/web/*' \( -name '*.ts' -o -name '*.tsx' \) ) \
  | grep -v -e __tests__ -e routeTree.gen -e node_modules -e '/src/server/' -e '/src/routes/web/' \
  | xargs rg -n --no-heading -e '"[^"]*\p{Han}' -e '`[^`]*\p{Han}' \
  | awk -F: '{rest=$0; sub(/^[^:]*:[0-9]+:/,"",rest); if (rest ~ /^[ \t]*(\/\/|\*|\/\*)/) next; print}' | wc -l
```

  **未修的按位置登记**（同一口径逐条判读，修完的已从下表删掉）：
  - **`model-management` 的两个目录页共 301 处**（`web/src/pages/agent-panel/pages/AlgorithmsPage.tsx` 228、`web/pages/agent-panel/pages/VerticalModelsPage.tsx` 73）：算法目录与垂直模型目录的整份领域数据（名称 / 描述 / 分类 / 场景 / 参数说明）都写死在模块里，两页都是**生产路由页面**（`apps/web/src/routes/agent/_panel/algorithms.tsx`、`vertical-models.tsx`）。这是本轮剩下的最大一块，未动的理由：约 300 条词条要连英文译文一起落两份字典，属一次独立的迁移（改法与 `agent-config` 的目录数据同形），不是收口批次能顺带做的量级。移除条件：两页的数据表改为从字典按 id 取（`models.algorithms.<id>.*` / `models.vertical.<id>.*`）。
  - **`ui-components` 的 chat 演示数据 96 处**（`web/chat/mocks/mock-fixtures.ts` 31、`web/chat/mocks/internal/mock-conversation.ts` 42、`web/chat/mocks/internal/mock-stream-script.ts` 21、`mock-reducer.ts` 2）：包内 chat 演示的假会话（用户提问、待办、子 Agent 结论）。它们是 demo / 用例的样本数据而非生产渲染路径，价值在于「像真的会话」——不入字典，留着。
  - **`ui-components` 的 chat 叙述层与少量默认值 30 处**：`web/chat/narrators/*.ts` 的 `verb`（「打开文件」「搜索网页」等 14 处）与兜底串 4 处（`helpers.ts` 的「文件」/「未知错误」×2、`task.ts` 的「子任务」）是**纯函数模块**（§9.3 不许 import UI i18n，与 `preview-source` 同性质）；`web/chat/timeline/HindsightToolCard.tsx` 6 处、`web/chat/shell/ChatHeader.tsx` 2 处（`重命名失败: ${err.message}`——顺带把原始 message 拼进了界面文案）、`web/chat/shell/internal/chat-header-session-row.tsx` 的 `aria-label="取消"`、`web/chat/composer/useDragUpload.ts` 2 处（上传失败 / 超限提示）、`web/chat/view/CitationLink.tsx` 的 `title` 默认值各属同一类。移除条件：叙述层收 `t` 参数（`narrate(tool, t)`）、组件默认值改由调用方注入。
  - **`ui-components` 的预览插件 7 处**（`web/components/preview/html-plugin.ts` 5、`native-pdf-plugin.ts` 2）：插件直接操作 DOM、不在 React 树内，**已登记**在该包 README「已知限制」第 12 条（本轮把 `native-pdf-plugin` 并入同一条；`preview-source` 那条随本轮修复删掉）。移除条件同该条：插件工厂接受文案参数。
  - **`agent-config` 的兜底文案 12 处**（`web/pages/agent-panel/pages/AgentManagementPage.tsx` 8：`t("categories.general", { defaultValue: "通用助理" })` 这类；`AgentHomePage.tsx` 3：默认提示模板 `defaults="<strong>你好，</strong>…"` 与两处 `defaultValue`；`web/components/agent-panel/SiteFrame.tsx` 的 `title={\`创建者: ${…}\`}`）：`defaultValue` 只在字典缺键时显示（键都在），属「兜底串写死中文」；默认提示模板与 `title` 是直接渲染的用户可见文案。移除条件：删掉 `defaultValue` 兜底（靠 i18n 测试保证键齐）、模板与 `title` 改走 `t()`。
  - **不是用户可见文案、且不属违规的两类**（列出以免下次复扫重新判一遍）：**开发诊断串 28 处**（`console.warn/error` 与错误边界的 `onError`——§9.3 明示 `console.log` 不受 i18n 限制）；**域模块内部错误消息 26 处**（`web-runtime/api/request.ts` 的 `请求失败 (500)` / `网络异常，请检查连接`、宿主 `api/fs.ts`、knowledge / skill / observer 的 api 归一分支、`shell-navigation.ts` 与 `org-session.tsx` 的开发者错误、identity `auth-client.ts` 的 `注册失败`）——界面一律按稳定 error code 取字典文案、**不展示 raw message**（§9.3），这些串只进 `console` 与 `ApiError.message`。
...
- [ ] **`bun run precheck` 通过**（17 步，见 §11.2）
- [ ] **前端改动额外跑 `bun run build:web`**（后端从 `apps/web/dist/` 挂载静态资源，类型检查通过 ≠ 构建通过）
- [ ] 前端用例已随 `precheck` 跑过（第 15–17 步：`apps/server/src/__tests__/` + `scripts/__tests__/` + `platform-sdk`、`packages/`、`apps/web/src/__tests__/`）——迭代时可先单独跑 `bun test packages/<该包>/web/__tests__/`，交付前仍以 `precheck` 的整批为准
...
各包的 `web/__tests__/*-browser-surface.test.ts`（当前 14 份，全在 `packages/resources/*`；`chat-channel` 的对应文件在 `src/__tests__/`）静态走根出口的**值导入图**，断言图中不出现服务端模块与 `node:*`。**新增包外运行时依赖时必须同步登记到该测试的白名单**——否则守卫会在别人的改动里失败。
...
| 规则 | 现状 |
|------|------|
| 组件中裸调 `fetch()` / `XMLHttpRequest` | 未自动化；现有例外点是登记制（§5.3）。2026-09-23 全仓复扫（`apps/web/**` + `packages/**/web/**`，口径 `\bfetch(` / `new XMLHttpRequest` / `new WebSocket` / `new EventSource`）：命中 **12 个文件**，逐个处置为「**9 处登记准确**（宿主 `fs.ts` / `file-events.ts`，identity `auth-client.ts`，knowledge `knowledge-bases.ts`，observer `system-logs.ts`，sandbox `system-sandbox.ts`，skill `skills.ts`，workflow `workflow-sse.ts`，`prompt-input-file.ts` 的本地 blob 读取）／**1 处仅注释提及**（`prompt-input-context.tsx`）／**1 处是统一层底座**（`request.ts`，不是调用点，不登记）／**1 处登记失准**」。失准的那处是 `ui-components` 的 `html-plugin.ts`：登记写的是「本地 blob 读取，不是后端调用」，实际读的是**后端文件代理路由**，且组件包内直调全局 `fetch` 并自带一份路由字面量，已下沉到宿主 `api/fs.ts`（`readPreviewSource` / `buildPreviewSourceUrl`）并重写 §5.3 登记（见 §6.5）。**口径之外还有 2 处**：`fetch` 被当值传递（`preview-source.ts` 的 `= fetch` 默认参数、`FileViewerPreview.tsx` 的 `, fetch,`）——形态 grep 扫不到，本轮正是这样漏的，已同批收口并新增用例（`ui-components/web/__tests__/preview-fetch-injection.test.ts` 同时钉住这两种形态）。**第 20 轮按同一口径复扫**：`apps/web/src` + `packages/**/web/**` 的生产代码命中 **11 个文件**——即上列 9 处登记 + 统一层底座 `request.ts` + 仅注释提及的 `prompt-input-context.tsx`，`html-plugin.ts` 未回流。口径之外（不在本规范管辖）另行列出以免下次重判：服务端与运行时包（`plugin-{ccb,opencode,peri}` 的 `createWebSocket`、`acp-link`、`chat-channel/src/transport/ws.ts`、`workflow-engine` 的 API 节点执行器、`acp-runtime-cli`）与包内 demo（`ui-components/demo/sections/preview-l2.tsx` 传给预览组件的 `demoFetchPreview`） |
| 直接 `await` 域模块不解包 | 未自动化，签名层面无法区分 |
| `window.location` 写操作 | 无**全仓**门禁（2026-09-23 复扫：生产代码仍零命中；`workflow/web/__tests__/workflow-page-route.test.ts` 只守 workflow 页面，`ui-components/web/testing.ts` 里的一处属测试工具） |
| `dangerouslySetInnerHTML` 不经清洗 | 未自动化。2026-09-23 复扫：**仍是 3 处**（knowledge 的 docx / 切片 / 检索高亮）全部经 `web/lib/sanitize-html.ts` 的显式白名单，无一处用 DOMPurify 默认配置；白名单内容与三处接线由 `knowledge/web/__tests__/sanitize-html.test.ts` 钉住 |
| `localStorage` 读写组织身份 | 未自动化。2026-09-23 复扫：**直读点 0、直写点 0**——读只有契约模块 `@fenix/web-runtime/lib/active-org`，写只有身份的 `OrgContext.tsx`（均经键常量，无字面量），两侧各有一用例钉住（`apps/web/src/__tests__/file-events-org-contract.test.ts`） |
| 原生 `confirm()` / `alert()` / `prompt()` | 未自动化。2026-09-23 复扫：全仓仍**零命中**（4 处已迁 `ConfirmDialog`）；新增只能靠 review |
| 凭据写入 `localStorage` / `sessionStorage` | 未自动化（§6.3 的硬红线）。2026-09-23 复扫：生产代码写过的键只有 UI 偏好 / 布局 / `rcs-lang` / `active_org_id`，**唯一凭据是已登记的 master key**（`rcs_admin_master_key`，`web/lib/admin-key.ts`，写入点只有 `use-admin-key-gate` 的 `unlock`）；全仓无 `document.cookie` 读写、无 `indexedDB` |
| 外链 `target="_blank"` 的 `rel` | 未自动化（规范未单列该条）。2026-09-23 复扫：全仓 6 处**全带 `rel`**（Markdown 两处 `noopener noreferrer`，其余四处 `noreferrer`）；按现状记录，不构成偏离 |
| `console.error` 缺配对用户可见反馈 | 未自动化，判据见 §5.8；2026-09-22 全量复核后仍有已知残留（见 §5.9）。**错误边界是已豁免的一类**（§7.1：降级 UI 本身就是反馈） |
| 日志与提示里的敏感信息 | 未自动化。2026-09-23 复扫：`console.*` 不含 token / key / 密码；唯一整份打印用户数据的是工作流运行参数的调试日志，**已删**（见 §6.5）。`request.ts` 记完整 URL（含 query），全仓无调用方把凭据放进 query（凭据只走 `Authorization` 头），故不脱敏 |
...
| iframe 的 `sandbox` 取值 | 未自动化。2026-09-23 复扫现状（与 R2 收口时一致，无回流）：`IframePreview`（Markdown / Agent 输出）为 `allow-scripts allow-popups` 且 `src` 过协议白名单、`sandbox` 不可被 props 覆盖，有包内用例；`SiteFrame` / `AgentSitesCard`（用户自己的站点）保留 `allow-same-origin` 但带 `referrerPolicy`；UGC 的两处 HTML 预览（knowledge 的 `srcDoc`、工作区预览器的 `html-plugin`）都是 `allow-scripts` + 无 `allow-same-origin`；同源无 `sandbox` 的三处（knowledge 的 pdf 与 office 转 pdf、`native-pdf-plugin`）风险低，逐处位置见 §6.2 的表 |
| 单文件 500 行上限 | 未自动化（`precheck` 不管这一条）。前端生产文件已于 2026-09-23 **清零**：超限 0 个、最大 496（复测命令与口径见 §4.8）；新增模块仍靠 review 对照 |
| 组件重复开发检测 | 未自动化。2026-09-23（第 19 轮）核实：原记的「2 处本地重复实现」是文档重写带进来的旧数（§4.8 无对应条目）。按「生产 `.tsx` 文件名重名」+「同一包内组件声明重名」两个机械化判据复扫，真实命中 **1 处**（workflow 的 `CollapsibleGroup` 两份实现），**已收敛为一份**并由 `workflow/web/__tests__/collapsible-group-single-source.test.ts` 钉住；另 2 处同名但契约不同（`nodes.tsx` 的 `StatusDot` 图标槽 vs `ui-components/ui/status-dot` 通用圆点、sandbox 与 knowledge 各自的 `InstanceRow`）核实为不属重复实现（逐条见 §4.8）。**跨包**的同一形态重复仍无门禁——判据是「第二个包开始消费就该下沉」（§4.1），只能靠 review |
