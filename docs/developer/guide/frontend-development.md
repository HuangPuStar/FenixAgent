...
> **版本**：v3.0.17 | **最后更新**：2026-09-23 | **维护者**：前端团队
>
> **最近变更**：
...
- **10 个生产文件超 500 行**（2026-09-23 §4.8 批次一收口后实测，从大到小）：`model-management/AdminModelGatewayPage.tsx` 1251、`knowledge/AgentKnowledgeBasesPage.tsx` 1243、`workflow/components/NodeConfigCard.tsx` 1180、`workflow/WorkflowEditor.tsx` 1077、`memory/hindsight/components/Constellation.tsx` 978、`memory/hindsight/components/DataView.tsx` 955、`memory/hindsight/components/Graph2d.tsx` 713、`agent-config/AgentHomePage.tsx` 675、`knowledge/ResourcePreviewContent.tsx` 570、`workflow/hooks/useWorkflowRun.ts` 569。400–499 行区间另有 28 个（口径：`apps/web/src` + `packages/**/web/**`，排除测试、生成文件与服务端路径，行数是文件总行数；统计命令：`(find apps/web/src -name '*.ts' -o -name '*.tsx' | grep -v __tests__ | grep -v routeTree.gen; find packages -path '*/web/*' \( -name '*.ts' -o -name '*.tsx' \) | grep -v __tests__ | grep -v '/src/server/' | grep -v '/src/routes/web/') | xargs wc -l | awk '$1>500'`）。本轮（v3.0.17）消掉的是 500–560 行带那 10 个：9 个按职责拆分、1 个（`workflow/components/NodeConfigPanel.tsx`，557 行）经核实全仓零引用且文件头已标 `@deprecated`，按 CLAUDE.md 原则 10 直接删除——不把死代码拆成多个死模块。
