...
> **版本**：v3.0.28 | **最后更新**：2026-09-23 | **维护者**：前端团队
>
> **最近变更**：
...
- **硬编码用户可见文案**：2026-09-23 的前两批收口把 `agent-config/AgentSitesCard.tsx`（6 处）、`knowledge/ChunkDetailSheet.tsx`、`task/TasksPanel.tsx`、`task/TaskForm.tsx`（3 处）、`ui-components/FileViewerPreview.tsx`（内置预览文案 + 错误边界提示）与 `agent-config/SiteFrame.tsx`（2 条 toast）的文案改走各包字典。**第 19 轮（v3.0.28）再收五处前批报出但当时判为超范围的命中**：`task/agent-tasks-utils.ts` 的 zod 与 cron 校验文案（20 处中文 → `error.*` 键，`validateCronExpression` 改为返回键、schema 的 `message` 只装键，译在 `TaskForm` / `CronEditor` 的渲染边界取）、`task/components/CronEditor.tsx` 的 `describeCron` 四条动态模板与时段 / 星期字形（15 处 → `cron.describe.*`）、`knowledge/…/RetrievalTestPanel.tsx` 的 10 个语言名（→ `retrieval.languages.*`）、`knowledge` 的导入 / 覆盖弹窗与两个域 hook（`importDialog.*` / `confirm.overwrite*` / `toast.imported` / `toast.remoteDeleted`，并删掉 `t(key) || "中文"` 这行永不命中的死兜底）、`ui-components` 的预览失败态（`preview-source.ts` 改抛结构化的 `PreviewSourceError`，失败文案由 `FileViewerPreview` 按当前语言取 `fileTree.preview.loadFailed*`，原始错误只进 console）。
  本轮全仓复扫的口径与实测：**字符串字面量 + JSX 文本中的中文，排除注释、测试与字典文件**（`apps/web/src` + `packages/**/web/**`，排除 `__tests__`、`routeTree.gen.ts`、`/src/server/`、`/src/routes/web/`）——修复前 **554** 处 / 62 个文件，修复后 **500** 处 / 54 个文件。可复现的等价命令（口径略宽：**注释行**里带引号的中文串会被算进来，实测修复前 546 / 修复后 504；上面的 554 → 500 是用 TS AST 只取字符串字面量与 JSX 文本后的数）：
