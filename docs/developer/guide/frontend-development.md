...
> **版本**：v3.0.15 | **最后更新**：2026-09-23 | **维护者**：前端团队
>
> **最近变更**：
...
- **域模块路径仍回显服务端 `err.message`**（**批次一已收口三个范围，剩余 10 个包**）：宿主 `apps/web/src`、`resources/task`、`resources/agent-config` 已全部改为「按字典取文案」（23 个调用点，v3.0.15）；**剩余 83 个调用点 / 28 个文件**分布在 workflow(28)、knowledge(17)、mcp(8)、sandbox(7)、skill(6)、identity(4)、memory(4)、prod-view(3)、model-management(2)、observer(2)、plugin-market(2)。实测两个口径（2026-09-23）：
  - **单行口径**（旧快照的「32 个文件、102 处」就是它）：`grep -rn "toast\.error" apps/web/src packages/*/*/web packages/*/web --include=*.ts --include=*.tsx | grep -c "\.message"` → **102 行 / 34 文件 → 80 行 / 28 文件**。它只认「`toast.error` 与 `.message` 同行」，跨行调用与经 `t()` 插值回显都不计。
  - **括号配对口径**（从每个 `toast.*(` 起做括号配对，取整段实参再判是否含 `.message`）：**106 个调用点 / 34 文件 → 83 / 28**，其中 18 处是模板字符串形态。本轮两个口径在三个范围内都是 **0**。
...
- **错误文案回显**：Chat 域按稳定 `error.type` 映射字典（`public-error-text.ts` + 协议侧明确"不得使用原始异常文本"，并有 `public-error-i18n.test.ts` 守护）；域模块与页面的失败提示原普遍直接显示 `err.message`，其中宿主 `apps/web/src` 与 `resources/{task,agent-config}` 已于 v3.0.15 改为只上屏本包字典文案（§5.9 批次一，判定「Chat 的 `publicErrorText` 不可提炼为通用能力」的理由也在该条），其余 10 个包仍待收口（清单与实测口径见 §5.9）。
