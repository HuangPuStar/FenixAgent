...
| 可观测性与系统诊断 | 系统日志查询与清理；Observer 采集/关系图；ACP 链接观察；构建和健康信息；Chat Error ID 与诊断关联 | `src/services/system-log-service.ts`、`src/services/observer/`、`src/services/build-info.ts`、`src/routes/api/system-logs.ts`、`src/routes/api/system-observer.ts`、`docs/arch/23-chat-error-diagnostics.md` | 观察 Agent、连接、平台运行状态 | `apps/server` 系统诊断服务；结构化日志复用 `@fenix/logger` 与 `requestId`，不设独立 platform 包 |
