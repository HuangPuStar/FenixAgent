...
function ChatRoute() {
  ...
  // 引用预览状态：当引用预览打开时，禁止 ArtifactsPanel 自动展开，
  // 避免两个右侧面板同时可见造成"双对话框"视觉效果。
  const { preview: citationPreview } = useCitationPreview();
  ...
  // 窄屏（<=768px）自动折叠 ArtifactsPanel，避免挤压 chat 区
  // biome-ignore lint/correctness/useExhaustiveDependencies: 仅在 mount 时注册 media query 监听，artifactsPanelRef 是稳定引用
  useEffect(() => {
  ...
  return (
    <CitationPreviewScope artifactsPanelRef={artifactsPanelRef}>
      <Suspense
        fallback={
      ...
      >
        ...
        */}
          ...
          */}
            ...
            {/*
              ArtifactsPanel：引用预览打开时完全隐藏，避免与右侧预览面板重叠。
            */}
            {!citationPreview && (
              <ResizablePanel
