...
function getInitialSessionSnapshot(acpSessionId: string): SessionStateSnapshot {
  return {
    acpSessionId,
    status: "idle",
    loading: null,
 ...
 * - acpSessionId 变化时同步切换 Y.Doc（在渲染函数体内，非 useEffect）
 */
export function useSessionState(acpSessionId: string) {
  ...
  if (!storeRef.current) {
    storeRef.current = createYjsStore<SessionStateSnapshot>(
      ...
      getInitialSessionSnapshot(acpSessionId),
    );
  }
  ...
  //    注意：prevKeyRef 必须初始化为 null（非 acpSessionId），否则首次渲染时 key 相等，switchDoc 被跳过
  const prevKeyRef = useRef<string | null>(null);
  if (prevKeyRef.current !== acpSessionId) {
    prevKeyRef.current = acpSessionId;

    store.switchDoc(acpSessionId, () => {
      const ydoc = new Y.Doc();
      // 将 session ID 写入 Y.Doc meta，applyUpdate 据此校验是否接收传入的 update
