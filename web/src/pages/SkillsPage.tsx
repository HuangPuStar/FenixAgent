...
import {
    buildSkillUploadFormData,
    parseSkillUploadFiles,
    validateUploadBatch,
...
export function normalizeSkillUploadResult(
    response: unknown,
): SkillUploadResult {
    const data =
        unwrapConfigData<Partial<SkillUploadResult>>(response) ??
        (response as Partial<SkillUploadResult> | null);
    return {
...
export function getUploadConflictData(
    error: unknown,
): SkillUploadConflictResponse | null {
    if (
        !error ||
        typeof error !== "object" ||
        !("code" in error) ||
    ...
    if (
        ...
        !Array.isArray(data.allowedStrategies)
    ) {
        return null;
    }
    return data;
...
export function getInvalidUploadSkillNames(
    items: UploadSkillSummary[],
): string[] {
    return items
        .filter((item) => !item.hasSkillMd)
        ...
        <div
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <span className="font-mono text-sm font-medium text-text-bright truncate">
                        {item.skillName}
                    </span>
...
const directoryInputProps = { webkitdirectory: "", directory: "" } as Record<
    string,
    string
>;
...
export function SkillsPage() {
    ...
    const [conflicts, setConflicts] = useState<
        SkillUploadConflictResponse["conflicts"]
    >([]);
    const [_conflictStrategy, setConflictStrategy] =
        useState<SkillUploadConflictStrategy | null>(null);
    ...
    const loadSkills = useCallback(async () => {
        setLoading(true);
        try {
            const { data: res, error: resErr } =
                await client.web.config.skills.post({ action: "list" });
            if (resErr)
                throw new Error(resErr.message ?? t("toast.loadListFailed"));
    ...
    const handleOpenEdit = async (skill: SkillInfo) => {
        ...
        try {
            const { data: res, error: resErr } =
                await client.web.config.skills.post({
                    action: "get",
                    name: skill.name,
                });
            if (resErr) {
    ...
    const handleTextSave = async () => {
        ...
        try {
            ...
            if (setErr)
                throw new Error(setErr.message ?? t("toast.saveFailed"));
            toast.success(
                editingSkill
                    ? t("toast.skillUpdated")
                    : t("toast.skillCreated"),
            );
            setDialogOpen(false);
            loadSkills();
    ...
    const handleUploadSubmit = async (
        ...
        try {
            ...
            const result = normalizeSkillUploadResult(
                await fetchUpload<unknown>(
                    "/web/config/skills/upload",
                    formData,
                ),
            );
            toast.success(
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (delErr)
                throw new Error(delErr.message ?? t("toast.deleteFailed"));
                ...
                selected.map((s) =>
                    client.web.config.skills
                        ...
                        .then((r: { error?: { message?: string } }) => {
                            if (r.error)
                                throw new Error(
                                    r.error.message ?? t("toast.deleteFailed"),
                                );
                        }),
                ),
            );
        ...
        <div className="p-6 space-y-4">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={() => handleOpenCreate("text")}>
                        {t("btn.createSkill")}
                    </Button>
                </div>
            </div>
            ...
            <FormDialog
                ...
                submitLabel={
                    editingSkill || createMode === "text"
                        ? t("dialog.save")
                        : t("dialog.startUpload")
                }
                loading={
                    editingSkill || createMode === "text"
                        ? formSaving
                ...
                disabled={
                    !editingSkill &&
                    createMode === "upload" &&
                    uploadItems.filter((i) => i.hasSkillMd).length === 0
                }
                width="sm:max-w-4xl">
                {!editingSkill ? (
                    <Tabs
                        ...
                        <TabsContent value="upload" className="space-y-4">
                            ...
                            {conflicts.length > 0 && (
                                <div className="space-y-3 rounded-lg border border-warning-border bg-warning-bg px-4 py-3 text-sm">
                                    <div className="font-medium text-warning-text">
                                        {t("conflict.title")}
                                    </div>
