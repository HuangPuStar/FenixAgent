...
import { buildMcpPayload, type KeyValueEntry, stringifyMcpCommand, validateMcpEditor } from "./agent-mcp-utils";
...
export function AgentMcpDialog({ target, onClose, onSaved }: Props) {
  ...
  const save = useRequest(
    async () => {
      ...
      return server ? unwrap(mcpApi.update(server.name, payload)) : unwrap(mcpApi.create(form.name, payload));
    },
    {
      manual: true,
      onSuccess: () => {
        toast.success(server ? t("toast.serverUpdated") : t("toast.serverCreated"));
        onSaved();
        onClose();
      },
      onError: (error) => {
  ...
  return (
    ...
    >
      <div className="space-y-4" aria-busy={loadingDetail}>
        ...
        <Field label={t("form.type")} hint={form.type === "local" ? t("form.typeLocalDesc") : t("form.typeRemoteDesc")}>
          <Select
            value={form.type}
