...
export function FileTreeTab({
    ...
    const loadTree = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `/web/environments/${envId}/user-file/tree`,
                {
                    credentials: "include",
                },
            );
    ...
    const handleRename = useCallback(
        async (path: string) => {
            const currentName = path.endsWith("/")
            ...
            try {
                const res = await fetch(
                    `/web/environments/${envId}/user-file/rename`,
                    {
                        method: "POST",
    ...
    const handleDelete = useCallback(
        async (path: string) => {
            if (
                !window.confirm(`${t("fileTree.contextMenu.delete")}: ${path}?`)
            )
                return;
            try {
                const res = await fetch(
                    `/web/environments/${envId}/user-file/batch`,
                    {
                        method: "DELETE",
                        headers: { "Content-Type": "application/json" },
                        credentials: "include",
    ...
    const handleNewFolder = useCallback(
        async (parentPath: string) => {
            ...
            try {
                const res = await fetch(
                    `/web/environments/${envId}/user-file/mkdir`,
                    {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        credentials: "include",
    ...
    const handleDrop = useCallback(
        async (e: React.DragEvent) => {
            ...
            const treePath = dataTransfer.getData(
                "application/pierre-tree-path",
            );
            if (treePath) {
                const dirPath = treePath.endsWith("/")
                    ? treePath
                    : `${treePath}/`;
