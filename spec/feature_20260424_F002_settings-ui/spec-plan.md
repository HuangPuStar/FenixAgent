...
**背景:**
当前 Sidebar 只有 Dashboard、Session、API Keys 三个入口，且 SidebarItem 接口不支持分隔线渲染。需要新增 4 个配置模块入口（服务商、模型、代理、技能），并按要求在 API 密钥下方、退出上方插入 Separator 分隔线。
...
- [ ] 更新 pageTitle 计算逻辑
  ...
  - 将逻辑改为:
    ...
    const pageTitle = useMemo(() => {
      ...
      if (configView === "agents") return "代理";
...
- [ ] 在 `web/src/pages/AgentsPage.tsx` 中创建 AgentsPage 组件，实现页面整体布局
  ...
  - 页面布局: `div.h-full.overflow-y-auto` > `div.mx-auto.max-w-6xl.px-6.py-6` > 标题行（"代理" + 新建按钮）+ DataTable
...
- [ ] 实现批量删除逻辑
  ...
  - 批量删除处理:
    ...
    const handleBatchDelete = async () => {
      ...
      toast.success(`已删除 ${customNames.length} 个代理`);
      setSelectedIds(new Set());
      setDeleteConfirm({ open: false, names: [] });
      await loadAgents();
    };
    ```
  - 批量删除前弹出 ConfirmDialog，提示"确定要删除选中的 N 个自定义代理吗？"
  - 原因: 批量删除需过滤掉内置 Agent，仅删除自定义 Agent

- [ ] 实现设为默认逻辑
  - 位置: `AgentsPage` 组件内
  - 逻辑: 调用 `apiSetDefaultAgent(name)` → 成功后 `toast.success("已将 xxx 设为默认代理")` + 刷新列表
...
2. 验证 Sidebar 显示 7 个入口 + 分隔线
   ...
   - 预期: Dashboard + Session + API Keys 为一组，下方有分隔线，服务商/模型/代理/技能为另一组，再下方分隔线后是退出
   - 失败排查: 检查 Task 2 Sidebar.tsx 的 navItems 配置

3. 验证 4 个配置页面懒加载正常
   - 依次点击 Sidebar 的"服务商"、"模型"、"代理"、"技能"入口
