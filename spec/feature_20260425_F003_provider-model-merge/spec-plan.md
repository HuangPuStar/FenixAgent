...
- [x] 从 `pageTitle` 的 titles 映射中移除 providers 条目
  ...
  - 将 `const titles: Record<string, string> = { providers: "服务商", models: "模型", agents: "代理", skills: "技能" };` 改为 `const titles: Record<string, string> = { models: "模型", agents: "代理", skills: "技能" };`
...
- [x] 为迁移后的函数运行单元测试验证
  ...
  - 测试场景（已有，无需新增）:
    ...
    - `buildProviderPayload("", "http://api.test.com", "@ai-sdk/openai", "MyProvider")` → 含 baseURL/npm/name 的对象
