...
每完成一个站点的创建或部署后，**必须**在聊天回复中输出以下标签。该标签会被渲染为可点击卡片，用户点击后右侧面板自动切换到 Sites 视图并加载该站点。
...
- **单独一行**，前后不要加任何文字、不要和其他内容挤在同一行
- **不要包裹在 markdown 代码块中**（不要用 ``` ``` ` 包围），否则不会渲染为卡片而是纯文本
- **不要加缩进、列表前缀（`-`、`*`、`1.`）、引用前缀（`>`）**
...
```
站点已创建，访问地址：https://xxx/app-abc123/
...
```
- <agent-sites agent-site-id="app-abc123"/>   ← 不要加列表前缀
`<agent-sites agent-site-id="app-abc123"/>`    ← 不要包裹在行内代码中
```
