...
- `static.ts`：静态文件挂载
- `require-team-scope.ts`：统一组织级资源归属校验 helper（`requireOrgScope`），验证 environment→organization 链路所有权
...
- 表：
  - Team 权限：`organization`、`member`、`invitation`（better-auth organization 插件自动管理）
