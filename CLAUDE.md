...
- 如果接口既不是给 `/web/*` 场景使用，也不是给 `/api/*` 场景使用，而是内部协议、系统桥接、Webhook、静态资源下载或框架透传能力，应放在 `src/routes/` 下独立文件或目录中，并使用独立前缀（如 `/mcp/*`、`/acp/*`、`/hooks/*`、`/skills/*`），不要混进 `src/routes/web/` 或 `src/routes/api/`。
...
- 既不属于 `/web/*` 也不属于 `/api/*` 的内部接口，默认按内部使用处理，OpenAPI 文档通常应设置 `detail.hide: true`；只有明确要求对外展示时才公开到文档中。
