...
// mcp / skill / provider / model 的配置面已在任务 1.2 迁入各自资源包，宿主 `services/config/index.ts`
// 与其 `upsertSystemMcpServer` 包装在任务 1.5c 作为零消费方死代码删除，此处不再为它安装替身。
//
// 用户偏好（`getUserConfig` / `setUserConfig`）在任务 1.5c 随 `user_config` 表归位 identity，按 identity
// 的仓储模块登记——与 `.../identity/src/services/system-api` 的既有做法一致（mock 按解析后的模块路径生效，
// 包入口的 re-export 会取到替身）。替身注册表仍是同一个 `config-pg-stub`，用例侧的 `stubConfigPg()` 无需改动。
...
// 模型标签端口同样绑真实实现（理由同上：`getModelManagementDatabase()` 同源于 platform-sdk 的
// `getDatabase()`，读的是本文件换过的替身）。漏绑不会 fail-fast 到「端口未绑定」，只会让模型标签静默
// 退化成 id，因此这里与 machine 的绑定放在一起，避免后续批次漏掉。
