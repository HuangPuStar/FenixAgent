...
// model-management 等资源包共用，而资源包不得依赖 platform-impl，因此守卫留在宿主并由宿主注入。
// 本用例验证的是**宿主装配后的已发布合同**（路由 + 守卫 + `RCS_SYSTEM_API_KEYS`），
// 所以随宿主测试运行，而不是放进 identity 包。
const apiSystemRoute = createApiSystemRoutes({ systemApiGuardPlugin: systemApiAuthPlugin });
